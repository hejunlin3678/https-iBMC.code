 ...
 * the KMC_KEY_TYPE_ENCRPT_INTEGRITY、KMC_KEY_TYPE_ASYM_ENCRYPT_DECRYPT、KMC_KEY_TYPE_ASYM_SIGN_VERIFY key type can be added.
 */
/**
 ...
 *  - Time consuming:   no consuming operation.
 */
unsigned long KmcCreateSyncCtx(WsecHandle *syncCtx, WsecVoid *sendParam, CallbackSendSyncData syncSend, WsecVoid *recvParam, CallbackRecvSyncData syncRecv);
 ...
 * If no negotiation capability set is available, the product needs to invoke this interface to set the negotiation capability set.
 * @param[in]  syncCtx sync ctx, create in KmcCreateSyncCtx.
