...
import { AlertMessageService, CommonService, ErrortipService, SystemLockService, TimeoutService, UserInfoService } from 'src/app/common-app/service';
import { SecondPasswordComponent } from 'src/app/common-app/components/second-password/second-password.component';

@Component({
    selector: 'app-home-quick-access',
...
export class HomeQuickAccessComponent implements OnInit {
    ...
    private nodeChange() {
        if (this.access.nodeIp) {
            const url = `https://${this.access.nodeIp}`;
            window.open(url);
        }
    }
