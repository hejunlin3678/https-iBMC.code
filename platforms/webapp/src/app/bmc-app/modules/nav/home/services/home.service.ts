...
export class HomeService {
    ...
    public overViewData: Observable<any>;
    public genericInfo: Observable<any>;
    ...
    constructor(
        ...
        this.overViewData = this.getData().pipe(share());
        this.genericInfo = this.http.get('/UI/Rest/GenericInfo').pipe(share());
    ...
    private factoryResourceInfo(overviewData): IResourceInfo {
        ...
        if (fan && !hideFansAndPower()) {
            const {
                MaxNum: fanMaxNum,
                CurrentNum: fanCurrentNum,
                FanHealthCode: fanStatus
            } = fan;
            resourceInfo = {
                ...resourceInfo,
                fanMaxNum,
        ...
        if (netadapter) {
            const {
                ...
                NetadapterHealthCode: netadapterStatus
            } = netadapter;
            resourceInfo = {
                ...resourceInfo,
                netMaxNum,
    ...
    // 查询VNC登录信息，用以匹配虚拟控制台可用选项
    public getVncInfo(timingRequest: boolean) {
        const header = {
            timingRequest,
            ignoreKeepAlive: true
        };
        return this.http.get(`/UI/Rest/Services/VNC`, header).pipe(
            pluck('body')
        );
    }
