...
import { CommonService, UserInfoService, TimeoutService, LanguageService, AlertMessageService } from 'src/app/common-app/service';
...
export class HomeVirtualControllerComponent implements OnInit {
    ...
    private downloadFile(right: string) {
        ...
        this.homeService.exportKvmStartupFile(params).subscribe((res) => {
            ...
            this.alert.eventEmit.emit({type: 'error', label: 'REMOTE_ERR_LINK_FAILED'});
        });
    }
    ...
    private searchKvmActiveStatus(timingRequest: boolean): void {
        forkJoin([this.homeService.searchKvmActiveStatus(timingRequest), this.homeService.getVncInfo(timingRequest)])
            .subscribe(([kvmData, vncData]) => {
                const data = kvmData['body'];
                const visibleJavaKvm = data?.VisibleJavaKvm;
                const activeStatus = data?.ActivatedSessionsType || vncData?.ActivatedSessionsType || [];
                const allDisabled = data?.MaximumNumberOfSessions === data?.NumberOfActiveSessions ||
                                    vncData?.MaximumNumberOfSessions === vncData?.NumberOfActiveSessions;
