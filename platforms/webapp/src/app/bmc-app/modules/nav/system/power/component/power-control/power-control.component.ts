...
import { AlertMessageService, UserInfoService, LoadingService, SystemLockService, PRODUCTTYPE } from 'src/app/common-app/service';
...
export class PowerControlComponent implements OnInit, OnDestroy {
    ...
    public notTurnOnRo: boolean = false;
        ...
        confirm: () => {
            const param = {
                PowerOffAfterTimeout: {
                    TimeoutSeconds: parseInt(this.powerOff.value, 10)
                }
            };
    ...
    init() {
        // TCE当PowerOnStrategy的值为TurnOnRO时，不显示通电开机策略
        this.notTurnOnRo = this.powerContorlInfo.getPowerOnStrategy() !== 'TurnOnRO';
