...
import { LeakDetection, LeakDetectionArray } from '../../models';
...
export class LeakDetectionService {
    ...
    factory(activeCard: CardUrlArray): Observable<LeakDetectionArray> {
        const leakDetectionArray = new LeakDetectionArray();
        ...
        if (activeCard.getCardUrlArray && activeCard.getTotalCount) {
            activeCard.getCardUrlArray.forEach((url) => {
                post$.push(this.othersService.getData(url).pipe(map((bodyDatas) => {
                    ...
                    leakDetectionArray.addLD(new LeakDetection(boardName, pCBVersion, boardId, partNumber));
                    });
                })));
            });
        }
        return forkJoin(post$).pipe(
            map(() => leakDetectionArray)
        );
    }
}
