...
import { AlertMessageService, CommonService, GlobalDataService, HttpService } from 'src/app/common-app/service';
...
export class NavComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        // 关闭预置证书提示
        this.globalData.closePresetCert.subscribe((data) => {
            if (data && this.label === 'PRESET_CERT_TIP') {
                this.open = false;
            }
        });
        ...
        this.subscription = this.alertMessage.eventEmit.subscribe((data) => {
            ...
            if (data.type === 'success' || data.type === 'warn') {
                this.time = 5000;
            } else {
                this.time = 10000;
            }
    ...
    // 当处于Web服务页面，预置证书提示出现时，点击链接跳转无效，此时进行重新刷新，展示最新证书信息
    public reload() {
        if (this.route.url.indexOf('web') > -1) {
            location.reload();
        }
    }
