...
export class Comunication {
    ...
    _MsgDispatch(data) {
        switch (data[0]) {
            ...
            case packet.C_CMD_RSP_SUITE:
                ...
                break;
            case packet.H_CMD_SET_CIPHER_ALGO_SUITE_RSP:
                this._ProcSetCipherAlgoSuiteRsp(data[2]);
                break;
            ...
            case packet.H_CMD_REPORT_KVMKEY:
                ...
                if (data.length > 128) {
                    ...
                    this._client._security.generateReconnectKey(data.slice(2, data.length));
                    ...
                    this._client._security.generateKvmKeyAndIv(data.slice(2, data.length), this._remainKvmKeyAndIv);
                }
                break;
    ...
    _ProcSuiteList(data) {
        ...
        while (i < count && (i + 1) * 5 + offset <= data.length) {
            ...
            if (Number(hmac) === 3) {
                ...
                // 发送设置加密算法和padding方式消息
                const algoType = 0; // 使用AES-CBC算法加密业务数据
                const padding = 1; // 使用PKCS7Padding方式
                pack = packet.setCipherAlgoSuitePacket(0, this._client._security, algoType, padding);
                this._Send(pack);
                break;
            }
            i++;
    ...
    _ProcSetCipherAlgoSuiteRsp(state) {
        let pack = null;

        if (state === 1) { // 1表示协商成功
            this._client._security.setPaddingStyle(1); // 协商成功设置使用pkcs7pading,否则使用默认的NoPadding
        }
