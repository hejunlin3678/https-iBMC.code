...
export class Security {
    ...
    _WordToUint8(data) {
        ...
        const maxIndex = Math.floor(sigBytesLength / ratio);
        for (let i = maxIndex; i >= 0; i--) {
            let min;
            if (i === maxIndex) {
                min = ratio - sigBytesLength % ratio;
            } else {
                min = 0;
            }
            for (let j = min; j < ratio; j++) {
                const temp = words[i] >>> j * OUTPUT_BYTE_LENGTH & 0xff;
                arr.unshift(temp);
            }
        }
    ...
    kvmEncrypt(data) {
        ...
        if (this._padding === 0) {
            ...
            dataW = this._Uint8ToWord(dataAlign);
        } else {
            dataW = this._Uint8ToWord(data);
        }
    ...
    generateKvmKeyAndIv(data, remainKvmKeyAndIv) {
        const copyedRemainArr = remainKvmKeyAndIv.slice(0, remainKvmKeyAndIv.length);
