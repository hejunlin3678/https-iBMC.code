...
import { CommonService, GlobalDataService, HttpService, LanguageService, PRODUCT, PRODUCTTYPE } from 'src/app/common-app/service';
...
export class KvmH5Component implements OnInit {
    ...
    public hideResetModule: boolean = false;
    ...
    init() {
        if (localStorage.getItem('H5ClientModel')) {
            ...
            this.showMessage(this.translate.instant('REMOTE_ERR_SESSION_EXPIRATION'));
        }
        if (this.globalData.productType === PRODUCTTYPE.PANGEA ||
            localStorage.getItem('productType') === PRODUCTTYPE.PANGEA ||
            CommonData.productType === PRODUCT.PANGEA) {
            this.hideResetModule = true;
        }
    }
