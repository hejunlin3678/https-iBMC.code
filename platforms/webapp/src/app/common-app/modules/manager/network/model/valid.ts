...
export class IpV4Vaild {
    ...
    // 校验当前控件是否与目标控件在同一网段, 第二个参数为比较控件（子网掩码）
    public static isInSameNetSegment(targetPrefix: string, maskPrefix: string): ValidatorFn {
        return (control: AbstractControl): ValidationErrors | null => {
            const errorModel = {
                isEqualAddress: {
                    tiErrorMessage: ''
            ...
            let targetControl;
            let maskControl;
            Object.keys(parentControls).forEach(key => {
                if (key.indexOf(targetPrefix) === 0) {
                    targetControl = parentControls[key];
                } else if (key.indexOf(maskPrefix) === 0) {
                    maskControl = parentControls[key];
                }
            });

            // 若IP地址、子网掩码地址、网关地址有无效的数据值（如null），则校验通过，注：删除IP值后，值变为....，4个点号
            if (!control.value || !maskControl.value || !targetControl.value) {
                return null;
            }
            const currentResult = control.value.split('.');
            const targetResult = targetControl.value.split('.');
            const maskResult = maskControl.value.split('.');
            const res1 = [];
            const res2 = [];

            for (let i = 0; i < currentResult.length; i++) {
                res1.push(parseInt(currentResult[i], 10) & parseInt(maskResult[i], 10));
                res2.push(parseInt(targetResult[i], 10) & parseInt(maskResult[i], 10));
            }
