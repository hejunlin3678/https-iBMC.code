...
export class NetworkLldpComponent implements OnInit, OnChanges {
    ...
    public protocolEnabledEN = () => {
        ...
        if (this.netLldpForm.root.get('lldpEnableState').value !== this.lldpParams['lldpEnable']) {
            this.param.LLDP['Enabled'] = this.netLldpForm.root.get('lldpEnableState').value;
        }
        if (parseInt(this.netLldpForm.root.get('sendControl').value, 10) !== this.lldpParams['lldpSend']) {
            this.param.LLDP['TxDelaySeconds'] = parseInt(this.netLldpForm.root.get('sendControl').value, 10);
        }
        if (parseInt(this.netLldpForm.root.get('preiodControl').value, 10) !== this.lldpParams['lldpPreiod']) {
            this.param.LLDP['TxIntervalSeconds'] = parseInt(this.netLldpForm.root.get('preiodControl').value, 10);
        }
        if (parseInt(this.netLldpForm.root.get('mutiplyControl').value, 10) !== this.lldpParams['lldpMutiply']) {
            this.param.LLDP['TxHold'] = parseInt(this.netLldpForm.root.get('mutiplyControl').value, 10);
        }
        if (JSON.stringify(this.param.LLDP) !== '{}') {
    ...
    public lldpButton(): void {
        ...
        this.networkService.saveNetWork(JSON.stringify(this.param)).subscribe((response) => {
            ...
            this.networkService.lldpDataDetails(this.networkLldp, error.error.data);
            this.lldpInit(this.networkLldp);
        });
    }
}
