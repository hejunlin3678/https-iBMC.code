...
import { PowerBoardArray, PowerBoard } from '../../models';
...
export class PowerBoardService {
    ...
    factory(activeCard: CardUrlArray): Observable<PowerBoardArray> {
        const powerBoardArray = new PowerBoardArray();
            ...
            activeCard.getCardUrlArray.forEach((url) => {
                post$.push(this.othersService.getData(url).pipe(map((bodyDatas) => {
                    bodyDatas.forEach((bodyData) => {
                        ...
                        powerBoardArray.addPowerBoard(new PowerBoard(boardName, deviceType, pCBVersion, boardId));
                    });
