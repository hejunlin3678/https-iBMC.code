...
import { LCD, LCDArray } from '../../models';
...
export class LcdService {
    ...
    factory(activeCard: CardUrlArray): Observable<LCDArray> {
        const lCDArray = new LCDArray();
        ...
        if (activeCard.getCardUrlArray && activeCard.getTotalCount) {
            activeCard.getCardUrlArray.forEach((url) => {
                post$.push(this.othersService.getData(url).pipe(map((bodyDatas) => {
                    ...
                    const lcdFirmwareVersion = isOtherEmptyProperty(bodyData.Version) || ' ';
                    lCDArray.addLCD(new LCD(lcdFirmwareVersion));
                    });
                })));
            });
        }
