...
if [ ! -f hpmimage_ext4 ]
then
   echo ERROR:hpmimage_ext4 is not exist!
   exit
fi
...
#8.1加密配置文件
chmod +x crypto_tool_ext4
./crypto_tool_ext4 encrypt "$CONF" "conf.tar.gz" oaep

#8.2加密升级文件
./crypto_tool_ext4 encrypt "$1" "$IPMCFILE" oaep

#9、打成整个包
chmod +x hpmimage_ext4
./hpmimage_ext4 $2
chmod 444 $APPHPM
