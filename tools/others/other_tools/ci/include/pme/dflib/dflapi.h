 ...
 ******************************************************************************
  ...
  生成日期   : 2013年8月14日
  最近修改   :
  功能描述   : app模块动态调用的框架库api函数接口定义
               供app/用户进程调用

            ==========================================================================
            所有app模块可以调用的接口都在本头文件中定义，超出本头文件的函数接口请勿调用
            ==========================================================================
...
Parameters    :  obj_handle
                 property_name
                 property_value(*)   函数返回成功后，需要调用g_variant_unref释放
Return        :
Description   : 获取属性值
...
Description   : 封装获取string属性值
              ...
              相比原接口，不再使用gvarint变量传递属性值，减少动态申请释放。
*****************************************************************/
extern gint32 dal_get_property_value_string(OBJ_HANDLE obj_handle, const gchar* property_name, gchar* out_val, guint32 out_len);
/*****************************************************************
Parameters    :  class_name
                 property_name
                 in_prop_val        属性值
...
Description   : 封装根据byte属性值查找对象的接口
              相比原接口，不再使用gvarint变量传递属性值，减少动态申请释放。
*****************************************************************/
extern gint32 dal_get_specific_object_byte( const gchar* class_name, const gchar* property_name,
                                guchar in_prop_val, OBJ_HANDLE* obj_handle );
/*****************************************************************
Parameters    :  class_name
                 property_name
                 in_prop_val        属性值
...
Description   : 封装根据int16属性值查找对象的接口
              相比原接口，不再使用gvarint变量传递属性值，减少动态申请释放。
*****************************************************************/
extern gint32 dal_get_specific_object_int16( const gchar* class_name, const gchar* property_name,
                                gint16 in_prop_val, OBJ_HANDLE* obj_handle );
/*****************************************************************
Parameters    :  class_name
                 property_name
                 in_prop_val        属性值
...
Description   : 封装根据uint16属性值查找对象的接口
              相比原接口，不再使用gvarint变量传递属性值，减少动态申请释放。
*****************************************************************/
extern gint32 dal_get_specific_object_uint16( const gchar* class_name, const gchar* property_name,
                                guint16 in_prop_val, OBJ_HANDLE* obj_handle );
/*****************************************************************
Parameters    :  class_name
                 property_name
                 in_prop_val        属性值
...
Description   : 封装根据int32属性值查找对象的接口
              相比原接口，不再使用gvarint变量传递属性值，减少动态申请释放。
*****************************************************************/
extern gint32 dal_get_specific_object_int32( const gchar* class_name, const gchar* property_name,
                                gint32 in_prop_val, OBJ_HANDLE* obj_handle );
/*****************************************************************
Parameters    :  class_name
                 property_name
                 in_prop_val        属性值
...
Description   : 封装根据uint32属性值查找对象的接口
              相比原接口，不再使用gvarint变量传递属性值，减少动态申请释放。
*****************************************************************/
extern gint32 dal_get_specific_object_uint32( const gchar* class_name, const gchar* property_name,
                                guint32 in_prop_val, OBJ_HANDLE* obj_handle );
/*****************************************************************
Parameters    :  class_name
                 property_name
                 in_prop_val        属性值
...
Description   : 封装根据double属性值查找对象的接口
              相比原接口，不再使用gvarint变量传递属性值，减少动态申请释放。
*****************************************************************/
extern gint32 dal_get_specific_object_double( const gchar* class_name, const gchar* property_name,
                                gdouble in_prop_val, OBJ_HANDLE* obj_handle );
/*****************************************************************
Parameters    :  class_name
                 property_name
                 in_prop_val        属性值
...
*****************************************************************/
extern gint32 dal_get_specific_object_position(OBJ_HANDLE obj_in, const gchar* class_name,
                                OBJ_HANDLE* obj_out);
/*****************************************************************
Parameters    :  obj_handle 输入对象
                 class_name 指定类名
                 obj_list   输出对象链表，调用者需释放链表，内容无需释放
Return        :
Description   : 新增根据某个对象查找同级position指定类的所有对象接口
*****************************************************************/
extern gint32 dal_get_object_list_position(OBJ_HANDLE obj_handle, const gchar* class_name, 
                                GSList** obj_list);
...
*****************************************************************/
extern gint32 dfl_set_property_value(OBJ_HANDLE obj_handle, const gchar *property_name, GVariant *property_value, DF_OPTIONS options);

/*****************************************************************
Parameters    :  obj_handle
                 property_name
                 property_value(*)  返回的GVariant*需要释放
                 options
Return        :
...
*****************************************************************/
extern gint32 dfl_get_saved_value(OBJ_HANDLE obj_handle, const gchar* property_name, GVariant** property_value, DF_OPTIONS options);
...
*****************************************************************/
extern gint32 dfl_get_saved_value_remote(OBJ_HANDLE obj_handle, const gchar* property_name, GVariant** property_value, DF_OPTIONS options);
extern gint32 dfl_get_saved_value_v2(OBJ_HANDLE obj_handle, const gchar* property_name, GVariant** property_value, DF_OPTIONS options);

/*****************************************************************
Parameters    :  obj_handle
                 property_name
                 options : 默认填0,
Return        : true--属性值已做持久化保存，false--执行失败或属性值未持久化保存
...
/*****************************************************************
Parameters    :  obj_handle
                 property_name_list     输入list<gchar*>
                 property_value_list(*) 输出list<GVariant*>   函数返回成功后，需要调用g_slist_free_full释放
Return        :
Description   :  同时获取对象的多个属性值
                 ...
                 这些属性值可能在状态上是不同步的
*****************************************************************/
extern gint32 dfl_multiget_property_value(OBJ_HANDLE obj_handle, GSList *property_name_list, GSList**property_value_list);
...
/*****************************************************************
Parameters    :  object_handle
              property_name
              accessor[MAX_NAME_SIZE]
Return        :
...
Parameters    :  object_handle
              property_name
Description   :  判断属性是否关联accessor
**************************************************************** */
extern gboolean dfl_is_property_relate_accessor(OBJ_HANDLE object_handle, const gchar *property_name);

/* ****************************************************************
Parameters    :  void
Description   :  是否在进行属性值的初始化值的设置
**************************************************************** */
extern gboolean dfl_is_setting_init_property(void);
...
Parameters    :  obj_handle
                 property_name
                 referencd_object[MAX_NAME_SIZE] --- 用数组是明确告知调用者所需内存空间大小
                 referencd_property[MAX_NAME_SIZE]
Return        :
...
/*****************************************************************
Parameters    :  obj_handle
                 referenced_object_property
                 referencd_handle
Return        :
Description   : 一个对象的某个属性可以引用另一个对象(名称)，通过自己的句柄和引用属性名称返回被引用的对象句柄
*****************************************************************/
extern gint32 dfl_get_referenced_object(OBJ_HANDLE obj_handle, const gchar *referenced_object_property, OBJ_HANDLE *referencd_handle);
...
*****************************************************************/
extern gint32 dfl_get_specific_object( const gchar* class_name, const gchar* property_name, GVariant* property_value, OBJ_HANDLE* obj_handle );
...
*****************************************************************/
extern gint32 dfl_call_class_method(OBJ_HANDLE obj_handle, const gchar *method_name, GSList* user_data_list, GSList *input_list, GSList**output_list);

/*****************************************************************
Parameters    :  obj_handle
                 method_name
                 property_value(*)   需要调用者释放
Return        :
Description   :  查询类方法的属性

                 方法也可以具备属性，例如方法的权限级别等。
*****************************************************************/
extern gint32 dfl_get_method_property(const gchar* class_name, const gchar* method_name,
...
*****************************************************************/
extern gint32 dfl_call_remote_class_method(  const gchar *object_name,
...
*****************************************************************/
extern gint32 dfl_get_remote_property_value2( const gchar *ip, gint32 port, const gchar *object_name, GSList *input_list, GSList **output_list);
...
typedef void (*FOREACH_EXCEPTION_FUNC)(const gchar* class_name, OBJ_HANDLE object_handle, gpointer user_data, gint32 result);
/*****************************************************************
Parameters    :  class_name
                 handler
                 user_data
...
Description   :  依据类名遍历回调对象处理，依次调用回调函数并输入对象句柄
                 框架负责维护对象的动态添加删除
*****************************************************************/
extern gint32 dfl_foreach_object(const gchar* class_name,
                                          OBJECT_FOREACH_FUNC handler,
                                          gpointer user_data,
                                          FOREACH_EXCEPTION_FUNC exception_func);

/*****************************************************************************
 函 数 名  : dfl_foreach_object_family
 功能描述  : 依据类名遍历回调对象处理，依次调用回调函数并输入对象句柄
             框架负责维护对象的动态添加删除, 会遍历子类对象
 输入参数  : const gchar* class_name
             OBJECT_FOREACH_FUNC handler
             gpointer user_data
             FOREACH_EXCEPTION_FUNC exception_func
 输出参数  : 无
...
*****************************************************************************/
extern gint32 dfl_foreach_object_family(const gchar* class_name,
                                                    OBJECT_FOREACH_FUNC handler,
                                                    gpointer user_data,
                                                    FOREACH_EXCEPTION_FUNC exception_func);
...
*****************************************************************/
extern gint32 dfl_send_ipmi_request(gsize req_size, gconstpointer request, gsize resp_size, gpointer response, gboolean wait_response);
...
*****************************************************************/
typedef gint32 (*PropertyEventFunc)(OBJ_HANDLE object_handle,
    gpointer user_data, gchar* property_name, GVariant *property_value);
// 注册类属性变更事件的回调处理函数
extern gint32 dfl_class_event_handler(const gchar *class_name,
...
*****************************************************************/
extern gint32 dfl_get_class_method_paralist(const gchar* class_name,
...
typedef gint32 (*DftCtrlFunc)(guint32 command, gsize para_size, gconstpointer para_data,
    DFT_RESULT_S *dft_data, gpointer user_data);
...
*****************************************************************/
extern gint32 dfl_register_dft_item(guint32 dft_id, guint32 slot, guint32 dev_id,
    DftCtrlFunc dft_func, gpointer user_data);
...
*****************************************************************/
extern gint32 dfl_block_read(OBJ_HANDLE obj_handle, const gchar *property_name,
...
Description   : 块设备写
*****************************************************************/
extern gint32 dfl_block_write(OBJ_HANDLE obj_handle, const gchar *property_name,
             gint32 offset, gsize length, gconstpointer in_data);
/*****************************************************************
Parameters    :  obj_handle
              property_name
              chip_name
...
Description   : 直接操作器件接口: 从器件读数据，以byte为单位
                ...
                bit 操作请使用byte读写接口
*****************************************************************/
extern gint32 dfl_chip_blkread(const gchar* chip_name,
             gint32 offset, gsize length, gpointer out_data);
...
*****************************************************************/
extern gint32 dfl_chip_byteread(const gchar* chip_name,
    gint32 offset, guint32 mask, gpointer out_data);
...
*****************************************************************/
extern gint32 dfl_chip_bytewrite(const gchar* chip_name,
    gint32 offset, guint32 mask, gpointer in_data);
...
*****************************************************************/
extern gint32 dfl_chip_blkwrite(const gchar* chip_name,
             gint32 offset, gsize length, gpointer in_data);
...
Description   :  读取硬件，获得属性对应的硬件的值，依据options选项确定是否同步到当前属性值
                 ...
                 ** 注意: 除非你非常确信这个值一定在所有产品上都必须从硬件上
                          获取，你才需要调用此接口
                    推荐使用dfl_get_extern_value接口替代
*****************************************************************/
extern gint32 dfl_get_hardware_value(OBJ_HANDLE obj_handle,
             const gchar *property_name, GVariant **value, DF_OPTIONS options);

/*****************************************************************
Parameters    :  obj_handle
              property_name
              value(*)        返回的GVariant*外部需要释放
              options
Return        :
Description   :  灵活接口。
                 DF_MEM  查询时从内存读取
...
*****************************************************************/
extern gint32 dfl_get_extern_value(OBJ_HANDLE obj_handle,
    const gchar *property_name, GVariant **value, DF_OPTIONS options);
...
/*****************************************************************************
 函 数 名  : dfl_get_class_info_by_id
 功能描述  : 根据功能号获取类名称
 输入参数  : guint16 class_id       : 对应的类ID
 输出参数  : gchar* class_name      : 类名称
 返回值    : gint32                 : 操作结果，成功/失败
*****************************************************************************/
extern gint32 dfl_get_class_info_by_id( guint16 class_id, gchar class_name[MAX_NAME_SIZE] );


/*****************************************************************************
 函 数 名  : dfl_get_property_info_by_id
 功能描述  : 根据功能号、信息号获取类名称、属性名称及属性类型
 输入参数  : guint16 class_id       :对应的类ID
             guint16 property_id    :对应的属性ID
 输出参数  : gchar** class_name     :类名称
             gchar** property_name  :属性名称
*****************************************************************************/
extern GVariantType* dfl_get_property_info_by_id( guint16 class_id, guint16 property_id,
    gchar class_name[MAX_NAME_SIZE], gchar property_name[MAX_NAME_SIZE] );


/*****************************************************************************
 函 数 名  : dfl_get_method_info_by_id
 功能描述  : 根据功能号、方法号获取类名称、方法名称
 输入参数  : guint16 class_id       :对应的类ID
             guint16 method_id      :对应的方法ID
 输出参数  : gchar* class_name     :类名称
             gchar* method_name    :方法名称
 返 回 值  : gint32                 :操作结果

*****************************************************************************/
extern gint32 dfl_get_method_info_by_id( guint16 class_id, guint16 method_id,
    gchar class_name[MAX_NAME_SIZE], gchar method_name[MAX_NAME_SIZE] );


/*****************************************************************************
 函 数 名  : dfl_dynamic_create_object
 功能描述  : 动态创建对象
 输入参数  : const gchar* connector_name : connector对象名称
             OBJECT_FILE object_file   : 将要添加的动态文件句柄
 输出参数  : 无
 返 回 值  : gint32
 调用函数  : 
 被调函数  : 
 说    明  : 1.单个模块只进行一次性创建，如果创建对象时，已存在动态创建对象
...
/*****************************************************************************
 函 数 名  : dfl_dynamic_create_object_file
 功能描述  : 创建对象文件句柄
 输入参数  : 无
 输出参数  : 无
 返 回 值  : OBJECT_FILE
...
/*****************************************************************************
 函 数 名  : dfl_dynamic_create_object_item
 功能描述  : 创建对象文件单项条目
 输入参数  : OBJECT_FILE  obj_file 
...
*****************************************************************************/
extern OBJECT_ITEM dfl_dynamic_create_object_item( OBJECT_FILE obj_file, const gchar* object_name, const gchar* class_name );

/*****************************************************************************
 函 数 名  : dfl_dynamic_add_property
 功能描述  : 添加对象的属性内容
 输入参数  : OBJECT_ITEM object_item
             const gchar* property_name
             const gchar* property_type
...
*****************************************************************************/
extern gint32 dfl_dynamic_add_property( OBJECT_ITEM object_item, const gchar* property_name, const gchar* property_type, const gchar* property_value );
...
*****************************************************************************/
extern gint32 dfl_get_property_info(const gchar* class_name, guint32* property_num, PROPERTY_INFO_S **properity_info_list);

/*****************************************************************************
 函 数 名  : dfl_foreach_class
 功能描述  : 遍历类
 输入参数  : ClassForeachFunc handler  
 输出参数  : 无
 返 回 值  : gint32
 调用函数  : 
 被调函数  : 
...
/*****************************************************************************
 函 数 名  : dfl_manage_module
 功能描述  : 模块管理接口
 输入参数  : const gchar* module_name  注册时使用的模块名称
             guint32 options 管理选项: 0, 模块去使能
                                       1,模块使能
 输出参数  : 无
 返 回 值  : gint32
 调用函数  : 
 ...
 修改历史      :
  1.日    期   : 2017年1月11日
    作    者   : lingmingqiang
    修改内容   : 新生成函数
    说    明   : options目前只支持0和1两种取值；模块disable主要对进程状态进行修改，
...
/*****************************************************************************
 函 数 名  : dfl_set_rpc_timeout
 功能描述  : 设置全局rpc调用超时时间
 输入参数  : guint32 timeout:超时时间，单位ms
 输出参数  : 无
 返 回 值  : void
 ...
 修改历史      :
  1.日    期   : 2020年2月26日
    ...
    修改内容   : 新生成函数
*****************************************************************************/
extern void dfl_set_rpc_timeout(guint32 timeout);

/*****************************************************************************
 函 数 名  : dfl_get_rpc_timeout
 功能描述  : 获取全局rpc调用超时时间
 输入参数  : 无
 输出参数  : 无
 返 回 值  : 超时时间，单位ms
 ...
 修改历史      :
  1.日    期   : 2020年2月26日
    ...
    修改内容   : 新生成函数
*****************************************************************************/
extern guint32 dfl_get_rpc_timeout(void);
