...
/**
 * 清除json对象中指定属性(必须是字符串对象)的内存数据.
 *
 * @param jso 指向需要清除数据的json对象, 传入类型必须是json object对象
 * @param prop_name json对象中需要清除数据的属性名称，指向的属性在jso中必须以json字符串对象存在
 *
 */
JSON_EXPORT void json_object_clear_string(json_object *jso, const char *prop_name);

/**
 * 清空json字符串对象中的内存数据
 *
 * @param jso 指向需要清除数据的json对象，传入的对象必须为json字符串对象
 *
 */
JSON_EXPORT void json_string_clear(json_object *jso);
