...
/**
 * @ingroup AdaptiveLM
 * @brief 根据当前license状态查询当前已激活或被去激活的License文件内容。
 *
 * @par 描述: 与ALM_LicGeneralQuery类似，返回的SBOM的值、SBOM中BBOM的值、BBOM的值、PACKAGE的值会随License状态、时间变化。
 *
 * @attention 参数pstLic，使用前注意初始化：ALM_StInitLicStore，使用后注意清理输出：ALM_StClsLicStore。
 *
 * @li Memory operation: alloc、free、size:
 * -# 该接口会动态分配参数pstLic内存，使用前注意初始化：ALM_StInitLicStore，使用后注意清理输出：ALM_StClsLicStore。
 * @li Thread safe:
 * -# 线程安全，允许调用者多任务重入</br>
 ...
 * -# 非耗时接口</br>
 *
 * @param iPrdId [IN] 参数类型#ALM_INT32，指定产品编号。
 * @param pstLic [INOUT] 参数类型#ALM_LIC_STORE_STRU *，查询得到的License文件的内容。
 * @param pstLicAttri [OUT] 参数类型#ALM_LIC_ATTRI_STRU *，查询得到的License文件的若干属性。
 *
 * @retval #ALM_OK 0 成功。
...
/* 描述Spart信息: 含Spart的名字和整数量，Spart的转换系数，以及Spart下的Bpart列表 */
typedef struct {
 ...
 * 描述: 计算Bpart-Spart装换。
 * 参数: 3个输入参数、1个输出参数
 * pBpart - 指定Bpart数据，包含Bpart的名字和整数量，忽略浮点数量。
 * pSbRelation - Spart信息。
 * uiNum - 输入多少个Spart。
 * pRsts - 输出参数，Bpart分配到了哪些Spart上，。
 */
...
/**
 * @ingroup AdaptiveLM
 * @brief 忽略ESN=ANY的已运行时间，比较指定License文件与当前已激活License文件。
 *
 * @par 描述: 同ALM_LicGeneralCmp功能。如果License ESN=ANY，则忽略ESN=ANY的已运行时间。
 *
 * @attention 参数pstCmpRslt，使用前注意初始化：ALM_StInitLicCmp，使用后注意清理输出：ALM_StClsLicCmp。
 *
 * @li Memory operation: alloc、free、size:
 * -# 该接口会动态分配参数pstCmpRslt内存，使用前注意初始化：ALM_StInitLicCmp，使用后注意清理输出：ALM_StClsLicCmp。
 * @li Thread safe:
 * -# 线程安全，允许调用者多任务重入</br>
 * @li OS/CPU difference:
 * -# 无OS差异</br>
 ...
 *
 * @param iPrdId [IN] 参数类型#ALM_INT32，指定产品编号。
 * @param pcLicStr [IN] 参数类型#CONST ALM_CHAR *，License文件内容, 需要包含C语言字符串结束符'\0’。
 * @param uiLicLen [IN] 参数类型#ALM_UINT32，License文件长度。
 * @param pstCmpRslt [INOUT] 参数类型#ALM_LIC_CMP_STRU *，得到的比较结果。
 *
 * @retval #ALM_OK 0 成功。
 * @retval #其他返回值 失败 - 对应错误码详见API手册。
 */
