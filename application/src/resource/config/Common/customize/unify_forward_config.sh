...
SAVEFILE=unify_forward_config_save.txt
FLAG_ADD_USER=0
...
judge_pcert_state()
{
    ...
    ./upgrade_hpm.sh "wbd_verify.hpm" 0
    if [ $? -ne 0 ]; then
...
# 归一产品特殊化功能接口函数
unify_forward_custom_config()
{
    if [ ! -f "unify_custom_config.sh" ]; then
        echo_fail "no unify_custom_config.sh"
        echo "no unify_custom_config.sh" >>$SAVEFILE
        error_quit
    fi
    
    # 0代表正向定制内容  1代表逆向定制内容
    source ./unify_custom_config.sh 0
}
...
load_driver
# 判断是否有efuse，如果无efuse则升级支持伙伴模式efuse,如果是老efuse,则报错退出
upgrade_efuse
# 判断是否有根,如果有根，则退出脚本
judge_pcert_state
#删除BMA
