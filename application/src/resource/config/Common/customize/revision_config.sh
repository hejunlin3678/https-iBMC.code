#!/bin/sh
############################################
# bash script: 软件防回退脚本，用于定制BMC/BIOS/CPLD版本限制回退
############################################

REPEAT=6
REPEAT_TIME=1 
CONFIGFILE=revision.ini
parameter=0
IPMI_CMD_LOG_FILE=custom.log
IPMI_CMD_RETURN_FILE=ipmi_cmd_return_string.txt

# 打包时，文件被重命名为0502xxx_load.sh,取出0502xxx具体名称
...
#设置主板CPLD版本防回退RevisionNumber
getini $CONFIGFILE CPLDRevisionNumber
RevisionNumber=$parameter
FirmwareType=0xfc
if [ "$RevisionNumber" != "" ] ; then
	set_ipmi_revision_number $FirmwareType $RevisionNumber
	echo "Set CPLD revision number to $RevisionNumber success!"
	echo "Set CPLD revision number to $RevisionNumber success!" >>$SAVEFILE
else
	set_ipmi_revision_number $FirmwareType 0
fi
...
#查询CPLD版本防回退RevisionNumber是否设置成功
getini $CONFIGFILE CPLDRevisionNumber
RevisionNumber=$parameter
FirmwareType=0xfc
if [ "$RevisionNumber" != "" ] ; then
	check_revision_number $FirmwareType $RevisionNumber
	echo "Check CPLD revision number success!"
	echo "Check CPLD revision number to $RevisionNumber success!" >>$SAVEFILE
else
	check_revision_number $FirmwareType 0
fi
