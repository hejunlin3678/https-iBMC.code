...
if [ $? != 0 ] ; then
    echo "ipmicmd import Repair certificate failed"
    echo "ipmicmd import Repair certificate failed" >>$SAVEFILE
    error_quit
fi
echo "ipmicmd import Repair certificate successfully"
