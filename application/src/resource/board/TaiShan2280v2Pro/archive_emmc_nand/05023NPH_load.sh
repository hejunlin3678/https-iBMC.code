...
function backward_sp_function()
{
	...
	if [ "${filter_file}" == "" ]; then
		echo "no upgrade file ,please make surre"
		error_quit
	else	
		if [ -d "${filter_file}" ]; then
			cd ${filter_file}
			...
			echo_fail "no upgrade file ,please make surre"
			error_quit
		fi
	fi
...
get_bmc_upgrade_status()
{
    ...
    for((i=0;i<600;i++))
	{
		...
		if [ $((16#$status)) -eq 228 ] ; then
		   ...
		   echo "The $i time get BMC upgrade status is $((16#$status))%" >>$SAVEFILE
		fi
...
start_bma_upgrade()
{
	...
	http_code=$(curl -k -s -u ${UserName}:${Password} -H "Content-Type:application/json" -X POST -d '{"Mode":"Upgrade", "ImageURI":"/tmp/bma_image.tar.gz", "SignatureURI":"/tmp/bma_image.tar.gz.cms", "CrlURI":"/tmp/bma_image.tar.gz.crl"}' "https://[${IPV6_ADDR}]/redfish/v1/Managers/${managers_id}/SmsService/Actions/SmsService.RefreshInstallableBma" -o /dev/null -s -w %{http_code})
	echo "http_code=${http_code}"
    if [ $? -ne 0 -o ${http_code} -ne 202 ]; then
...
get_bma_upgrade_status()
{
    ...
    do
        #判断redfish是否是通路，如果不通，则重复发送，每次间隔1秒。
        http_code=$(curl -k -s -u ${UserName}:${Password} "https://[${IPV6_ADDR}]/redfish/v1/TaskService/Tasks" -o /dev/null -s -w %{http_code})
        ...
        then
		    ...
		    task_id=$(curl -k -s -u ${UserName}:${Password} "https://[${IPV6_ADDR}]/redfish/v1/TaskService/Tasks" | awk -F "\"Members\":" '{print $2}'| awk -F "\"" '{print $4}'| awk -F "/" '{print $NF}')
			if [ $? -ne 0 -o -z ${task_id} ];
			then
			    echo "task_id=${task_id}"
                echo_fail "Failed to get task id."
                error_quit
			else
                echo "task_id=${task_id}"
                echo "Get task id successfully." >>$SAVEFILE
                break
            fi
		fi
	done
	for id in $task_id
	do 
	    response_body=$(curl -k -s -u ${UserName}:${Password} "https://[${IPV6_ADDR}]/redfish/v1/TaskService/Tasks/${id}")
