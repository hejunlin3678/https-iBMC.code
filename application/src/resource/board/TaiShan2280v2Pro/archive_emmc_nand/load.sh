...
check_release_bmc_version()
{
    ...
    #为了兼容以前的版本，版本号小于3.02.00.06的版本作为生产包处理
...
upgrade_efuse_info()
{
	...
	hpm_upgrade efuse-crypt-image-partner.hpm
    if [ $? != 0 ] ; then
        echo "updrade efuse fail"
        echo "updrade efuse fail" >>$SAVEFILE
        error_quit
    fi
}
