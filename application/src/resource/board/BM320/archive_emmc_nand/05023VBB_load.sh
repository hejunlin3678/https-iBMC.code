...
#bma升级
function upgrade_bma()
{
	#校验文件是否存在
	image_file_path=${G_WORK_DIR}/bma_image/bma_image.tar.gz
	if [ ! -f "$image_file_path" ] ; then
	   echo_fail "image file is not present!"
	   echo "image file is not present!" >>$SAVEFILE
	   error_quit
	fi
	signal_file_path=${G_WORK_DIR}/bma_image/bma_image.tar.gz.cms
	if [ ! -f "$signal_file_path" ] ; then
	   echo_fail "signal file is not present!"
	   echo "signal file is not present!" >>$SAVEFILE
	   error_quit
	fi
	crl_file_path=${G_WORK_DIR}/bma_image/bma_image.tar.gz.crl
...
function upgrade_bma_function()
{
    ...
    for file_name in `ls ${G_WORK_DIR}`
    do
        if [ "${file_name}" == "bma_image.zip" ]; then
            filter_zip_file=${file_name}
            echo "${filter_zip_file}"
            filter_file=`echo ${file_name} | awk -F "." '{print $1}'`
            echo "${filter_file}"
            echo "find BMA upgrade zip file"
            break
        fi
    done
    if [ -d "${filter_file}" ]; then
    ...
    if [ "${filter_zip_file}" == "" ]; then
        echo "no bma upgrade zip file, please make sure the zip file"
        error_quit
    else
        unzip ${filter_zip_file}
    fi
    ...
    if [ "${filter_file}" == "" ]; then
        ...
        if [ -d "${filter_file}" ]; then
            ...
            echo_fail "no bma upgrade file ,please make surre"
            error_quit
        fi
    fi
