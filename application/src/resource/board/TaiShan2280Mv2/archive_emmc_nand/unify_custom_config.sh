...
function query_produce_name()
{
    ...
    dir=([0x0201A900]="S920X00" [0x0201A901]="S920X00" [0x0201A902]="S920X01" [0x0201A903]="S920X01" \
		 ...
		 [0x0207A502]="S920S01" [0x0207A503]="S920S01" [0x0207A904]="S920X00")
...
set_machine_alias_function()
{
    ...
    if [ ${strlen} -gt 16 ]; then
        ...
        echo "the machine_alias is too long" >>$CUSTOM_LOG_FILE
        error_quit
    fi
    ...
    if [ $? != 0 ]; then
        ...
        echo "set_machine_alias fail" >>$CUSTOM_LOG_FILE
        error_quit
    fi
...
set_machine_alias()
{
	...
	do
		...
		then
			...
			echo "the uid ${ProductUniqueID} match machine_alias and alias is ${machine_alias}" >>$CUSTOM_LOG_FILE
			break
		fi
	done
