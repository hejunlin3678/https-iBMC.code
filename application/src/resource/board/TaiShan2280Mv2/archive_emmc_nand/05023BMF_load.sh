...
SAVEFILE=05023BMF_save.txt
FLAG_ADD_USER=0
ENVIRONMENT_TYPE=0
EQUIPMENT_SEPARATION_SUPPORT_STATE=0
G_WORK_DIR=`pwd`
UID_X00_1U2P=0x0207A904
UID_X00Pro_1U2P=0x0207A905
UID_X00_2U2P=0x0207A900
UID_X01_2U1P=0x0207A902
UID_X00_4U2P=0x0207A901
UID_X00_4U1P=0x0207A903
UID_S00_2U2P=0x0207A500
UID_S00_4U2P=0x0207A501
UID_S01_2U1P=0x0207A502
UID_S01_4U1P=0x0207A503
UID_X00K_2U2P=0x0207A90A
UID_X01K_2U1P=0x0207A90C
UID_X00K_4U2P=0x0207A90B
UID_S00K_2U2P=0x0207A50A
UID_S00K_4U2P=0x0207A50B
PCERT_JUDGE_FLAG=1
...
# 判断环境类型
judge_environment_type()
{
    # 获取环境 uid
    uid_list=("0x0207A900" "0x0207A901" "0x0207A902" "0x0207A903" "0x0207A904" \
              "0x0207A90A" "0x0207A90B" "0x0207A90C" \
              "0x0207A500" "0x0207A501" "0x0207A50A" "0x0207A50B" \
              "0x0207A502" "0x0207A503" \
              "0x0207A905")

    for compare_uid in ${uid_list[*]}
    do
        if [[ "${compare_uid}" == "$machine_uid" ]]; then
            echo_success "this machine is 1711 type"
            return 1
        fi
    done

    echo_success "this machine isnot 1711 type"
    return 0
}
...
{	
    ...
    # 判断环境类型,如果是1710机型则无分区删除BMA失败重试机制
    delete_bma_repeat_time=1
    judge_environment_type
    if [ $? == 0 ]; then
        delete_bma_repeat_time=5
    fi
    for ((k=0; k<${delete_bma_repeat_time}; k++))
    do
        ...
        then
            if [ ${k} -ge $((delete_bma_repeat_time-1)) ]; then
                if [ ${delete_bma_repeat_time} == 5 ]; then
                    echo_success "it's no BMA partition, so execte pass"
                    echo "it's no BMA partition, so execte pass" >>$SAVEFILE
                else
...
function query_produce_name()
{
	...
	declare -A dir
	#创建一个字典进行uid和产品名称的对照
	dir=([0x0201A900]="S920X00" [0x0201A901]="S920X00" [0x0201A902]="S920X01" [0x0201A903]="S920X01" \
		 [0x0201A90A]="S920X00K" [0x0201A90B]="S920X00K" [0x0201A90C]="S920X01K" \
		 [0x0201B900]="S920X00" [0x0201B901]="S920X00" [0x0201B902]="S920X01" [0x0201B903]="S920X01" \
		 [0x0201B90A]="S920X00K" [0x0201B90B]="S920X00K" [0x0201B90C]="S920X01K" \
         [0x0201A500]="S920S00" [0x0201A501]="S920S00" [0x0201A50A]="S920S00K" [0x0201A50B]="S920S00K" \
		 [0x0207A900]="S920X00" [0x0207A901]="S920X00" [0x0207A902]="S920X01" [0x0207A903]="S920X01" \
		 [0x0207A90A]="S920X00K" [0x0207A90B]="S920X00K" [0x0207A90C]="S920X01K" \
		 [0x0207A500]="S920S00" [0x0207A501]="S920S00" [0x0207A50A]="S920S00K" [0x0207A50B]="S920S00K" \
		 [0x0207A502]="S920S01" [0x0207A503]="S920S01" [0x0207A904]="S920X00" [0x0207A905]="S920X00(1U)(Pro)")
...
request_machine_uid
# K机型进行无根环境校验，确保环境无根
if [ $machine_uid == $UID_X00K_2U2P ] || [ $machine_uid == $UID_X01K_2U1P ] || [ $machine_uid == $UID_X00K_4U2P ] || [ $machine_uid == $UID_S00K_2U2P ] || [ $machine_uid == $UID_S00K_4U2P ] || [ $machine_uid == $UID_X00_1U2P ]; then
...
# 非K机型进行导根操作
if [ $machine_uid == $UID_X00_2U2P ] || [ $machine_uid == $UID_X01_2U1P ] || [ $machine_uid == $UID_X00_4U2P ] || [ $machine_uid == $UID_X00_4U1P ] || [ $machine_uid == $UID_S00_2U2P ] || [ $machine_uid == $UID_S00_4U2P ] || [ $machine_uid == $UID_S01_2U1P ] || [ $machine_uid == $UID_S01_4U1P ]; then
    # 判断是否导入过根，如果导入过s1和s2，则返回值为1,则不进行导根。如果为0，则进行导根操作
    load_pcert 1
    if [ $? == 1 ]; then
        # 校验根是否正常导入，且是华为根
        echo "start verify the pcert"
