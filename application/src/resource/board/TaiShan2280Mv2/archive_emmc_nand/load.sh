...
VRD_VERSION_FLAG=0
PRODUCT_UID_1=0x0207A9FF
PRODUCT_UID_2=0x0207A5FF
...
bmc_vrd_version_check()
{
    ipmicmd -k "0f 00 30 93 db 07 00 36 89 00 01 20 00 00 ff ff 00 01 00 02 00" smi 0 > vrd_version_change_flag.txt
    if [ $? -eq 0 ] ; then    #命令执行成功
        VRD_VERSION_FLAG=`cat vrd_version_change_flag.txt | awk '{print $NF}'`  #取最后一行
    fi	
	
    echo "vrd version change flag  : $VRD_VERSION_FLAG" 
    echo "vrd version change flag  : $VRD_VERSION_FLAG" >>$SAVEFILE
    rm -f vrd_version_change_flag.txt
}
...
check_userid_null()
{
	...
	run_cmd_uid "f 00 06 46 $userid" "0f 07 00 46 00 00 00 00 00 00 00 00 00 00"
	if [ $? != 0 ] ; then
...
verify_h2p_info()
{
    run_cmd "0f 00 30 93 db 07 00 5b 2a 00" "0f 31 00 93 00 db 07 00 00"
    if [ $? != 0 ]; then
        echo_fail "verify h2p info fail"
        echo "verify h2p info fail" >>$SAVEFILE
        return 1
    else
        echo_success "verify h2p info successfully"
        echo "verify h2p info successfully" >>$SAVEFILE
        return 0
    fi    
}
...
upgrade_efuse_judge()
{
	...
	do
		...
		if [ -z ${ProductUniqueID} ]; then	
		    echo_fail "fail to get ProductUniqueID ${ProductUniqueID} count $i"
			sleep 1
		else
		    echo_success "success to get ProductUniqueID ${ProductUniqueID}"
			break
	    fi
	done
	check_userid_null 17
	...
	if [ ${ProductUniqueID} == ${PRODUCT_UID_1} ] || [ ${ProductUniqueID} == ${PRODUCT_UID_2} ] ; then
	    echo "This ProductUniqueID not updrade"
	    return 0;
	fi

	hpm_file_path=${G_WORK_DIR}/efuse-crypt-image-partner.hpm
	if [ ! -f "$hpm_file_path" ] ; then
		return 0;
	fi
...
if [ $? -ne 0 ]; then
    ...
    # 校验h2p秘钥信息 返回 1 ：代表h2p秘钥信息不存在或者错误   返回 0 ：代表h2p秘钥信息正确
    verify_h2p_info
    if [ $? != 0 ]; then
        # 如果h2p秘钥不存在或者校验失败，则通过升级工厂包，进行h2p秘钥信息的加载或者替换
        upgrade_function "factory_image.hpm" 1
        upgrade_function "image.hpm" 1
    fi
    ...
    verify_h2p_info
    if [ $? != 0 ]; then
        echo_fail "after upgrade BMC version, the h2p info also fail"
        echo "after upgrade BMC version, the h2p info also fail" >>$SAVEFILE
        error_quit
    fi
fi

# 再次进行h2p校验，确保是在装备脚本调用下升级的，确保h2p区域的搬迁
verify_h2p_info
if [ $? != 0 ]; then
    # 如果h2p秘钥不存在或者校验失败，则通过升级工厂包，进行h2p秘钥信息的加载或者替换
    upgrade_function "factory_image.hpm" 1
