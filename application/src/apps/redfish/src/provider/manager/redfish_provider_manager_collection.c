...
LOCAL gint32 get_managers_collection_members(PROVIDER_PARAS_S *i_paras, json_object **o_message_jso,
    json_object **o_result_jso);


LOCAL PROPERTY_PROVIDER_S g_managers_collection_provider[] = {
    {  RFPROP_MEMBERS, MAP_PROPERTY_NULL, RFPROP_ODATA_ID, USERROLE_READONLY, SYS_LOCKDOWN_NULL, get_managers_collection_members, NULL, NULL, ETAG_FLAG_ENABLE },
};
    ...
    json_object **o_result_jso)
{
    ...
    iRet = snprintf_s(slot_uri, sizeof(slot_uri), sizeof(slot_uri) - 1, URI_FORMAT_MANAGER, pslot);
...
gint32 manager_provider_collection_entry(PROVIDER_PARAS_S *i_paras, PROPERTY_PROVIDER_S **prop_provider, guint32 *count)
{
    *prop_provider = g_managers_collection_provider;
    *count = sizeof(g_managers_collection_provider) / sizeof(PROPERTY_PROVIDER_S);
