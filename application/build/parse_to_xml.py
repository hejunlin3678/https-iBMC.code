...
def set_env():
    ...
    g_llt = args.llt
    g_xml_path = f"{g_common.code_path}/dependency_for_hpm"
...
def get_version():
    ...
    """
    os.chdir(os.path.join(g_common.code_path, "dependency_for_hpm/downxml"))
    ...
    os.chdir(os.path.join(g_common.code_path))
    return ele_version.text


def update_oem_component(component_content):
    """
    ...
    返回值：更新后的解析结果
    """
    logger.info("adding OEM component_content...")
    oem_component_dir = os.path.join(g_common.oem_config_path,"OEM_component")
    if not os.path.exists(oem_component_dir):
        logger.info(f"no OEM component_content found in {oem_component_dir}")
...
def plat_parse(ele_dependencies, plat_components, dependency_content):
    """
    功能：平台文件下载xml格式化
    ...
    返回值：无
    """
    plat_dep = dependency_content.get("board").get(g_board_name).get("Platform")
    logger.info("Parse component platform")
    if plat_components == None or len(plat_dep) == 0:
        logger.info(f"Platform part dependency is empty")
        return
...
def third_party_parse(ele_dependencies, third_party_components, dependency_content):
    """
    功能：第三方文件下载xml格式化
    参数说明：  ele_dependencies        xml配置的ET句柄
    ...
    返回值：无
    """
    logger.info("Parse component third part")
    third_party_dep = dependency_content.get("board").get(g_board_name).get("ThirdParty")
    if third_party_dep == None:
        logger.info("Independent of third-party components")
        return

    if third_party_components == None or len(third_party_dep) == 0:
        logger.info(f"Third part dependency is empty")
        return
...
def component_common_parse(ele_dependencies, component_name, offering):
    """
    功能：其他组件文件下载xml格式化
    参数说明：  ele_dependencies     xml配置的ET句柄
    ...
    if component_name == None:
        logger.info(f"This component is empty")
        return
    ...
    for component in component_name.get("Artifactory"):
        ...
        # 仅zip包需要dft工具
        if args.no_packet and ele_componentName.text == "DftTools":
            continue
...
def components_json2xml(board_name, conponent_json="component.json", dependency_json="dependency.json"):
    """
    功能：将传入json格式化为xml
    ...
    domTree = ET.parse(f"{g_xml_path}/downxml/dependency.xml")
    root = domTree.getroot()
    ...
    # 加载json文件到变量
    component_content = getJsonContent(conponent_json)
    dependency_content = getJsonContent(dependency_json)

    # 有OEM时解析并覆盖部分组件配置，以用来替换
    component_content = update_oem_component(component_content)
    ...
    third_party_grp = component_content.get("ThirdPartyGroupware")
    ...
    # 对各组件解析
    ele_dependencies = root.find("dependencies")
    plat_parse(ele_dependencies, plat_components, dependency_content)
    third_party_parse(ele_dependencies, third_party, dependency_content)
    ...
    components_correspond_table = {
        ...
        "HostSec": hostsec,
        "ThirdPartyGroupware": third_party_grp,
    ...
    components_dep = dependency_content.get("board").get(g_board_name).get("Component")
    for component in components_dep:
        logger.info(f"Parse component {component}")
        ...
        if component == "BinCMC" and component_conf != None:
            for bin_cmc_component in component_conf.get("Artifactory"):
                if bin_cmc_component['name'] == "BinIBMC":
                    bin_cmc_component['version'] = get_version()
        component_common_parse(ele_dependencies, component_conf, offering_table.get(component, "iBMC V2"))


    # 写入到目标目录
    xml_dest_path = f"{g_xml_path}/downxml/board"
    g_common.check_path(xml_dest_path)
    logger.info(f"Generate xml file is {xml_dest_path}/{board_name}_dependency.xml")
...
if __name__ == "__main__":
    set_env()
    if g_llt == True:
        components_json2xml(g_board_name, "llt_component.json", "llt_dependency.json")
    else:
        components_json2xml(g_board_name)
