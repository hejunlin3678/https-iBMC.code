...
# coding: utf-8
"""
功 create_firmware_k_pkg.py脚本 -- 用于生成鲲鹏K与非K需求互转，非K转K
...
class FirmwareK(Work):
    def __init__(self, config, work_name=""):
        super(FirmwareK, self).__init__(config)
        self.config = config
        self.archive_conf_dic = self.common.get_archive_config(self.config.board_name)
        self.release_dir = f"{self.config.board_name}_hqe_release"
    ...
    def run(self):
        # 没有配置 togdp_dir_hqe ，不需要打包
        if self.archive_conf_dic.get("togdp_dir_hqe", "") == "":
            return
        self.logger.info(f"create compressed packet for FirmwareK of hqe PKG ------------> [begin]")
        # 创建临时目录
        ...
        # 生成ToGDP目录下的zip包
        os.chdir(self.config.build_path)
        target_pkg = f"{self.archive_conf_dic['togdp_name_hqe']}{ver_num}"
        self.common.check_path(target_pkg)

        # 拷贝脚本及镜像文件到目标目录
        config_file_tuple = (
            f"{self.archive_emmc_nand_dir}/{self.archive_conf_dic['togdp_hqe_0502']}_load.sh",
        ...
        self.common.copy(f"{target_pkg}/hqe.sh", 
            f"{target_pkg}/{self.archive_conf_dic['togdp_hqe_0502']}_config.sh")
        self.common.copy(f"{target_pkg}/hqe.sh", 
            f"{target_pkg}/{self.archive_conf_dic['togdp_hqe_0502']}_check.sh")
        self.common.copy(f"{target_pkg}/hqe.sh", 
            f"{target_pkg}/{self.archive_conf_dic['togdp_hqe_0502']}_verify.sh")
        ...
        shutil.copy(ar_ver_file, f"{target_pkg}/{self.archive_conf_dic['togdp_hqe_0502']}_version.ini")
        subprocess.run(f"sed -i '/^archive_version/s/{old_archieve_ver}/{major_ver}.{minor_ver}.{release_major_ver}.{release_minor_ver}/g' {target_pkg}/{self.archive_conf_dic['togdp_hqe_0502']}_version.ini", shell=True)
        subprocess.run(f"sed -i '/^ActiveBMC/s/{old_archieve_ver}/{major_ver}.{minor_ver}.{release_major_ver}.{release_minor_ver}/g' {target_pkg}/{self.archive_conf_dic['togdp_hqe_0502']}_version.ini", shell=True)
        subprocess.run(f"sed -i '/^BackupBMC/s/{old_archieve_ver}/{major_ver}.{minor_ver}.{release_major_ver}.{release_minor_ver}/g' {target_pkg}/{self.archive_conf_dic['togdp_hqe_0502']}_version.ini", shell=True)

        os.chdir(self.config.build_path)
        self.run_command(f'zip -rq "{target_pkg}".zip {target_pkg}')
        self.common.remove_path(f"{self.common.work_out}/packet/{self.archive_conf_dic['togdp_dir_hqe']}")
        self.common.check_path(f"{self.common.work_out}/packet/{self.archive_conf_dic['togdp_dir_hqe']}")
        self.logger.info(f"move file to  '{self.common.work_out}/packet/{self.archive_conf_dic['togdp_dir_hqe']}'")
        shutil.move(f"{self.config.build_path}/{target_pkg}.zip", f"{self.common.work_out}/packet/{self.archive_conf_dic['togdp_dir_hqe']}")
        self.common.remove_path(f"{self.config.build_path}/{target_pkg}")
        self.logger.info(f"create compressed packet for FirmwareK of hqe PKG ------------> [end]")
