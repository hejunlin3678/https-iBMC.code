...
#ifndef __IPSI_FILE_H__
#define __IPSI_FILE_H__
...
*/
IPSILINKDLL int ipsi_fopen(ipsi_file_handle_t *stream, const char *path, const char *mode);
...
*/
IPSILINKDLL size_t ipsi_fread(void *buffer, size_t size, ipsi_file_handle_t stream);
...
The ipsi_fread function reads size elements of data, each unitSize
    ...
    @retval         size_t  Number of elements read.
*/
IPSILINKDLL size_t ipsi_fread_ex(void *buffer, size_t unitSize, size_t size, ipsi_file_handle_t stream);
...
*/
IPSILINKDLL size_t ipsi_fwrite(const void *buffer, size_t size, ipsi_file_handle_t stream);
...
/*
    ...
    @retval         On Successful Completion it returns the number of byte
                    ...
                    On Failure it returns IPSI_FAILURE.
*/
IPSILINKDLL long ipsi_filelength_ex(const char *path, const ipsi_file_handle_t stream);
...
/*
    ...
    @retval         On Successful Completion it returns the number of byte
                    ...
                    On Failure it returns IPSI_FAILURE.
*/
IPSILINKDLL SEC_INT64 ipsi_filelength64_ex(const char *path, const ipsi_file_handle_t stream);
...
*/
/* 2 fmt位置 0 可变参位置 */
IPSILINKDLL int ipsi_vfprintf(ipsi_file_handle_t stream, const char *format, va_list argptr) IPSIFORMAT(printf, 2, 0);
...
*/
IPSILINKDLL int ipsi_fseek(ipsi_file_handle_t stream, long lOffset, int iOrigin);
...
*/
IPSILINKDLL int ipsi_fseek64(ipsi_file_handle_t stream, SEC_INT64 lOffset, int iOrigin);
...
/*
    ...
    @retval         On Successful Completion it returns success
                    On Failure it returns negative value.
*/
IPSILINKDLL int ipsi_fgets(char *cBuff, unsigned int uiSize, ipsi_file_handle_t stream);
...
*/
IPSILINKDLL int ipsi_set_filesecurityinfo(const char *pcPath, const void *psidOwner, const void *psidGroup);
...
*/
IPSILINKDLL int ipsi_set_fileattributes(char *pcFileName, unsigned long ulAttrib);
...
*/
IPSILINKDLL int ipsi_set_filetime(const char *pcFileName, const ipsi_utimebuff_t pstUtimeBuffer);
...
*/
IPSILINKDLL int ipsi_createdirectory(const char *pcDirName, unsigned int uiMode, unsigned int uiMask);
...
*/
IPSILINKDLL int ipsi_removedirectory(const char *pcDirName);
...
*/
IPSILINKDLL int ipsi_chmod(const char *pcFileName, int iMode);
...
*/
IPSILINKDLL int ipsi_deletefile(const char *pcFileName);
...
/*
    ...
    @retval         This function returns success if it is able to get the handle
                    and the first directory entry else returns failure.
*/
IPSILINKDLL int ipsi_open_initial_dir(const char *pucBuffer, ipsi_dir_handle_t *dpDirHandle,
                                      ipsi_dir_entries_t *dpDirEntries, char **dpucOutBuff);
...
/*
    ...
    @par Note  Input buffer pucBuffer should contain the additional memory to concatinate atleast three char's
                    For the platforms windows,dopra,dopraV2,visp,vispDopraV2 else causes undefined behaviour.

*/
IPSILINKDLL int ipsi_opendir(const char *pucBuffer, ipsi_dir_handle_t *dpDirHandle, ipsi_dir_entries_t *dpDirEntries,
                             char **dpucOutBuff);
...
*/
IPSILINKDLL int ipsi_readdir(const ipsi_dir_handle_t dirHandle, ipsi_dir_entries_t *dirEntries, char **dpucOutBuff,
...
/*
    ...
    @param          pcBuf The output buffer
                    ...
                    uiLen  The maximum length that of the output buffer
...
*/
IPSILINKDLL char *ipsi_fullpath(const char *pcBuf, const char *pcPathBuf, unsigned int uiLen, int *piError);
...
*/
IPSILINKDLL int ipsi_set_permission(const char *pcFileName, unsigned int uiMode, unsigned int uiMask);
...
*/
IPSILINKDLL int ipsi_list_directory(char *pcFullPath, char *pcOutBuff, unsigned int uiBuffSize);

/*
    @brief          ipsi_set_filepath_validate_permission_cb:This function is used to
                    set the callback for filepath validation and permission callback. Callback for filepath
                    cannocalization and validation will be called before the file open and the permission callback
                    will be called after the file creation

                    IPSI_CANONICALIZE_VALIDATE_FILEPATH_CB has the parameter  inPath, outPath and uiOutPathLen which
                    are described below inPath provides the input of the application outPath is the output path after
                    the cannocalization of the inPath uiOutPathLen is the length of the outPath.
                    Application must validate path after the cannocalization and if the verification is successful than
                    copy the cannocalized path in the outPath and return IPSI_SUCCESS else IPSI_FAILURE.
                    outPath must be updated as a string. The size of the outPath is mentioned in uiOutPathLen. The
                    string should be with in the uiOutPathLen including the null character

    @param          pCanonicalizeValidateCb cannocalization and validation of cannocalized path
                    pPermissionCb   file permission callback after the file creation.
                    Application can set the file permission

    @retval         N/A

    @par Note       pPermissionCb will not be called if the file is opened in read mode. If the mode is anything apart
                    from read, the file permission callback will be called.
                     So for the existing files also the callback will be called if opened in write or append mode.

*/
IPSILINKDLL int ipsi_set_fopen_validate_permission_cb(IPSI_CANONICALIZE_VALIDATE_FILEPATH_CB pCanonicalizeValidateCb,
    IPSI_FILE_PERMISSION_CB pPermissionCb);
