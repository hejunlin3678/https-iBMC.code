...
#ifndef __IPSI_NET_H__
#define __IPSI_NET_H__
...
/*
    ...
    @retval         int     Number of bytes received On Success.
                    ...
                    -1      On Failure.
*/
IPSILINKDLL int ipsi_recvfrom(ipsi_sock_id_t s, void *buf, size_t len, int flags, void *from, void *fromlen,
                              int *pErrno);
...
/*
    ...
    @retval         int     Number of bytes received On Success.
                    ...
                    -1      On Failure.
*/
IPSILINKDLL int ipsi_recvfrom_timeout(ipsi_sock_id_t s, void *buf, size_t len, int flags, void *from, void *fromlen,
                                      const IPSI_TIMEVAL_S *timeout, int *pErrno);
...
/*
    ...
    @retval         int     Number of bytes sent On Success.
                    -1      On Failure.
*/
IPSILINKDLL int ipsi_sendto(ipsi_sock_id_t s, const void *msg, size_t len, int flags, const void *to, size_t tolen,
                            int *pErrno);
...
/*
    ...
    @retval         int     Get socket option.
                    -1      On Failure.
*/
IPSILINKDLL int ipsi_getsockopt(ipsi_sock_id_t s, int level, int optname, void *optval, void *optlen);

/*
    @brief          The ipsi_getsockopt function is used to mtu from the socket

    @param[in]      s       The socket ID.
    @param[in]      optval       mtu value.
    @param[in]      optlen       mtu length.
    @retval         int     Get socket option.
                    -1      On Failure.
*/
IPSILINKDLL int ipsi_getsockmtu(ipsi_sock_id_t s, void *optval, void *optlen);
...
/*
    ...
    @retval         int     Set socket option
                    -1      On Failure.
*/
IPSILINKDLL int ipsi_setsockopt(ipsi_sock_id_t s, int level, int optname, const void *optval, int optlen);
IPSILINKDLL ipsi_sock_id_t ipsi_socket(int af, int type, int protocol);

IPSILINKDLL int ipsi_bind(ipsi_sock_id_t sockfd, ipsi_sock_addr_t addr, int namelen);

IPSILINKDLL int ipsi_connect(ipsi_sock_id_t sockfd, ipsi_sock_addr_t *addr, int namelen);

IPSILINKDLL int ipsi_ioctlsocket(ipsi_sock_id_t sockfd, long cmd, unsigned long *parg);

IPSILINKDLL int ipsi_send_ex(ipsi_sock_id_t sockfd, const void *msg, size_t len, int flags);

IPSILINKDLL int ipsi_recv_ex(ipsi_sock_id_t sockfd, void *buf, size_t len, int flags);

IPSILINKDLL int ipsi_select(int nfds, void *readfds, void *writefds, void *exceptfds, void *timeout);

IPSILINKDLL int ipsi_select_ex(int nfds, void *readfds, void *writefds, void *exceptfds, void *timeout);
