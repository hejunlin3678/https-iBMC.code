...
/* IPSI_LDAP_SYMBOL_RESOLUTION - macro needs to be defined in user makefile, if user getting symbol */
 ...
 */
typedef struct stCRMF_PKMACValue {
    SEC_ALG_IDENTIFIER_S *pstAlgId; /* PwdBasedMac with PBMParameter */
    SEC_AsnBits_S value;            /* Contains a pwd-based MAC on the
...
typedef struct stCRMF_POPOSigningKeyInputChoice {
    ...
    union unCRMF_POPOSigningKeyInputChoice {
        ...
        CRMF_PKMAC_S *publicKeyMAC; /* publicKeyMAC contains a pwd-based MAC
 ...
 * @datastruct pstAlgorithmIdentifier Algorithm for Signing POP SigningKeyInput or the Cert Request in the Certificate
 * Request Message.
 ...
 * @datastruct pstPubLocation General Name indicating where the requester would like the certificate to be found
 * (Optional).
 */
typedef struct stCRMF_SinglePubInfo {
 ...
 * @datastruct raVerified AsnNull Value - used if the RA has already verified that the requester is in possession of the
 * private key.
 ...
 * @datastruct pstRegInfo Contains supplementary information related to the context of the certification request
 * (Optional).
 */
typedef struct stCRMF_CertReqMsg {
...
/* This Control Option enables subscribers to supply
    information needed to form an archive of the private key
...
typedef struct stGMEncryptedContentInfo {
    ...
    SEC_AsnOcts_S stSharedInfo1;         /*  sharedInfo1 is the result of encrypting the content */
    SEC_AsnOcts_S stSharedInfo2;         /*  sharedInfo1 is the result of encrypting the content */
} GM_ENCRYPTED_INFO_S;

typedef struct stGMEnvelopedData {
    SEC_AsnInt version;                              /* It shall be 1 for this version of the standard. */
    SEC_List_S *pstRecipientInfos;                   /* RecipientInfos is a group of per-recipient information.
   There must be at least one element in the group. */
    GM_ENCRYPTED_INFO_S *pstEncryptedContentInfo; /* pstEncryptedContentInfo is the encrypted content information.
                                                        It include information like type of Content encrypted ,
                                                        Algorithm used to encrypt and encrypted data */
} GM_ENVELOPED_S;

typedef enum enGmEncryptKeyChoiceId {
    GM_ENCRYPTED_KEY_ENVELOPEDDATA = 0
} GM_ENCRYPTEDKEY_E;
typedef struct stGM_EncryptedKey {
    GM_ENCRYPTEDKEY_E enEncryptedKeyType; /* Choice to specify EncryptedKey Type */
    union unGm_EncryptedKey {
        GM_ENVELOPED_S *pstGmEncryptedEnveloped;
    } unEncryptedKey;
} GM_ENCRYPTEDKEY_S;
 ...
 * @retval CRMF_CertReqMsgList* If the input is NULL or on memory allocation failure [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
...
* @retval CRMF_CERT_REQ_MSG_S* If either of the inputs are NULL or on memory allocation failure[SEC_NULL|N/A]
*
* @par Dependency
* crmf.h
*
...
* N/A
*/
PSELINKDLL CRMF_CERT_REQ_MSG_S *CRMF_createCertReqMsg(SEC_INT32 iCertReqID, CRMF_CERT_TEMPLATE_S *pstCertTemp);
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 CRMF_addCertReqMsg(CRMF_CertReqMsgList *pCertReq, CRMF_CERT_REQ_MSG_S *pstCertReqMsg);
...
* const SEC_AsnBits_S* pstIssrUID,
* const SEC_AsnBits_S* pstSubjUID,
...
* @retval CRMF_CERT_TEMPLATE_S* If the Input is NULL or on memory allocation fail [SEC_NULL|N/A]
*
* @par Dependency
* crmf.h
*
...
*
*/
PSELINKDLL CRMF_CERT_TEMPLATE_S *CRMF_createCertTemplate(CRMF_REQMSG_TYPE_E enReqMsgType, SEC_UINT32 ulVersion,
    SEC_UINT32 ulHashAlgID, SEC_NAME_S *pstIssuer, SEC_NAME_S *pstSubject, CRMF_OPT_VALIDITY_S *pstValidity,
    SEC_PUBKEY_INFO_S *pstPubKeyInfo, const SEC_AsnBits_S *pstIssrUID, const SEC_AsnBits_S *pstSubjUID,
    const X509_CERT_S *pstCert);
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 CRMF_addExtns(CRMF_CERT_TEMPLATE_S *pstCertTemplate, Extensions *pExtnlist);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 CRMF_addControls (const CRMF_CERT_REQ_MSG_S* pstCertReqMsg,
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 CRMF_addControls(const CRMF_CERT_REQ_MSG_S *pstCertReqMsg, SEC_ATTR_TYPE_AND_VALUE_S *pstControl);
...
* N/A
*/
PSELINKDLL SEC_UINT32 CRMF_addRegInfo(CRMF_CERT_REQ_MSG_S *pstCertReqMsg, SEC_ATTR_TYPE_AND_VALUE_S *pstRegInfo);
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 CRMF_addPOPSigningKey(SEC_UINT32 ulHashAlgID, SEC_PKEY_S *pPrivKey,
    CRMF_POPOSIGNING_KEY_TYPE_E enPOPKeyInputType, SEC_GEN_NAME_S *pSender, SEC_UINT32 ulPBAlgID,
    SEC_UCHAR *pucPassword, SEC_UINT32 ulPwdLen, SEC_UINT32 ulOWFAlgID, SEC_UINT32 ulMACAlgID,
...
* const SEC_PKEY_S* pstPrivateKey,
* const SEC_PKEY_S* pstPublicKey,
* SEC_UINT32 uiSubsequentMsgCh,
* SEC_NAME_S* pstIssuer,
* SEC_NAME_S* pstSubject,
* const SEC_AsnBits_S* pstPrivBits,
* CRMF_CERT_REQ_MSG_S* pCertReqMsg);
* @endcode
*
* @par Purpose
...
* @param[in] ulHMACAlgID hmac alg id , support ALGID_HMAC_SHA1, ALGID_HMAC_SHA224, ALGID_HMAC_SHA256,
* ALGID_HMAC_SHA384, ALGID_HMAC_SHA512
...
*
* @par Note
* - the ulHMACAlgID must keep same with CRMF_verifyPOPSigningKey
* N/A
*
* @par Related Topics
* N/A
*/
PSELINKDLL SEC_UINT32 CRMF_addPOPPrivKey(CRMF_POP_E enType, CRMF_POPOPRIV_KEY_E enPOPPrivType,
    const SEC_PKEY_S *pPrivKey, /* CA or EE */
    const SEC_PKEY_S *pPubKey,  /* EE or CA */
    SEC_UINT32 uiSubsequentMsgCh, SEC_NAME_S *pstIssuer, SEC_NAME_S *pstSubject, SEC_UINT32 ulHMACAlgID,
    const SEC_AsnBits_S *pPrivBits, CRMF_CERT_REQ_MSG_S *pCertReqMsg);
 ...
 * @par Prototype
 * @code
 * CRMF_CERT_TEMPLATE_S* CRMF_getCertTemplate (const CRMF_CERT_REQ_MSG_S* pCertReqMsg);
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * This function Returns Certificate Template structure from Certificate Request Message.
 *
 ...
 * N/A
 */
PSELINKDLL CRMF_CERT_TEMPLATE_S *CRMF_getCertTemplate(const CRMF_CERT_REQ_MSG_S *pstCertReqMsg);
 ...
 *
 * @par Description
 * This function Returns the First Certificate Request Message from the Certificate Request List.
 *
 ...
 * @retval CRMF_CERT_REQ_MSG_S* If the input is NULL [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 *
 * @par Description
 * This function Returns the Next Certificate Request Message from the Certificate Request List.
 *
 * @param[in] pstCertReqMsgList List from which the Certificate Request Message is retrieved [N/A]
 * @param[out] N/A N/A [N/A]
 *
 ...
 * @par Prototype
 * @code
 * CRMF_POP_S* CRMF_getPOP (const CRMF_CERT_REQ_MSG_S* pCertReqMsg);
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * This function Returns Proof of Possession Structure from Certificate Request Message.
 *
 ...
 * @retval CRMF_POP_S* If the input is NULL [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * N/A
 */
PSELINKDLL CRMF_POP_S *CRMF_getPOP(const CRMF_CERT_REQ_MSG_S *pstCertReqMsg);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 CRMF_getCertReqId (const CRMF_CERT_REQ_MSG_S* pstCertReqMsg);
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * This function Returns Certificate Request ID from the Certificate Request structure.
 *
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 CRMF_getCertReqId(const CRMF_CERT_REQ_MSG_S *pstCertReqMsg);
 ...
 *
 * @par Description
 * This function Returns Proof of Possession of Private Key from Certificate Request Message.
 *
 ...
 * @par Prototype
 * @code
 * SEC_VOID* CRMF_getSigningKeyInput (const CRMF_POPOSIGNING_KEY_S* pstPOPOSigningKey);
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * This function Returns POPOSigningKeyInput structure from POPOSigningKey structure.
 *
 * @param[in] pstPOPOSigningKey Pointer to CRMF_POPOSIGNING_KEY_S structure [N/A]
 * @param[out] N/A N/A [N/A]
 *
 ...
 * N/A
 */
PSELINKDLL SEC_VOID *CRMF_getSigningKeyInput(const CRMF_POPOSIGNING_KEY_S *pstPOPOSigningKey);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 CRMF_getSigningKeyAlgId (const CRMF_POPOSIGNING_KEY_S* pstPOPSigningKey);
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * This function Returns Signing Key Algorithm Identifier structure
 * from POPSigningKey structure.
 *
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 CRMF_getSigningKeyAlgId(const CRMF_POPOSIGNING_KEY_S *pstPOPOSigningKey);
 ...
 * @par Prototype
 * @code
 * CRMF_POPOSIGNING_KEY_TYPE_E CRMF_getAuthInfo (const CRMF_POPOSIGNING_KEYIN_S* pstPOPSigningKeyInput);
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * This function Returns POP Signing Key Input Choice.
 *
 ...
 * N/A
 */
PSELINKDLL CRMF_POPOSIGNING_KEY_TYPE_E CRMF_getAuthInfo(const CRMF_POPOSIGNING_KEYIN_S *pstPOPOSigningKeyInput);
 ...
 * @par Prototype
 * @code
 * SEC_PUBKEY_INFO_S* CRMF_getPubKeyInfo (const CRMF_POPOSIGNING_KEYIN_S* pPOPOSigningKeyInput);
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * This function Returns Subject Public Key Information structure from
 * POP Signing Key Input structure.
 *
 ...
 * N/A
 */
PSELINKDLL SEC_PUBKEY_INFO_S *CRMF_getPubKeyInfo(const CRMF_POPOSIGNING_KEYIN_S *pstPOPOSigningKeyInput);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 CRMF_getAlgId (const CRMF_PKMAC_S* pPublicKeyMAC);
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * This function Returns Algorithm Identifier from
 * PKMAC Value structure.
 *
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 CRMF_getAlgId(const CRMF_PKMAC_S *pstPublicKeyMAC);
 ...
 *
 * @par Description
 * This function Returns MAC Value from CRMF_PKMAC_S structure.
 *
 ...
 * N/A
 */
PSELINKDLL SEC_AsnBits_S *CRMF_getMACValue(CRMF_PKMAC_S *pstPublicKeyMAC);
 ...
 * GeneralName includes a URL and an IP addr) [N/A]
 * @param[out] N/A N/A [N/A]
 *
 * @retval CRMF_SINGLE_PUBINFO_S* Pointer to
 * CRMF_SINGLE_PUBINFO_S structure [pstSinglePubInfo|N/A]
 * @retval CRMF_SINGLE_PUBINFO_S* If any of the inputs are NULL or Invalid [SEC_NULL|N/A]
 * @retval CRMF_SINGLE_PUBINFO_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 *
 */
PSELINKDLL CRMF_SINGLE_PUBINFO_S *CRMF_createSinglePubInfo(SEC_UINT32 ulPubMethod, SEC_GEN_NAME_S *pstLocation);
 ...
 * @retval CRMF_PUBLICATION_INFO_S* If any of the inputs are NULL or Invalid [SEC_NULL|N/A]
 * @retval CRMF_PUBLICATION_INFO_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 *
 */
PSELINKDLL CRMF_PUBLICATION_INFO_S *CRMF_createPubInfo(SEC_UINT32 ulAction, CRMF_SINGLE_PUBINFO_S *pstSinglePubInfo);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 CRMF_addSinglePubinfo (const CRMF_PUBLICATION_INFO_S* pstPubInfo,
 * CRMF_SINGLE_PUBINFO_S* pstSinglePubInfo);
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 CRMF_addSinglePubinfo(const CRMF_PUBLICATION_INFO_S *pstPubInfo,
    CRMF_SINGLE_PUBINFO_S *pstSinglePubInfo);
...
* @param[in] ulAttrValLen Length of pucAttrValue ,if ulAttrID is CID_REGCTRL_REGTOKEN or  CID_REGCTRL_AUTHENTICATOR ,
            ulAttrValLen should be greater than zero and less than or equal to 1048576 bytes [N/A]
...
* @retval SEC_ATTR_TYPE_AND_VALUE_S* If any of the inputs are NULL or Invalid or memory allocation fails
[SEC_NULL|N/A]
*
* @par Dependency
* crmf.h
...
* @par Note
*
* X509_freeAttrTypeAndValue should be called to free the return value of CRMF_createAttrTypeAndValForCntrl when failed
*
* @par Related Topics
* N/A
*
*
*/
PSELINKDLL SEC_ATTR_TYPE_AND_VALUE_S *CRMF_createAttrTypeAndValForCntrl(SEC_UINT32 ulAttrID, SEC_UINT32 ulAttrValLen,
    const SEC_UCHAR *pucAttrValue, SEC_VOID *pComponent);
...
*                                    CertRequest [N/A]
* @param[in] ulAttrValLen Length of pucAttrValue.If ulAttrID is CID_REGINFO_UTF8PAIRS,
            ulAttrValLen should be greater than zero and less than or equal to 1048576 bytes[N/A]
* @param[in] pucAttrValue Pointer to Attribute Value
*                        This parameter should not be NULL with
...
*
*/
PSELINKDLL SEC_ATTR_TYPE_AND_VALUE_S *CRMF_createAttrTypeAndValForRegInfo(SEC_UINT32 ulAttrID, SEC_UINT32 ulAttrValLen,
    const SEC_UCHAR *pucAttrValue, SEC_VOID *pComponent);
 ...
 * @par Prototype
 * @code
 * SEC_VOID CRMF_freeSinglePubInfo (CRMF_SINGLE_PUBINFO_S* pstSinglePKIPubInfo);
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @param[in] pstSinglePKIPubInfo Pointer to CRMF_SINGLE_PUBINFO_S structure [N/A]
 * @param[out] N/A N/A [N/A]
 *
 * @retval SEC_VOID This does not return any value [N/A|N/A]
 *
 ...
 * N/A
 */
PSELINKDLL SEC_VOID CRMF_freeSinglePubInfo(CRMF_SINGLE_PUBINFO_S *pstSinglePKIPubInfo);
...
* @par Prototype
* @code
* SEC_ATTR_TYPE_AND_VALUE_S* CRMF_getFirstRegInfo (const CRMF_CERT_REQ_MSG_S* pstCertReqMsg);
* @endcode
*
* @par Purpose
...
*
* @par Description
* This function Returns First Registration Information from the list in Certificate Request Message.
*
* @param[in] pstCertReqMsg Pointer to CRMF_CERT_REQ_MSG_S Structure [N/A]
* @param[out] N/A N/A [N/A]
...
* N/A
*/
PSELINKDLL SEC_ATTR_TYPE_AND_VALUE_S *CRMF_getFirstRegInfo(const CRMF_CERT_REQ_MSG_S *pstCertReqMsg);
...
* @par Prototype
* @code
* SEC_ATTR_TYPE_AND_VALUE_S* CRMF_getNextRegInfo (const CRMF_CERT_REQ_MSG_S* pstCertReqMsg);
* @endcode
*
* @par Purpose
...
*
* @par Description
* This function Returns Next Registration Information from the list in Certificate Request Message.
*
* @param[in] pstCertReqMsg Pointer to CRMF_CERT_REQ_MSG_S Structure [N/A]
* @param[out] N/A N/A [N/A]
...
* N/A
*/
PSELINKDLL SEC_ATTR_TYPE_AND_VALUE_S *CRMF_getNextRegInfo(const CRMF_CERT_REQ_MSG_S *pstCertReqMsg);
...
* @par Prototype
* @code
* SEC_ATTR_TYPE_AND_VALUE_S* CRMF_getFirstControls (const CRMF_CERT_REQ_MSG_S* pstCertReqMsg);
* @endcode
*
* @par Purpose
...
*
* @par Description
* This function Returns First Attribute from the Controls list of the  Certificate Request.
*
* @param[in] pstCertReqMsg Pointer to CRMF_CERT_REQ_MSG_S Structure [N/A]
* @param[out] N/A N/A [N/A]
...
* N/A
*/
PSELINKDLL SEC_ATTR_TYPE_AND_VALUE_S *CRMF_getFirstControls(const CRMF_CERT_REQ_MSG_S *pstCertReqMsg);
...
* @par Prototype
* @code
* SEC_ATTR_TYPE_AND_VALUE_S* CRMF_getNextControls (const CRMF_CERT_REQ_MSG_S* pstCertReqMsg);
* @endcode
*
* @par Purpose
...
*
* @par Description
* This function Returns Next Attribute from the Controls list of the  Certificate Request.
*
* @param[in] pstCertReqMsg Pointer to CRMF_CERT_REQ_MSG_S Structure [N/A]
* @param[out] N/A N/A [N/A]
...
* N/A
*/
PSELINKDLL SEC_ATTR_TYPE_AND_VALUE_S *CRMF_getNextControls(const CRMF_CERT_REQ_MSG_S *pstCertReqMsg);
...
* @retval SEC_AsnBits_S* If the Input is NULL or the memory allocation fails [SEC_NULL|N/A]
*
* @par Dependency
* crmf.h
*
...
* @par Prototype
* @code
* SEC_AsnBits_S* CRMF_createEncryptedPrivKey(const SEC_PKEY_S* pstPrivKey,
...
* @param[in] ulValueHint Length of value hint. Max Value is 1048576 [N/A]
* @param[out] N/A N/A [N/A]
*
* @retval SEC_AsnBits_S* Pointer to SEC_AsnBits_S [pPrivBits|N/A]
* @retval SEC_AsnBits_S* If any of the Inputs are Invalid or memory allocation fails [SEC_NULL|N/A]
*
* @par Dependency
* crmf.h
*
* @par Note
* - This API is deprecated. It will always return NULL
* - If application does not set seeds and entropy on frequent intervals, it can lead to vulnerable keys or random value.
*   It is recommended to set seeds and entropy on frequent intervals or update the version that migrates the
*   random implementation based on DRBG.
* - Before calling this API, please make sure that DRBG is initialized for random number generation by calling
*   IPSI_CRYPT_rand_init. If application don't want to use DRBG then appplication should call
*   IPSI_CRYPT_enable_drbg(0) to disable the DRBG Random feature.
...
*
*/
PSELINKDLL SEC_AsnBits_S *CRMF_createEncryptedPrivKey(const SEC_PKEY_S *pPrivKey, SEC_UINT32 enIntndAlg,
    SEC_UINT32 enSymmAlg, SEC_PKEY_S *pPubKey, SEC_UINT32 enPubKeyAlgId, SEC_UINT32 enKeyGenAlgId,
    const SEC_UCHAR *pucValueHint, SEC_UINT32 ulValueHint);
 ...
 * supply information needed to form an archive of the private key
 * corresponding to the public key of the certification request.
 *
 ...
 * @retval CRMF_ARCHIVE_OPTS_S* If any of the inputs are Invalid or memory allocation fails[SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 *
 */
PSELINKDLL CRMF_ARCHIVE_OPTS_S *CRMF_createPKIArchiveOpts(SEC_UINT32 ulChoice, SEC_VOID *pstType);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 CRMF_verifyPOPSigningKey (const CRMF_CERT_REQ_MSG_S* pCertReqMsg,
 * SEC_PKEY_S* pPubKey);
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 CRMF_verifyPOPSigningKey(const CRMF_CERT_REQ_MSG_S *pCertReqMsg, SEC_PKEY_S *pPubKey);
...
* @par Prototype
* @code
* SEC_UINT32 CRMF_verifyPOPPrivKey (const CRMF_CERT_REQ_MSG_S* pCertReqMsg,
* SEC_NAME_S* pstIssuer,
* SEC_NAME_S* pstSubject,
* SEC_PKEY_S* pPrivKey,
...
* @param[in] ulHMACAlgID hmac alg id , support ALGID_HMAC_SHA1, ALGID_HMAC_SHA224, ALGID_HMAC_SHA256, ALGID_HMAC_SHA384,
* ALGID_HMAC_SHA512
* @param[out] N/A N/A [N/A]
...
*
* @par Note
* - Support for POPOPRIVKEY_THISMESSAGE is deprecated. API will return failure when pCertReqMsg have
POPOPRIVKEY_THISMESSAGE.
* - If application does not set seeds and entropy on frequent intervals, it can lead to vulnerable keys or random value.
*   It is recommended to set seeds and entropy on frequent intervals or update the version that migrate random
*   implementation based on DRBG.
* - Before calling this API, please make sure that DRBG is initialized for random number generation by calling
*   IPSI_CRYPT_rand_init. If application don't want to use DRBG then appplication should call
*   IPSI_CRYPT_enable_drbg(0) to disable the DRBG Random feature.
* - DRBG is enabled by default.
* - The ulHMACAlgID must keep same with CRMF_addPOPPrivKey
* @par Related Topics
* N/A
*/
PSELINKDLL SEC_UINT32 CRMF_verifyPOPPrivKey(const CRMF_CERT_REQ_MSG_S *pCertReqMsg, SEC_NAME_S *pstIssuer,
    SEC_NAME_S *pstSubject, SEC_PKEY_S *pPrivKey, const SEC_PKEY_S *pPubKey, SEC_UINT32 ulHMACAlgID);
 ...
 * @retval CRMF_OPT_VALIDITY_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_PKMAC_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_PKMAC_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_ENCRYPTED_VALUE_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_CERT_ID_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_POPOSIGNING_KEY_TYPE_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_POPOSIGNING_KEY_TYPE_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_CERT_TEMPLATE_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_CERT_TEMPLATE_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_ControlsLst* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_ControlsLst* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_POPOSIGNING_KEYIN_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_POPOSIGNING_KEYIN_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_POPOPRIV_KEY_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_POPOPRIV_KEY_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_SINGLE_PUBINFO_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_SINGLE_PUBINFO_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 ...
 * @retval CRMF_ENCRYPTED_KEY_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_ENCRYPTED_KEY_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_PublicationInfoList* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_PublicationInfoList* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_CERT_REQ_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_CERT_REQ_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_POPOSIGNING_KEY_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_POPOSIGNING_KEY_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_ARCHIVE_OPTS_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_ARCHIVE_OPTS_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_POP_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_POP_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_CERT_REQ_MSG_S* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_CERT_REQ_MSG_S* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * @retval CRMF_CertReqMsgList* If the input is NULL [SEC_NULL|N/A]
 * @retval CRMF_CertReqMsgList* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * N/A
 */
PSELINKDLL CRMF_CertReqMsgList *SEC_dupCertReqMessages(CRMF_CertReqMsgList *pSrc);

/**
 * @defgroup CRMF_addPopKeyBySignature
 * @ingroup crmfFunctions
 * @par Prototype
 * @code
 * SEC_UINT32 CRMF_addPopKeyBySignature(CRMF_CERT_REQ_MSG_S *pCertReqMsg,
 * SEC_UCHAR *pucSigned, SEC_UINT32 ulSignLen, SEC_UINT32 ulSignAlgID)
 * @endcode
 *
 * @par Purpose
 * To add the signature in the CRMF request for POPKey
 *
 * @par Description
 * CRMF_addPopKeyBySignature API adds the signature in the CRMF template for POPKey
 *
 * @param[in] pCertReqMsg Pointer to CRMF Request [N/A]
 * @param[in] pucSigned Signature of the CRMF request[N/A]
 * @param[in] ulSignLen length of the pucSigned buffer [N/A]
 * @param[in] ulSignAlgID Signature algorithn id [N/A]
 * @param[out] N/A N/A [N/A]
 *
 * @retval SEC_UINT32 SEC_SUCCESS in case of success|N/A]
 * @retval SEC_UINT32 SEC_ERR in case of failure[SEC_ERR|N/A]
 *
 * @par Dependency
 * crmf.h
 *
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 CRMF_addPopKeyBySignature(CRMF_CERT_REQ_MSG_S *pCertReqMsg, SEC_UCHAR *pucSigned,
    SEC_UINT32 ulSignLen, SEC_UINT32 ulSignAlgID);

PSELINKDLL CRMF_ENCRYPTED_KEY_S *CRMF_creatGmEncryptedKey(GM_ENVELOPED_S *pstEnvelopedData);

/**
 * @defgroup CRMF_freeEncryptedKey
 * @ingroup crmfFunctions
 * @par Prototype
 * @code
 * SEC_VOID CRMF_freeEncryptedKey(CRMF_ENCRYPTED_KEY_S *pstEncPrivateKey);
 * @endcode
 *
 * @par Purpose
 * @par Description
 * 释放 CRMF_ENCRYPTED_KEY_S 结构数字信封
 *
 * @param[in] pstEncPrivateKey        待释放对象，CRMF_ENCRYPTED_KEY_S 结构数字信封
 *
 * @retval N/A [N/A]
 * @par Dependency
 *
 ...
 *
 */
PSELINKDLL SEC_VOID CRMF_freeEncryptedKey(CRMF_ENCRYPTED_KEY_S *pstEncPrivateKey);

/**
 * @defgroup SEC_decryptSm2EnvelopedData
 * @ingroup crmfFunctions
 * @par Prototype
 * @code
 * SEC_UINT32 SEC_decryptSm2EnvelopedData(const CRMF_ENCRYPTED_KEY_S *pstEncPrivateKey,
 *     X509_CERT_S *pstCert, SEC_PKEY_S *pstDeCryptPriKey, SEC_PKEY_S **ppstPvtKey);
 * @endcode
 *
 * @par Purpose
 * @par Description
 * 解密 CRMF_ENCRYPTED_KEY_S 结构数字信封
 *
 * @param[in] pstEncPrivateKey        待解密的数字信封
 * @param[in] pstCert                 与解密后的私钥匹配的公钥证书
 * @param[in] pstDeCryptPriKey        用于解密的私钥
 * @param[out] ppstPvtKey             解密后的私钥
 *
 * @retval SEC_UINT32 成功 [SEC_SUCCESS|N/A]
 * @retval SEC_UINT32 无效的参数 [SEC_ERR_INVALID_ARG|N/A]
 * @retval SEC_UINT32 国密数字信封解码失败，如信封格式错误 [SEC_GM_ERR_ENVELOPED_FORMAT_ERR|N/A]
 ...
 *
 * @par Note
 * 编码数据见接口 CRMF_encodeEncryptKey
 * 数字信封中  recipientInfo->keyEncryptionAlgorithm 支持的算法及参数如下：
 * 1) OID:SM2-3(1.2.156.10197.1.301.3)  PARAM:NULL
 * 2) OID:SM2(1.2.156.10197.1.301)      PARAM:NULL
 * 3) OID:SM2(1.2.156.10197.1.301)      PARAM:SM2(1.2.156.10197.1.301)
 * 4) OID:ECC(1.2.840.10045.2.1)        PARAM:SM2(1.2.156.10197.1.301)
 * @li Memory operation: alloc、free、size
 *    ppstPvtKey 由调用者通过 CRYPT_PKEY_free 释放
 * @li Thread safe:
 *     线程安全函数
 * @li OS difference:
 *     无OS差异
 ...
 *
 */
PSELINKDLL SEC_UINT32 SEC_decryptSm2EnvelopedData(const CRMF_ENCRYPTED_KEY_S *pstEncPrivateKey,
    X509_CERT_S *pstCert, SEC_PKEY_S *pstDeCryptPriKey, SEC_PKEY_S **ppstPvtKey);

/**
 * @defgroup CRMF_encodeEncryptKey
 * @ingroup crmfFunctions
 * @par Prototype
 * @code
 * SEC_UCHAR *CRMF_encodeEncryptKey(CRMF_ENCRYPTED_KEY_S *pstEncryptKey, SEC_UINT32 *puiEncodedLen);
 * @endcode
 *
 * @par Purpose
 * @par Description
 * 编码 CRMF_ENCRYPTED_KEY_S 结构数字信封
 *
 * @param[in] pstEncryptKey   CRMF_ENCRYPTED_KEY_S 类型数字信封
 * @param[out] puiEncodedLen  编码后长度
 *
 * @retval 成功 [返回指针不为空]
 * @retval 失败 [返回指针空]
 * @par Dependency
 ...
 *
 */
PSELINKDLL SEC_UCHAR *CRMF_encodeEncryptKey(CRMF_ENCRYPTED_KEY_S *pstEncryptKey, SEC_UINT32 *puiEncodedLen);

/**
 * @defgroup CRMF_decodeEncryptKey
 * @ingroup crmfFunctions
 * @par Prototype
 * @code
 * CRMF_ENCRYPTED_KEY_S *CRMF_decodeEncryptKey(SEC_UCHAR *ucMsg, SEC_UINT32 ulMsgLen, SEC_UINT32 *pulDecLen);
 * @endcode
 *
 * @par Purpose
 * @par Description
 * 解码数字信封
 *
 * @param[in] ucMsg      CRMF_ENCRYPTED_KEY_S 编码后的 buffer
 * @param[in] ulMsgLen   ucMsg 长度
 * @param[out] pulDecLen 解码后长度
 *
 * @retval 成功 [返回指针不为空]
 * @retval 失败 [返回指针空]
 * @par Dependency
 ...
 * 编码数据见接口 CRMF_encodeEncryptKey
 * @li Memory operation: alloc、free、size
 *    解码后的CRMF_ENCRYPTED_KEY_S 类型结构体通过 CRMF_freeEncryptedKey 释放
 * @li Thread safe:
 *     线程安全函数
 * @li OS difference:
 *     无OS差异
 ...
 *
 */
PSELINKDLL CRMF_ENCRYPTED_KEY_S *CRMF_decodeEncryptKey(SEC_UCHAR *ucMsg, SEC_UINT32 ulMsgLen, SEC_UINT32 *pulDecLen);
