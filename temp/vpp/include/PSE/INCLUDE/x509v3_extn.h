...
#ifndef X509V3_EXTN_H
#define X509V3_EXTN_H
...
typedef struct stX520Name {
    /* The various string formats in which the string can be represented */
    enum X520nameChoiceId choiceId;
...
typedef struct stDisplayText {
    /* The various string formats in which the string can be represented */
    enum DisplayTextChoiceId choiceId;
 ...
 * @endcode
 *
 * @datastruct noticeRef Optional - Notices text issued by an organization and identifies, by number, a particular
 * textual statement prepared by that organization.
 * @datastruct explicitText Optional - An explicitText field which includes the textual statement directly in the
 * certificate that has to be displayed to a relying party when a certificate is used.
 */
typedef struct stUserNotice {
...
typedef struct stDistributionPointName {
    ...
    enum DistributionPointNameChoiceId choiceId;
 ...
 * @datastruct authorityCertIssuer Optional - The name of Certificate of the CRL Issuer(incase used in CRL as an
 * extension).
 ...
 * @datastruct pathLenConstraint Optional (0...MAX) - The pathLenConstraint field is meaningful only if the cA boolean
 * is asserted and the key usage extension asserts the keyCertSign bit. It gives the maximum number of non-self-issued
 ...
 * @datastruct ediPartyName Electronic Document Interface information of how the information and services can be
 * exchanged.
 * @datastruct uniformResourceIdentifier URL information.
 * @datastruct iPAddress IP addr.
 ...
 * indicates the number of additional certificates that may appear in the path before policy mapping is no longer
 * permitted.
 */
typedef struct stPolicyConstraintsSyntax {
 ...
 * not allowed during path validation. If a name is present in excluded list, the validation should stop even if the
 * name is present in permitted list.
 */
typedef struct stNameConstraintsSyntax {
 ...
 * @datastruct distributionPoint This optional field is used when the CRL issuer is the same as the certificate issuer.
 * If this field is set then reason also has to be associated with it.
 * @datastruct reasons This optional field specifies the reason on which the CRL is generated. The reason field should
 * not be the only option set in the structure. The reasons include:\n 1) unused\n 2) keyCompromise\n 3) cACompromise\n
 ...
 * 9) aACompromise
 * @datastruct cRLIssuer This optional field identifies the entity who signs and issues the CRL. If present, this field
 * is set when the certificate issuer is not the CRL issuer.
 */
typedef struct stDistributionPoint {
...
typedef struct CRL_CID_S {
    ...
    SEC_BIGINT_S *crlNum;            /* Optional - CRL number of the relevant CRL */
...
typedef struct stSkaeIssuerSerial {
    GeneralNames *pstIssuer; // list of SEC_GEN_NAME_S
    SEC_BIGINT_S serialNumber;
} X509_SKAE_ISSUER_SERIAL_S;

typedef struct stTPMIdentityCredentialAccessInfo {
    SEC_List_S *pstAuthorityInfoAccess;         // list of X509_ACCESS_DESC_S
    X509_SKAE_ISSUER_SERIAL_S *pstIssuerSerial; // optional and can be set to NULL
} X509_SKAE_TPM_ID_CREDENTIAL_INFO_S;

typedef struct stTPMCertifyInfo {
    SEC_AsnBits_S *pstCertifyInfo;
    SEC_AsnBits_S stSignature;
} X509_SKAE_TPM_CERTIFY_INFO_S;

typedef struct stAttestationEvidence {
    X509_SKAE_TPM_CERTIFY_INFO_S stCertifyInfo;
    X509_SKAE_TPM_ID_CREDENTIAL_INFO_S *pstCredentialInfo;
} X509_SKAE_ATTEST_EVIDENCE_S;

typedef struct stKeyAttestationEvidence {
    SEC_INT iChoice;
    union {
        X509_SKAE_ATTEST_EVIDENCE_S *pstAttestEvi;
        void *pstEnvelopAttest;
    } u;
} X509_SKAE_KEY_ATTEST_EVIDENCE_S;

typedef struct stTCGSpecVersion {
    SEC_INT major;
    SEC_INT minor;
} X509_SKAE_TCG_SPEC_VERSION_S;

typedef struct stSubjectKeyAttestationEvidence {
    X509_SKAE_TCG_SPEC_VERSION_S stVersion;
    X509_SKAE_KEY_ATTEST_EVIDENCE_S stKeyattestEvi;
} X509_SKAE_EXT_S;
