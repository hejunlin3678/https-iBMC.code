...
************************************************************************** */
#ifndef SEC_PKI_CONTEXT_H
#define SEC_PKI_CONTEXT_H
 ...
 * const SEC_CHAR *pcName,SEC_UINT ilen)
 * @endcode
 *
 * @par Purpose
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxSetName(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcName, SEC_UINT iLen);
 ...
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxGetName(const SEC_PKI_CTX_S *pstContext,
 ...
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetName(const SEC_PKI_CTX_S *pstContext, SEC_CHAR *pcName, SEC_UINT iLen,
    SEC_UINT *piNamelen);
...
* @ingroup Context_Functions
* @image html Security.jpg <b> User should take care for file access controls
  and should provide canonical path when using this APIs. </b>
* @par Prototype
* @code
...
* 	- For PEM files, the default data size is limited to 256KB. If application needs to set size more than 256KB,
*        then need to call API IPSI_setPemMaxSize.
*/
PSELINKDLL SEC_INT SEC_PKI_ctxLoadTrustCACertificateFile(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCAfile,
    SEC_INT iType, SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen, SEC_BOOL bAddToCAReqList);
 ...
 * then need to call API IPSI_setPemMaxSize.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadTrustCACertificateBuffer(SEC_PKI_CTX_S *pstContext, SEC_CHAR *pcCertBuf,
    SEC_UINT uCertBufLen, SEC_INT iType, SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen, SEC_BOOL bAddToCAReqList);

/**
 * @defgroup SEC_PKI_ctxLoadTrustCACertificateBuffer_ex
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxLoadTrustCACertificateBuffer_ex( SEC_PKI_CTX_S *pstContext,
 * SEC_CHAR *pcCertBuf,
 * SEC_UINT uCertBufLen,
 * SEC_INT iType,
 * SEC_CHAR *pcCertPasswd,
 ...
 *
 * @par Description
 * SEC_PKI_ctxLoadTrustCACertificateBuffer_ex function reads the buffer
 * that contains certificate in "PEM","PFX" or "DER" format and adds the
 * certificate to the trust store. Password is an optional input, and it is
 * needed only for certificates which are in PFX format. To make this
 * CA to be part of "CA Request list" user needs to pass
 ...
 * 	- For PEM buffer, the default data size is limited to 256KB. If application needs to set size more than 256KB,
 * then need to call API IPSI_setPemMaxSize.
 * - If this API invoke with pem buffer having multiple certificate and it return failure for some reason , no
 * CA certificate is added to PKI context.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadTrustCACertificateBuffer_ex(SEC_PKI_CTX_S *pstContext, SEC_CHAR *pcCertBuf,
    SEC_UINT uCertBufLen, SEC_INT iType, SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen, SEC_BOOL bAddToCAReqList);

/**
 * @defgroup SEC_PKI_ctxLoadChainCACertificateBuffer
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxLoadChainCACertificateBuffer( SEC_PKI_CTX_S *pstContext,
 * SEC_CHAR *pcCertBuf,
 * SEC_UINT uCertBufLen,
 * SEC_INT iType,
 * SEC_CHAR *pcCertPasswd,
 * SEC_UINT uCertPwdLen,
 * SEC_BOOL bAddToCAReqList
 * SEC_BOOL bUseRefence);
 * @endcode
 *
 * @par Purpose
 * This function is used to load the CA certificate(s) from buffer into the chain
 * store of PKI context.
 *
 * @par Description
 * SEC_PKI_ctxLoadChainCACertificateBuffer function reads the buffer
 * that contains certificate in "PEM","PFX" or "DER" format and adds the
 * certificate to the chain store. Password is an optional input, and it is
 * needed only for certificates which are in PFX format. To make this
 * CA to be part of "CA Request list" user needs to pass
 * bAddToCAReqList as true.
 *
 ...
 * @param[in] bAddToCAReqList Flag to indicate, if the CA has to be added to
 * CA request list or not.[NA/NA]
 * @param[in] bUseRefence Flag to indicate, if the CA refence from Trust Store or not.
 * if set true, then find certs from Trust store and then reference it,
 * if one cert in buffer not found in Trust store , then load faild.
 * Trust store load by SEC_PKI_ctxLoadTrustCACertificateBuffer_ex or SEC_PKI_ctxLoadTrustCACertificateBuffer
 * @param[out] NA NA [NA/NA]
 *
 *
 * @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
 ...
 * CA certificate is added to PKI context.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadChainCACertificateBuffer(SEC_PKI_CTX_S *pstContext, SEC_CHAR *pcCertBuf,
    SEC_UINT uCertBufLen, SEC_INT iType, SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen, SEC_BOOL bAddToCAReqList,
    SEC_BOOL bUseReference);

/**
 * @defgroup SEC_PKI_ctxLoadVerifyCACertificateBuffer
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxLoadVerifyCACertificateBuffer( SEC_PKI_CTX_S *pstContext,
 * SEC_CHAR *pcCertBuf,
 * SEC_UINT uCertBufLen,
 * SEC_INT iType,
 * SEC_CHAR *pcCertPasswd,
 ...
 *
 * @par Purpose
 * This function is used to load the CA certificate(s) from buffer into the verify
 * store of PKI context.
 *
 * @par Description
 * SEC_PKI_ctxLoadVerifyCACertificateBuffer function reads the buffer
 * that contains certificate in "PEM","PFX" or "DER" format and adds the
 * certificate to the verify store. Password is an optional input, and it is
 * needed only for certificates which are in PFX format. To make this
 * CA to be part of "CA Request list" user needs to pass
 * bAddToCAReqList as true.
 *
 ...
 * CA certificate is added to PKI context.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadVerifyCACertificateBuffer(SEC_PKI_CTX_S *pstContext, SEC_CHAR *pcCertBuf,
    SEC_UINT uCertBufLen, SEC_INT iType, SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen, SEC_BOOL bAddToCAReqList,
    SEC_BOOL bUseReference);
/**
...
*        then need to call API IPSI_setPemMaxSize.
*/
PSELINKDLL SEC_INT SEC_PKI_ctxLoadTrustCrossCACertificateFile(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCAfile,
    SEC_INT iType, SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen);
 ...
 * then need to call API IPSI_setPemMaxSize.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadTrustCrossCACertificateBuffer(SEC_PKI_CTX_S *pstContext, SEC_CHAR *pcCertBuf,
    SEC_UINT uCertBufLen, SEC_INT iType, SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen);

/**
 * @defgroup SEC_PKI_ctxLoadVerifyCrossCACertificateBuffer
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxLoadVerifyCrossCACertificateBuffer (
 * SEC_PKI_CTX_S *pstContext,
 * SEC_CHAR *pcCertBuf,
 * SEC_UINT uCertBufLen,
 * SEC_INT iType,
 ...
 * @param[in] pcCertPasswd Required only for SEC_PKI_FILETYPE_PFX type.[NA/NA]
 * @param[in] uCertPwdLen Length of the password.[0-255/NA]
 * @param[in] bUseRefence Flag to indicate, if the Cross CA refence from Trust Store or not.
 * if set true, then find certs from Trust store and then reference it,
 * if one cert in buffer not found in Trust store , then load faild.
 * Trust store load by SEC_PKI_ctxLoadTrustCrossCACertificateBuffer or SEC_PKI_ctxLoadTrustCrossCACertificateFile
 * @param[out] NA NA [NA/NA]
 *
 *
 * @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
 ...
 * then need to call API IPSI_setPemMaxSize.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadVerifyCrossCACertificateBuffer(SEC_PKI_CTX_S *pstContext, SEC_CHAR *pcCertBuf,
    SEC_UINT uCertBufLen, SEC_INT iType, SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen, SEC_BOOL bReference);
/**
...
*        then need to call API IPSI_setPemMaxSize.
*/
PSELINKDLL SEC_INT SEC_PKI_ctxLoadCrlFile(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCrlFile, SEC_INT iType);

/**
 * @defgroup SEC_PKI_ctxLoadCrlFile_ex
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxLoadCrlFile_ex(SEC_PKI_CTX_S *pstContext,
 * const SEC_CHAR *pcCrlFile,SEC_INT iType,
 * X509_CRL_S *pstCrl)
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * SEC_PKI_ctxLoadCrlFile_ex function reads a file that contains a CRL
 * (Certificate Revocation List) in "PEM" or "DER" format and adds the CRL to
 * the context. If the application provides an already decoded CRL structure pstCrl then
 * CRL file is not used for decoding the CRL, and the user passed CRL pointer will be
 ...
 * @param[in] iType Mentions the file type to be loaded.
 * [SEC_PKI_FILETYPE_PEM/SEC_PKI_FILETYPE_ASN1/NA]
 * @param[in] pstCrl Pointer to the CRL structure. [NA/NA]
 * @param[out] NA NA [NA/NA]
 *
 *
 * @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
 * @retval SEC_INT On all failure conditions. If the input is invalid
 * (NULL pointer), when file cannot be opened, or decoding of CRL fails.[SEC_PKI_ERROR|NA]
 *
 * @par Note
 * \n
 * 	- It is recommended that CRL dates are verified before loading the same.\n
 * 	- PEM list of CRL is not suppoted.
 * 	- If the same CRL already exists in the store, then error will be returned.
 * 	- For PEM files, the default data size is limited to 256 KB. If the application needs to set size more than 256 KB,
 * then need to call the IPSI_setPemMaxSize API.
 * - If the application passes pstCrl, application must not use CRL structure when this API returns success
 * (as CRL pointer in stored in PKI context). In failure case, the application must free the CRL structure.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadCrlFile_ex(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCrlFile, SEC_INT iType,
    X509_CRL_S *pstCrl);
 ...
 * @param[in] ulCrlBufLen CRL buffer length.[NA/NA]
 * @param[in] iType Mentions the file type to be loaded.
 * [SEC_PKI_FILETYPE_PEM/SEC_PKI_FILETYPE_ASN1/NA]
 * @param[out] NA NA [NA/NA]
 *
 ...
 * then need to call API IPSI_setPemMaxSize.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadCrlBuffer(SEC_PKI_CTX_S *pstContext, SEC_CHAR *pcCrlBuffer, SEC_UINT uCrlBufLen,
    SEC_INT iType);
...
* @ingroup Context_Functions
* @image html Security.jpg <b> User must take care the file access controls
  and must provide canonical path before using this API. </b>
* @par Prototype
* @code
...
*/
PSELINKDLL SEC_INT SEC_PKI_ctxLoadDfltLocalCertAndKeyFile(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCertFile,
    SEC_INT iCertFileType, const SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen, const SEC_CHAR *pcKeyFile,
    SEC_INT iKeyFileType, const SEC_CHAR *pcKeyPasswd, SEC_UINT uKeyPwdLen, const SEC_CHAR *pcURL, SEC_UINT uUrlLen,
    PEM_WARNING_CODE_E *peWarningCode);
...
*   - DRBG is enabled by default.
*/
PSELINKDLL SEC_INT SEC_PKI_ctxLoadDfltLocalCertAndKeyBuffer(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCertBuffer,
    SEC_UINT uCertBufLen, SEC_INT iCertType, const SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen,
    const SEC_CHAR *pcKeyBuffer, SEC_UINT uKeyBufLen, SEC_INT iKeyType, const SEC_CHAR *pcKeyPasswd,
    SEC_UINT uKeyPwdLen, const SEC_CHAR *pcURL, SEC_UINT uUrlLen, PEM_WARNING_CODE_E *peWarningCode);
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetPeerCacheSize(SEC_PKI_CTX_S *pstContext, SEC_UINT *puiSize);
 ...
 * SEC_PKI_objGetCtxPeerCertByHash
 */
PSELINKDLL SEC_INT SEC_PKI_ctxResizePeerCache(SEC_PKI_CTX_S *pstContext, SEC_INT iSize);
...
*   - DRBG is enabled by default.
*/
PSELINKDLL SEC_INT SEC_PKI_ctxLoadLocalCertAndKeyFile(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCertFile,
    SEC_INT iCertFileType, const SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen, const SEC_CHAR *pcKeyFile,
    SEC_INT iKeyFileType, const SEC_CHAR *pcKeyPasswd, SEC_UINT uKeyPwdLen, const SEC_CHAR *pcURL, SEC_UINT uUrlLen,
    PEM_WARNING_CODE_E *peWarningCode);
...
*   - DRBG is enabled by default.
*/
PSELINKDLL SEC_INT SEC_PKI_ctxLoadLocalCertAndKeyBuffer(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCertBuffer,
    SEC_UINT uCertBufLen, SEC_INT iCertType, const SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen,
    const SEC_CHAR *pcKeyBuffer, SEC_UINT uKeyBufLen, SEC_INT iKeyType, const SEC_CHAR *pcKeyPasswd,
    SEC_UINT uKeyPwdLen, const SEC_CHAR *pcURL, SEC_UINT uUrlLen, PEM_WARNING_CODE_E *peWarningCode);
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxSetTime(SEC_PKI_CTX_S *pstContext, const DATETIME_S *pstTime);
 ...
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxGetTime(const SEC_PKI_CTX_S *pstContext,
 * DATETIME_S **dpstTime)
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetTime function returns the time set in the context
 ...
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetTime(const SEC_PKI_CTX_S *pstContext, DATETIME_S **dpstTime);
...
*
* @param[in,out] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
* @param[in] iDepth Depth of the certificate validation. iDepth should be in the range of [0,65535][NA/NA]
* @param[out] NA NA [NA/NA]
*
* @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
* @retval SEC_INT On all failure conditions. [SEC_PKI_ERROR|NA]
...
*
*/
PSELINKDLL SEC_INT SEC_PKI_ctxSetVerifyDepth(SEC_PKI_CTX_S *pstContext, SEC_INT iDepth);
 ...
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxGetVerifyDepth (const SEC_PKI_CTX_S *pstContext,
 * SEC_INT *piDepth);
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetVerifyDepth function returns verification depth of the context.
 * The certificate validation has to be performed until this depth.
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetVerifyDepth(const SEC_PKI_CTX_S *pstContext, SEC_INT *piDepth);
 ...
 * SEC_PKI_EXTENDED_CRL_SUPPORT | SEC_PKI_OBJ_CRL_SUPPORT)
 * The following OCSP flags (SEC_PKI_CHECK_OCSP | SEC_PKI_CHECK_OCSP_ALL
 * | SEC_PKI_OCSP_RESPONDER_CHECK_CRL | SEC_PKI_OCSP_RESPONDER_CHECK_DELTA_CRL)
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxSetVerifyParam(SEC_PKI_CTX_S *pstContext, SEC_UINT verifyFlags);
 ...
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxGetVerifyParam(const SEC_PKI_CTX_S *pstContext,
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetVerifyParam function returns verify parameter set in the context.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetVerifyParam(const SEC_PKI_CTX_S *pstContext, SEC_UINT *pVerifyFlags);
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxClearVerifyParam(SEC_PKI_CTX_S *pstContext, SEC_UINT verifyFlags);
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetCACertReq function returns the CA Certificate(s) or CA
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetCACertReq(SEC_PKI_CTX_S *pstContext, SEC_PKI_PARAMETER_E enId, SEC_List_S **dpOutList);
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetAllTrustCA function returns All CA certificates
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 ...
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetAllTrustCA(SEC_PKI_CTX_S *pstContext, SEC_List_S **dpOutCertList);

/**
 * @defgroup SEC_PKI_ctxGetAllVerifyCA
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxGetAllVerifyCA( SEC_PKI_CTX_S *pstContext,
 * SEC_List_S **dpOutCertList)
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetAllVerifyCA function returns All CA certificates
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[out] dpOutCertList List which contains all the certs.
 * pOutCertList is a list which holds elements of type X509_CERT_S[NA/NA]
 ...
 * - Retrival order might not be same as loading order of CA.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetAllVerifyCA(SEC_PKI_CTX_S *pstContext, SEC_List_S **dpOutCertList);
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetAllCRL function returns all CRL from store.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 ...
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetAllCRL(SEC_PKI_CTX_S *pstContext, SEC_List_S **dpOutCRLList);
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetAllTrustCrossCA function returns all cross CA from store.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 ...
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetAllTrustCrossCA(SEC_PKI_CTX_S *pstContext, SEC_List_S **dpOutCrossCAList);
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetDfltLocalCert function returns the default local cert associated
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetDfltLocalCert(SEC_PKI_CTX_S *pstContext, X509_CERT_S **pstCert);
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetAllLocalCert function returns all certificates from local store,
 ...
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetAllLocalCert(SEC_PKI_CTX_S *pstContext, SEC_List_S **dpOutCertList);
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetPvtKeyByCert function returns private key corresponding to the
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetPvtKeyByCert(SEC_PKI_CTX_S *pstContext, X509_CERT_S *pstCert,
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveTrustedCA(SEC_PKI_CTX_S *pstContext, SEC_NAME_S *pstIssuerName,
    SEC_BIGINT_S *pstSerialNum);

/**
 * @defgroup SEC_PKI_ctxRemoveChainCA
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxRemoveChainCA( SEC_PKI_CTX_S *pstContext,
 * SEC_NAME_S *pstIssuerName,
 * SEC_BIGINT_S *pstSerialNum)
 * @endcode
 *
 * @par Purpose
 * This function is used to remove the CA certificate from the chain store.
 *
 * @par Description
 * SEC_PKI_ctxRemoveChainCA function removes the CA for which the
 * input issuer name and serial number matches from the chain store.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.
 * Issuer name and serial number are unique identifier in RFC.[NA/NA]
 * @param[in] pstIssuerName Issuer name of the CA.[NA/NA]
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveChainCA(SEC_PKI_CTX_S *pstContext, SEC_NAME_S *pstIssuerName,
    SEC_BIGINT_S *pstSerialNum);

/**
 * @defgroup SEC_PKI_ctxRemoveVerifyCA
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxRemoveVerifyCA( SEC_PKI_CTX_S *pstContext,
 * SEC_NAME_S *pstIssuerName,
 * SEC_BIGINT_S *pstSerialNum)
 * @endcode
 *
 * @par Purpose
 * This function is used to remove the CA certificate from the verify store.
 *
 * @par Description
 * SEC_PKI_ctxRemoveVerifyCA function removes the CA for which the
 * input issuer name and serial number matches from the verify store.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.
 * Issuer name and serial number are unique identifier in RFC.[NA/NA]
 * @param[in] pstIssuerName Issuer name of the CA.[NA/NA]
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveVerifyCA(SEC_PKI_CTX_S *pstContext, SEC_NAME_S *pstIssuerName,
    SEC_BIGINT_S *pstSerialNum);
#ifndef __SEC_PKI_NO_MULTIPLE_LOCAL_CERT__
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveLocalCert(SEC_PKI_CTX_S *pstContext, SEC_NAME_S *pstIssuerName,
    SEC_BIGINT_S *pstSerialNum);
 ...
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveCrl(SEC_PKI_CTX_S *pstContext, SEC_NAME_S *pstIssuerName,
    SEC_BIGINT_S *pstExtnCRLNum);

/**
 * @defgroup SEC_PKI_ctxRemoveCrlByHash
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxRemoveCrlByHash(SEC_PKI_CTX_S *pstContext, const SEC_UCHAR *md)
 *
 * @endcode
 * @par Purpose
 * This function is used to remove the CRL from the store by md.
 *
 * @par Description
 * SEC_PKI_ctxRemoveCrlByHash function removes the CRL by CRL hash
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[in] md hash of the CRL.[NA/NA]
 * @param[in] length length of md,must be SEC_PKI_SHA_HASH_LEN(20)
 * @param[out] NA NA [NA/NA]
 *
 * @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
 * @retval SEC_INT On all failure conditions. [SEC_PKI_ERROR|NA]
 ...
 * @par Note
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveCrlByHash(SEC_PKI_CTX_S *pstContext, const SEC_UCHAR *md, const SEC_UINT32 length);

/**
 * @defgroup SEC_PKI_ctxVerifyStoreRefCrl
 * @ingroup Context_Functions
 * @par Prototype
 * @par Purpose
 * This function is used to reference the CRL from trust store.
 *
 * @par Description
 * SEC_PKI_ctxVerifyStoreRefCrl function load the CRL for which the
 * input issuer name and CRL number matches from the trusted store.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[in] pstIssuerName Issuer name of the CRL.[NA/NA]
 ...
 *
 * @par Note
 * the crl must in trust store.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxVerifyStoreRefCrl(SEC_PKI_CTX_S *pstContext, SEC_NAME_S *pstIssuerName,
    SEC_BIGINT_S *pstExtnCRLNum);

/**
 * @defgroup SEC_PKI_ctxRemoveVerifyCrl
 * @ingroup Context_Functions
 * @par Prototype
 * @par Purpose
 * This function is used to remove the CRL from verify store.
 *
 * @par Description
 * SEC_PKI_ctxRemoveVerifyCrl function removes the CRL  for which the
 * input issuer name and CRL number matches from the verify store.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[in] pstIssuerName Issuer name of the CRL.[NA/NA]
 * @param[in] pstExtnCRLNum Extension CRL number of the CRL.[NA/NA]
 ...
 * the matching CRL(S) and Delta CRL(S) will be removed.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveVerifyCrl(SEC_PKI_CTX_S *pstContext, SEC_NAME_S *pstIssuerName,
    SEC_BIGINT_S *pstExtnCRLNum);

/**
 * @defgroup SEC_PKI_ctxRemoveVerifyCrlByHash
 * @ingroup Context_Functions
 * @par Prototype
 * @par Purpose
 * This function is used to remove the CRL from verify store.
 *
 * @par Description
 * SEC_PKI_ctxRemoveVerifyCrlByHash function removes the CRL  for which the
 * input hash matches from the verify store.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[in] md hash of the CRL.[NA/NA]
 * @param[in] length  length of the hash.[NA/NA]
 * @param[out] NA NA [NA/NA]
 *
 * @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
 * @retval SEC_INT On all failure conditions. [SEC_PKI_ERROR|NA]
 ...
 * @par Note
 * \n
 * 	- If the user inputs only hash then all the matching CRL(S)
 * and Delta CRL(S) will be removed.
 * 	- If the user inputs hash then all
 * the matching CRL(S) and Delta CRL(S) will be removed.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveVerifyCrlByHash(SEC_PKI_CTX_S *pstContext, const SEC_UCHAR *md,
    const SEC_UINT32 length);

/**
 * @defgroup SEC_PKI_ctxLoadVerifyStoreCrlByHash
 * @ingroup Context_Functions
 * @par Prototype
 * @par Purpose
 * This function is used to reference the CRL from trust store.
 *
 * @par Description
 * SEC_PKI_ctxLoadVerifyStoreCrlByHash function load the CRL for which the
 * input Hash matches from the trusted store.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[in] pstIssuerName Issuer name of the CRL.[NA/NA]
 * @param[in] pstExtnCRLNum Extension CRL number of the CRL.[NA/NA]
 ...
 * the crl must in trust store.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadVerifyStoreCrlByHash(SEC_PKI_CTX_S *pstContext, const SEC_UCHAR *md,
    const SEC_UINT32 length);
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveCrossCA(SEC_PKI_CTX_S *pstContext, SEC_NAME_S *pstIssuerName,
    SEC_BIGINT_S *pstSerialNum);

/**
 * @defgroup SEC_PKI_ctxRemoveVerifyCrossCA
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxRemoveVerifyCrossCA( SEC_PKI_CTX_S *pstContext,
 * SEC_NAME_S *pstIssuerName,
 * SEC_BIGINT_S *pstSerialNum)
 * @endcode
 *
 ...
 *
 * @par Description
 * SEC_PKI_ctxRemoveVerifyCrossCA function removes the cross CA for which the
 * input issuer name and serial number matches from the verify store.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.
 * Issuer name and serial number are unique identifier in RFC.[NA/NA]
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveVerifyCrossCA(SEC_PKI_CTX_S *pstContext, SEC_NAME_S *pstIssuerName,
    SEC_BIGINT_S *pstSerialNum);
 ...
 * @ingroup Context_Functions
 * @image html icon-stop.png <b> This API is deprecated. </b>
 * @par Prototype
 * @code
 ...
 * @endcode
 * @par Purpose
 * This function is used to enable the multithreading feature. This api create locks for log and error module.
 * As each object internally refer to PKI context for CA store, cross CA store, CRL store etc.
 * when PKI context is created after this API,  will create lock to protect these store
 *
 * @par Description
 * SEC_PKI_enable_multithread function is used to enable the multithreading functionality of the library.
 * This api create locks for log and error module. As each object internally refer to PKI context for CA store, cross CA
 * store, CRL store etc. When PKI context is created after this API,  will create lock to protect these store
 *
 * @param[in] NA NA [NA/NA]
 * @param[out] NA NA [NA/NA]
 *
 ...
 * \n
 *
 * PKI Context should be created and used in the main thread. PKI Objects, which are created from the PKI context,
 * should be used in the separate threads. ie. Only one object/context  should be used in one thread. Multiple threads
 * using same object/context  is not supported. Parallel(Multi-thread) opeations on a same PKI context and same PKI
 * object is not supported This function should be called only in main thread at the starting of the application.
 */
 ...
 * @par Description
 * SEC_PKI_get_error_string function returns the error string for
 ...
 * @par Description
 * SEC_PKI_getlast_error function returns the last error occurred.
 *
 * @param[in] NA NA [NA/NA]
 * @param[out] NA NA [NA/NA]
 *
 ...
 *
 * @attention
 * When a new thread is created, the API needs to be called to release memory when the thread exits.
...
*        then need to call API IPSI_setPemMaxSize.
*/
PSELINKDLL SEC_INT SEC_PKI_ctxLoadDfltPreSharedPeerCertFile(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCertFile,
    SEC_INT iCertFileType, const SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen);
 ...
 * then need to call API IPSI_setPemMaxSize.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadDfltPreSharedPeerCertBuffer(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCertBuffer,
    SEC_UINT uCertBufLen, SEC_INT iCertFileType, const SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen);
...
*        then need to call API IPSI_setPemMaxSize.
*/
PSELINKDLL SEC_INT SEC_PKI_ctxLoadPreSharedPeerCertFile(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCertFile,
    SEC_INT iCertFileType, const SEC_CHAR *pcCertPasswd, const SEC_UINT uCertPwdLen);
 ...
 * then need to call API IPSI_setPemMaxSize.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadPreSharedPeerCertBuffer(SEC_PKI_CTX_S *pstContext, const SEC_CHAR *pcCertBuffer,
    SEC_UINT uCertBufLen, SEC_INT iCertFileType, const SEC_CHAR *pcCertPasswd, const SEC_UINT uCertPwdLen);
 ...
 * application has to call the X509_freeCert.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetDfltPreSharedPeerCert(SEC_PKI_CTX_S *pstContext, X509_CERT_S **pstCert);
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetURLByCert function returns URL corresponding to the
 * input certificate.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[in] pstCert Input certificate.[NA/NA]
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetURLByCert(SEC_PKI_CTX_S *pstContext, X509_CERT_S *pstCert,
    SEC_PKI_DATA_BUF_S **pstURLBuf);
 ...
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxGetAllPeerCerts( SEC_PKI_CTX_S *pstContext,
 * SEC_List_S **dpOutCertList)
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetAllPeerCerts returns all peer certificate from the peer cache
 * in context.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[out] dpOutCertList List which contains all the certs.
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetAllPeerCerts(SEC_PKI_CTX_S *pstContext, SEC_List_S **dpOutCertList);
#endif
...
*
*/
PSELINKDLL SEC_INT SEC_PKI_ctxGetAllPreSharedPeerCert(SEC_PKI_CTX_S *pstContext, SEC_List_S **dpOutCertList);
...
*/
PSELINKDLL
SEC_INT SEC_PKI_ctxRemovePreSharedPeerCert(SEC_PKI_CTX_S *pstContext, SEC_NAME_S *pstIssuerName,
    SEC_BIGINT_S *pstSerialNum);
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveDfltPreSharedPeerCert(SEC_PKI_CTX_S *pstContext);
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetBaseCRLbyCert function returns the
 ...
 */
PSELINKDLL X509_CRL_S *SEC_PKI_ctxGetBaseCRLbyCert(const SEC_PKI_CTX_S *pstContext, X509_CERT_S *pstCert);
 ...
 *
 * @par Description
 * SEC_PKI_ctxGetDeltaCRLbyCert function returns the
 ...
 *
 */
PSELINKDLL X509_CRL_S *SEC_PKI_ctxGetDeltaCRLbyCert(const SEC_PKI_CTX_S *pstContext, X509_CERT_S *pstCert);
 ...
 */
PSELINKDLL SEC_INT SEC_PKI_ctxSetOptions(SEC_PKI_CTX_S *pstContext, SEC_UINT uiOptions);
 ...
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetOptions(SEC_PKI_CTX_S *pstContext, SEC_UINT *puiOptions);

#ifndef __SEC_PKI_NO_OCSP__
/**
 * @defgroup SEC_PKI_ctxCreateOcspSingleReq
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * OCSP_SINGLE_REQ_S *SEC_PKI_ctxCreateOcspSingleReq
 * (
 * SEC_PKI_CTX_S *pstContext,
 * X509_CERT_S *pstCert,
 * SEC_UINT32 ulDigestAlgType
 * );
 * @endcode
 *
 * @par Purpose
 * This function is used to create ocsp single request based on the cert.
 *
 * @par Description
 * SEC_PKI_ctxCreateOcspSingleReq creates a OCSP_SINGLE_REQ_S object.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[in] pstCert Pointer to X509_CERT_S.[NA/NA]
 * @param[in] ulDigestAlgType Type of digest[NA/NA]
 * @param[out] NA NA [NA/NA]
 *
 * @retval OCSP_SINGLE_REQ_S* If creates OCSP_SINGLE_REQ_S is successful.[NA|NA]
 * @retval NULL If the creates OCSP_SINGLE_REQ_S is fails.[NULL|NA]
 *
 *
 * @par Memory Handling
 * \n
 * ipsi will allocate memory for OCSP_SINGLE_REQ_S and returns to the application.
 * To free this memory user has to call the OCSP_freeSingleReq function.
 */
PSELINKDLL OCSP_SINGLE_REQ_S *SEC_PKI_ctxCreateOcspSingleReq(
    SEC_PKI_CTX_S *pstContext, X509_CERT_S *pstCert, SEC_UINT32 ulDigestAlgType);

/**
 * @defgroup SEC_PKI_ctxVerifyOcspResp
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxVerifyOcspResp(
 * SEC_PKI_CTX_S *pstContext,
 * X509_CERT_S *pstCert,
 * const OCSP_RESP_S *pResp
 * );
 * @endcode
 *
 * @par Purpose
 * This function is used to verify ocsp resp based on the cert.
 *
 * @par Description
 * SEC_PKI_ctxVerifyOcspResp verify ocsp resp based on the cert.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[in] pstCert Pointer to X509_CERT_S.[NA/NA]
 * @param[in] pResp Pointer to OCSP_RESP_S[NA/NA]
 * @param[out] NA NA [NA/NA]
 *
 * @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
 * @retval SEC_INT On all failure conditions. [SEC_PKI_ERROR|NA]
 *
 * @par Note
 * The verification result of the time field involved in the response depends on the implementation of the platform.
 * For example: When "2022-13-10 12:12:12" is valid in Linux, it becomes "2023-01-10 12:12:12".
 * On other platforms such as Doprav2, it is illegal.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxVerifyOcspResp(SEC_PKI_CTX_S *pstContext, X509_CERT_S *pstCert, const OCSP_RESP_S *pResp);

/**
 * @defgroup SEC_PKI_ctxLoadCertOcspInfoBuffer
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxLoadCertOcspInfoBuffer(
 * SEC_PKI_CTX_S *pstContext,
 * OCSP_RESP_S *pOCSPResp
 * );
 * @endcode
 *
 * @par Purpose
 * This function is used to load ocsp response to the cache.
 *
 * @par Description
 * SEC_PKI_ctxLoadCertOcspInfoBuffer is used to load ocsp response to the cache.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[in] pOCSPResp Pointer to OCSP_RESP_S.[NA/NA]
 * @param[out] NA NA [NA/NA]
 *
 * @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
 * @retval SEC_INT On all failure conditions. [SEC_PKI_ERROR|NA]
 */
PSELINKDLL SEC_INT SEC_PKI_ctxLoadCertOcspInfoBuffer(SEC_PKI_CTX_S *pstContext, OCSP_RESP_S *pOCSPResp);

/**
 * @defgroup SEC_PKI_ctxRemoveCertOcspInfo
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxRemoveCertOcspInfo(
 * SEC_PKI_CTX_S *pstContext,
 * SEC_NAME_S *pstIssuerName,
 * SEC_BIGINT_S *pstSerialNum
 * );
 ...
 *
 * @par Purpose
 * This function is used to remove ocsp response from the cache.
 *
 * @par Description
 * SEC_PKI_ctxRemoveCertOcspInfo is used to remove ocsp response from the cache.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[in] pstIssuerName Pointer to SEC_NAME_S.[NA/NA]
 * @param[in] pstSerialNum Pointer to SEC_BIGINT_S.[NA/NA]
 * @param[out] NA NA [NA/NA]
 *
 * @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
 * @retval SEC_INT On all failure conditions. [SEC_PKI_ERROR|NA]
 */
PSELINKDLL SEC_INT SEC_PKI_ctxRemoveCertOcspInfo(
    SEC_PKI_CTX_S *pstContext, SEC_NAME_S *pstIssuerName, SEC_BIGINT_S *pstSerialNum);

/**
 * @defgroup SEC_PKI_ctxGetOcspInfoByCert
 * @ingroup Context_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_ctxGetOcspInfoByCert(
 * SEC_PKI_CTX_S *pstContext,
 * X509_CERT_S *pstCert,
 * OCSP_RESP_S **ppOCSPResp,
 * OCSP_SINGLE_RESP_S **ppSingleResp
 * );
 * @endcode
 *
 * @par Purpose
 * This function is used to get ocsp response from the cache.
 *
 * @par Description
 * SEC_PKI_ctxGetOcspInfoByCert is used to get ocsp response from the cache.
 *
 * @param[in] pstContext Pointer to SEC_PKI_CTX_S.[NA/NA]
 * @param[in] pstCert Pointer to X509_CERT_S.[NA/NA]
 * @param[out] ppOCSPResp Pointer to OCSP_RESP_S.[NA/NA]
 * @param[out] ppSingleResp Pointer to OCSP_SINGLE_RESP_S.NA [NA/NA]
 *
 * @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
 * @retval SEC_INT On all failure conditions. [SEC_PKI_ERROR|NA]
 *
 * @par Memory Handling
 * \n
 * ipsi will allocate memory for *ppOCSPResp if ppOCSPResp is not null and returns to the application.
 * ipsi will allocate memory for *ppSingleResp if ppSingleResp is not null and returns to the application.
 * To free this memory user has to call the OCSP_freeOCSPResp or OCSP_freeSingleResp function.
 */
PSELINKDLL SEC_INT SEC_PKI_ctxGetOcspInfoByCert(
    SEC_PKI_CTX_S *pstContext, X509_CERT_S *pstCert, OCSP_RESP_S **ppOCSPResp, OCSP_SINGLE_RESP_S **ppSingleResp);
