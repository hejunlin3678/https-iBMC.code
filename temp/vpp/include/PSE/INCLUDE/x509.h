 ...
 * Description: X509.h provides all the basic API for create,set,add and get
...
is used for enciphering application data, other than cryptographic keys. */
...
/* E-mail protection */
#define X509_EXT_KU_EMAIL_PROTECTION \
    (X509_KU_DIGITAL_SIGNATURE | X509_KU_NON_REPUDIATION | X509_KU_KEY_ENCIPHERMENT | X509_KU_KEY_AGREEMENT)

#define SEC_PKI_XKU_SSL_SERVER 0x1
#define SEC_PKI_XKU_SSL_CLIENT 0x2
#define SEC_PKI_XKU_SMIME 0x4
#define SEC_PKI_XKU_CODE_SIGN 0x8
#define SEC_PKI_XKU_SGC 0x10
#define SEC_PKI_XKU_OCSP_SIGN 0x20
#define SEC_PKI_XKU_TIMESTAMP 0x40
#define SEC_PKI_XKU_IPSECIKE 0x80
#define SEC_PKI_XKU_ANYEXTENDEDKEYUSAGE 0x100
...
EDI Party name ,URL ,RFC822 name ,DNS Name ,IPAddress, Regester id ,OTHER name ,which internally will be used in the
 creation of General Name. ORADDRESS structure not supported. */
 ...
 * @datastruct TYPE_IPADDRESS=18 To set IP addr.
 ...
 * of extension attribute
 * @datastruct TELETEX_PERSONAL_NAME_ANY_ID=32 This is used to create the  Personal name strcuture of extension
 * attribute.
 ...
 * @datastruct EXTENSION_PHYSICAL_DELIVERY_ADDRESS_COMPONENTS_ANY_ID=42 To create the physical delivery Address
 * components.
 ...
 * @datastruct pucAttrValue The NULL terminated String corresponding to the CID.
...
/* If both the fields,pointer to the string and list are present the struct  will not be created instead a NULL will be
returned back,the same if both the fields are absent,either of these fields should be present for the successfull
creation */
 ...
 * @datastruct pucData Indicates the pointer to NULL terminated Data string.
 ...
 * All the mandatory fields should be present for encoding,else the encoding will not be successfull.If encoded
 * successfully the DER code of the CRL will be returned back to the user in a character buffer.If not encoded
 * succesfully SEC_NULL will be returned.
 *
 ...
 *
 */
PSELINKDLL SEC_UCHAR *X509CRL_encode(X509_CRL_S *pstCRL, SEC_UINT32 *puiEnLength);
 ...
 *
 */
PSELINKDLL X509_CRL_S *X509CRL_decode(SEC_UCHAR *pucEncodedCRL, SEC_UINT32 uiLength, SEC_UINT32 *puiDLength);
 ...
 * This method will decrypt the hash and generates new hash on the to be TBS structure and both the hashes will be
 * compared. The public key needs to be extracted from the issuer certificate and passed to the method.
 *
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_verify(SEC_PKEY_S *pstPublickey, const X509_CRL_S *pstCRL);
 ...
 * @par Description
 * This function signs the CRL by a given private key.The signature is actually calculated on the hash computed on
 * tbsCRL structure. The calculated signature value is then assigned to the signature field. Intially the given CRL is
 * assumed to have a null signature value,if present will be overwritten with a new value The CRL issuer has to use the
 * private key to sign the CRL.It is also necessary to make sure that the all the valid extensions are present, if any
 * invalid extension is present then signing will not be a performed. If an invalid version is present then too the CRL
 * will not be signed. The hash algorithm required for signing will be fetched internally from the algorithm
 * identifier.Therefore it is necessary for the user to set the appropriate algorithm identifier.
 *
 ...
 * @retval SEC_ERR_INVALID_EXTN If duplicates are there in the list or extension and entry extensions is not valid
 * [N/A|N/A]
 * @par Dependency
 * x509.h
 * @par Note
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_sign(SEC_PKEY_S *pstPrivateKey, X509_CRL_S *pstCRL);
 ...
 * @par Description
 * This function adds an extension to the extension list in a given CRL.If the number of extensions in the list are
 * already seven, then there cannot be anymore extensions added to the list.If an invalid extension is given then the
 * extension will not be added to the list. If the add operation is successful then a success code will be returned.
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_addExtn(SEC_EXTN_S *pstExtension, const X509_CRL_S *pstCRL);
 ...
 * @par Description
 * This method adds a new revoked certificate to the list of revoked certificates. If the certificate serial number is
 * already in the list it cannot be added.Before the serial number is sent as input it needs to be converted to
 * hexadecimal representation using the api SEC_cvtToBigInt. If the revocation date is not present then an appropriate
 * return code will be returned to the user. Entry extension list is an optional parameter,if present it should be valid
 * else an appropriate error code will be returned back to the user. If added succesfully SEC_SUCCESS will be returned.
 * The certificate serial number should not have more than 20 Octets. If it has more than 20 octets an appropriate error
 * code will be returned back to the user. Note this API adds the certificate successfully even if the extension list
 * has zero nodes. The valid entry extensions that can be present in the entry extension list are : Reason Code Hold
 * Instruction Code Invalidity Date Certificate Issuer
 ...
 * @retval SEC_ERR_INVALID_ARG If the mandatory input paramater is NULL or serial number length is greater than 20
 * Octets. [SEC_ERR_INVALID_ARG|N/A]
 ...
 * This error code may be returned when criticality is not correct or extension data is not valid or it is not a valid
 * CRL entry extension. It retuns SEC_ERR_INVALID_EXTN if invalidity date has UTC Time encoded field It retuns
 * SEC_ERR_INVALID_EXTN if HOLD code has invalid OID It retuns SEC_ERR_INVALID_EXTN if REASONS has an invalid value
 * attached to it. [SEC_ERR_INVALID_EXTN|N/A]
 ...
 * Trying to add negative numbers like 0xff and 0xffff that result in same integer value will not be considered same and
 * hence will be added sucessfully.
 *
 * @par Related Topics
 * N/A
 *
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_addCert(SEC_BIGINT_S *pstCertSNumber, DATETIME_S *pstRevocationDate,
    SEC_List_S *pUserExtList, const X509_CRL_S *pstCRL);
 ...
 * @par Description
 * This function is used to set the version of the CRL.For setting V1 CRL and if it is already V2 the memory allocated
 * for V2 will be freed and SEC_SUCCESS will be returned. For a V2 CRL it allocates memory and sets the version. Default
 * version is 1.It is better to set the appropraiate version based on the existence of extensions An invalid version
 * will not allow the user to sign the CRL. Providing an invalid extension will result in an appropriate error code. Two
 * versions that are valid are X509_CRL_VERSION1 and X509_CRL_VERSION2 For a V2 CRL the value present in the version
 * field is 1.
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_setVersion(X509_CRLVERSION_E enVersion, const X509_CRL_S *pstCRL);
 ...
 * The Next Update field indicates when the next CRL will be Issued. If an invalid date is given the next update field
 * will not be added. If added successfully an appropriate success code will be returned. The user is allowed to set any
 * date but needs to ensure its value with ThisUpdate. If ThisUpdate is greater than NextUpdate condition arises then
 * CRL will not be signed. For years through 2049, nextUpdate will be encoded in UTCTime format. GeneralizedTime
 * encoding will be used for dates in the year 2050 or later.This is an optional field.
 ...
 * @par Note
 *
 * The ucUTCSign,ucUTCHour,ucUTCMinute, uwMillSecond fields of DATETIME_S are not considered in this function.Dates with
 * year less than 1970 will not be considered valid.
 *
 * @par Related Topics
 * N/A
 *
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_setNextUpdate(DATETIME_S *pstnextUpdate, const X509_CRL_S *pstCRL);
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509CRLExtn_setDPToIDP(X509_DPN_S *pstDistributionPoint, X509_IDP_S *pstIDP);
 ...
 * Each of these can be Bitwise ored to indicate more than one reasons. The Issuing distribution point structure can be
 * created
 * using the api	X509CRLExtn_createIDP. If the reasons already exist the one existing will be freed and replaced by
 * the new valid reasons.The valid reasons that need to be set are given below X509_REVOKED_UNUSED_CERT
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509CRLExtn_setReasonsToIDP(SEC_UINT32 uiReasonFlags, X509_IDP_S *pstIDP);
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_setThisUpdate(DATETIME_S *pstThisUpdate, const X509_CRL_S *pstCRL);
 ...
 * duplicated and added to the struct.If the name structure already exists the old structure will be freed and replaced
 * by a new one.The name structure can be created using the API X509_createDN.
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_setIssuer(SEC_NAME_S *pstIssuer, const X509_CRL_S *pstCRL);
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_setSignAlgId(SEC_ALG_IDENTIFIER_S *pstSignatureID, X509_CRL_S *pstCRL);
 ...
 * The HoldInstructionCode extension is a CRL entry extension that allows an instruction code identifying how to process
 * a certificate that has a reason code of CertificateHold. The List from which the value needs to be fetched has to be
 * provided as a mandatory field. Since HIC is a part of entry extensions,the list for a given certificate can be feched
 * using the method X509CRL_getEntryExtns.The Hold code returned is a CID  of some valid Hold OID. The value returned
 * should be one of the below CID_HOLDINSTRUCTION_NONE CID_HOLDINSTRUCTION_CALLISSUER CID_HOLDINSTRUCTION_REJECT
 ...
 * The InvalidityDate extension is a CRL entry extension that identifies the date at which it is known or suspected that
 * a private key was compromised.The extension is always non-critical. The List from which the value needs to be fetched
 * has to be provided as a mandatory field. Since Invalidity Date is a part of entry extensions,the list for a given
 * certificate can be fetched using X509CRL_getEntryExtns. The memory consisting of the invalidity date needs to be
 * freed explicitly by the user. If the list is null,SEC_NULL will be returned back to the user.
 * @param[in] pCRLEEList A pointer to a list containing all the entry extension for a given certificate [N/A]
 * @param[out] N/A N/A [N/A]
 ...
 * Since reason is a part of entry extensions, the list for a given certificate can be fetched using
 * X509CRL_getEntryExtns. The return value list will be of General Name structures. The certificate issuer list needs to
 * be freed explicitly by the user
 ...
 * @retval DATETIME_S* If the requested data is not available or input param is null or Time conversion fails.
 * [SEC_NULL|N/A]
 * @par Dependency
 * x509.h
 *
 ...
 * @param[in] pstCRL A reference pointer to CRL structure in the memory. [N/A]
 * @param[out] N/A N/A [N/A]
 * @retval DATETIME_S* A pointer to a structure containing the extracted next update field from the CRL.
 * [DATETIME_S*|N/A]
 ...
 * Since  CRL entry extensions is a optional field of a revoked node,it can be fetched if it is a part of the revoked
 * node. If the list is not available then a NULL is returned.
 * @param[in] pstCRL A reference to CRL structure in the memory. [N/A]
 * @param[in] pstSNumber Serial number that needs to be searched [N/A]
 * @param[out] N/A N/A [N/A]
 * @retval SEC_List_S* A pointer to a structure containing all the entry extensions for a given Serial number *
 * [SEC_List_S*|N/A]
 ...
 * N/A
 */
PSELINKDLL SEC_List_S *X509CRL_getEntryExtns(const X509_CRL_S *pstCRL, SEC_BIGINT_S *pstSNumber);
 ...
 * The CRLNumber extension is a CRL extension that contains a sequence number that allows a CRL user to determine
 * whether all CRLs prior to the current CRL have been seen. The sequence number increases sequentially.The extension is
 * always marked non-critical. Since CRL number is a part of extension list it can be fetched only if the Extension list
 * exists. If the CRL Number is returned, the memory for the structure needs to be freed explicitly by the user. The
 * List from which the value needs to be fetched has to be provided as a mandatory field. The pointer to the list is
 * obtained using X509CRL_getExtns.
 ...
 * The Delta CRL Indicator extension is a critical CRL extension that identifies this CRL as a delta CRL rather than a
 * base CRL. The corresponding base CRL is identified using a CRL number issued by the Certification Authority. Since
 * delta CRL indicator is a part of  CRL extension, the list can be fetched only if the Extension exists. If the pointer
 * is returned, memory for the returned structure needs to be freed explicitly by the user. If the extension is not
 * available SEC_NULL will be returned. The CRL extension list can be obtained using the API X509CRL_getExtns.
 ...
 * the certificate.Since Freshest CRL indicator is a part of CRL xtension list,it can be fetched only if extensions
 * exist. If the extension is not available then a NULL will be returned.If the pointer is returned the memory needs to
 * be freed explicitly by the user.The CRL extension list can be obtained using the API  X509CRL_getExtns.
 *
 ...
 *
 */
PSELINKDLL SEC_INT32 X509CRL_queryCertStatus(const X509_CRL_S *pstCRL, SEC_BIGINT_S *pstCertsnum,
    DATETIME_S *pstRevokedDate);
 ...
 * @par Description
 * Given the certificate serial number this method finds if the given certificate is present in the indirect CRL
 * structure. It searches the CRL and based on the availability of the serial number it returns the status,the
 * revocation date and the cert issuer as the output parameter. Memory for output parameter revocation date needs to be
 * provided by the user. If Cert is revoked and revocation type is not available then X509_CERT_REVOKED If the
 * revocation date is present and return value is X509_CERT_REVOKED then the certificate is revoked, but the reason is
 * not present. If the revocation date value is NULL and return value is X509_CERT_REVOKED then there is failure in time
 * conversion but the certificate is revoked.
 * If X509_CERT_UNREVOKED is returned then the cert is not revoked.
 * @param[in] pstCRL A pointer to the CRL structure in memory. [N/A]
 * @param[in] pstCertsnum The certificate serial number whose status has to be found in the CRL. [N/A]
 ...
 *
 */
PSELINKDLL SEC_INT32 X509_queryIndirectCrlCertStatus(const X509_CRL_S *pstCRL, SEC_BIGINT_S *pstCertsnum,
    DATETIME_S *pstRevokedDate, SEC_NAME_S **dpstIssuerName);
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_delCert(SEC_BIGINT_S *pstSNumber, SEC_List_S *pstRevokedList);
 ...
 * const DATETIME_S* pstDate
 * );
 * @endcode
 * @par Purpose
 * To check for the validity of a given CRL, in terms of date.
 * @par Description
 * This method is used to validate the given CRL. The input to the method is the pointer to CRL structure along with the
 * Date which needs to be validated on a given CRL.This method will compare the given date with the one available in the
 * CRL and returns the appropriate error code to ensure the validity of CRL .
 * @param[in] pstCRL A pointer to the CRL structure in memory. [N/A]
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_validateDate(const X509_CRL_S *pstCRL, const DATETIME_S *pstDate);
 ...
 * N/A
 */
PSELINKDLL SEC_BOOL X509CRLExtn_isCritical(const SEC_EXTN_S *pstExt);
...
* @ingroup x509Functions
* @image html Security.jpg <b> -# If the access permission for the CRL file is required to be configured,
   then user must not use this API. Instead, user should get encoded CRL using X509CRL_encode API
  and save the encoded CRL buffer in file with the required product access permission.
-# User must take care the file access controls and must provide canonical path before using this API.</b>
* @par Prototype
* @code
* SEC_UINT32 X509CRL_save (const SEC_CHAR* pcfileName,
* X509_CRL_S* pstCRL
* );
* @endcode
* @par Purpose
...
* The user needs to provide a pointer to the CRL struct along with the name of the file in which the CRL has to be
saved.
...
*
*/
PSELINKDLL SEC_UINT32 X509CRL_save(const SEC_CHAR *pcfileName, X509_CRL_S *pstCRL);

/**
 * @defgroup X509CRL_read
 * @ingroup x509Functions
 * @image html Security.jpg <b> User must take care the file access controls and must provide canonical path
 * when using this API. </b>
 * @par Prototype
 * @code
 ...
 * @par Description
 * This method is used to read the CRL from the local machine.The CRL needs to be available in its encoded format(DER
 * only). The application needs to provide the name of the file from which the CRL needs to be decoded. Once the CRL is
 * decoded successfully then CRL structure is returned back to the user.
 ...
 *
 */
PSELINKDLL X509_CRL_S *X509CRL_read(SEC_CHAR *pcfileName, SEC_INT32 *piRetVal);
 ...
 * N/A
 */
PSELINKDLL SEC_EXTN_S *X509CRL_getEntryExtnByCID(const X509_REVOKED_S *pstRevoked, SEC_UINT32 uiExtensionID);
 ...
 * @param[out] N/A N/A [N/A]
 * @retval SEC_EXTN_S* The extension that matches the ID of the identifier. Returns NULL if not available
 * [SEC_EXTN_S*|N/A]
 ...
 * N/A
 */
PSELINKDLL SEC_EXTN_S *X509CRL_getEntryExtnByIndex(const X509_REVOKED_S *pstRevoked, SEC_UINT32 uiIndex);
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509CRL_delEntryExtnByIndex(X509_REVOKED_S *pstRevoked, SEC_UINT32 uiIndex);
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509CRL_delEntryExtnByCID(X509_REVOKED_S *pstRevoked, SEC_UINT32 uiCID);
 ...
 * N/A
 */
PSELINKDLL SEC_EXTN_S *X509CRL_getExtnByCID(const X509_CRL_S *pstCRL, SEC_UINT32 uiExtensionID);
 ...
 *
 * @par Description
 * This method fetches a CRL  extension by a given Index,It searches through the list and if the extension is available
 * an appropriate pointer will be returned,else a null will be returned.Index value starts with zero.
 * @param[in] pstCRL The pointer to the CRL struct in memory [N/A]
 ...
 * N/A
 */
PSELINKDLL SEC_EXTN_S *X509CRL_getExtnByIndex(const X509_CRL_S *pstCRL, SEC_UINT32 uiIndex);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 X509CRL_delExtnByIndex(const X509_CRL_S* pstCRL,
 * SEC_UINT32 uiIndex
 * )
 * @endcode
 * @par Purpose
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_delExtnByIndex(const X509_CRL_S *pstCRL, SEC_UINT32 uiIndex);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 X509CRL_delExtnByCID(const X509_CRL_S* pstCRL,
 * SEC_UINT32 uiCID
 * )
 * @endcode
 * @par Purpose
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_delExtnByCID(const X509_CRL_S *pstCRL, SEC_UINT32 uiCID);
 ...
 * N/A
 */
PSELINKDLL X509_REVOKED_S *X509CRL_getRevoked(const X509_CRL_S *pstCRL, SEC_BIGINT_S *pstCertsnum);
 ...
 * @retval X509_REVOKED_S* If the input arguments are null or revoked node is not avialable or if the revoked node with
 * such a * index is not available [SEC_NULL|N/A]
 * @par Dependency
 * x509.h
 *
 * @par Memory Handling
 ...
 * N/A
 */
PSELINKDLL X509_REVOKED_S *X509CRL_getRevokedByIndex(const X509_CRL_S *pstCRL, SEC_UINT32 uiIndex);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 X509CRL_sortRevokedListBySN(const X509_CRL_S*  pstCRL)
 * @endcode
 * @par Purpose
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_sortRevokedListBySN(const X509_CRL_S *pstCRL);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 X509CRL_sortRevokedListByDate (const X509_CRL_S* pstCRL);
 * @endcode
 * @par Purpose
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_sortRevokedListByDate(const X509_CRL_S *pstCRL);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 X509CRL_delRevokedByIndex (const X509_CRL_S* pstCRL,
 * SEC_UINT32 uiIndex
 * );
 * @endcode
 * @par Purpose
 ...
 * @par Description
 * This method deletes a revoked node by a given Index. Internally the list will be searched and deletes the revoked
 * node if available else returns the appropriate error code. Index always starts with zero.
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_delRevokedByIndex(const X509_CRL_S *pstCRL, SEC_UINT32 uiIndex);
 ...
 * covers revocation for end entity  certificates only, CA certificates only, attribute certificates only, or a limited
 * set of reason codes.ALL the input fields are mandatory and the input value should be either SEC_FALSE or SEC_TRUE. If
 * the input data is not a boolean value then NULL will be returned.
 ...
 * N/A
 */
PSELINKDLL X509_IDP_S *X509CRLExtn_createIDP(SEC_BOOL bonlyContainsUCerts, SEC_BOOL bonlyContainsCACerts,
    SEC_BOOL bindirectCRL, SEC_BOOL bonlyContainsACerts);
 ...
 * @par Prototype
 * @code
 * X509_DPN_S* X509CRLExtn_getDPN (const X509_IDP_S* pstIDP);
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL X509_DPN_S *X509CRLExtn_getDPN(const X509_IDP_S *pstIDP);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509CRLExtn_getReasonsFromIDP(const X509_IDP_S* pstIDP)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509CRLExtn_getReasonsFromIDP(const X509_IDP_S *pstIDP);
 ...
 * N/A
 */
PSELINKDLL X509_CRL_S *X509_dupCRL(X509_CRL_S *pSrc);
 ...
 * N/A
 */
PSELINKDLL X509_IDP_S *X509_dupIssuingDistPoint(X509_IDP_S *pSrc);
 ...
 * 1 certificate. Along with this function to create Version 2 certiticate the other APIs to be used are
 * X509_setIssuerUID and X509_setSubjectUID. To create Version 3 certificate all the extension structures that have been
 * created has to be added to the certificate using X509_addExtnToCert API. All the input parameters mentioned has to be
 * created before calling this function. The signature algorithm field in the certificate structure and the signature
 * algorithm in the TBSCertificate structure is set with the same algorithm. The PublicKeyInfo structure is set with the
 * encoded public key and the OID of the Key Generation Algorithm (eg : RSA, DSA, * ECDSA) DH certificate can also be
 * generated by passing the DH Public key to this function. Steps for Input data preparation for this function are
 * listed below:
 ...
 *
 */
PSELINKDLL X509_CERT_S *X509_createCert(X509_CERTVERSION_E enVersion, const SEC_BIGINT_S *pstCertSelNum,
    SEC_ALG_IDENTIFIER_S *pstSignatureAlgoId, SEC_NAME_S *pstIssuerName, SEC_VALIDITY_S *pstValidity,
    SEC_NAME_S *pstSubjectName, const SEC_PKEY_S *pstSubjectPubKey);
 ...
 * const X509_CERT_S* pstCertificate
 * )
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_addExtnToCert(SEC_EXTN_S *pstExtension, const X509_CERT_S *pstCertificate);
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_checkPrivateKey(X509_CERT_S *pstCertificate, SEC_PKEY_S *pstPrivateKey);
 ...
 * For CA certificate, if Digital signature bit is set in Key usage extension and basic constraint extension present
 * then the criticality of the Basic Constriant extension has to be set to TRUE by the application.  Since the
 * DigitalSignature key usage depends on other extensions like Name constraints, Policy Mappings, Policy Constraints etc
 * that has to be validated by the Application.If all the conditions are verified then the certificate is signed and the
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_signCertificate(X509_CERT_S *pstCertificate, SEC_PKEY_S *pstPrivateKey);
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_setVersion(X509_CERTVERSION_E enCertVersion, const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509_getVersion(const X509_CERT_S* pstCertificate)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509_getVersion(const X509_CERT_S *pstCertificate);
 ...
 * @param[out] N/A N/A [N/A]
 * @retval SEC_TIME_S* Structure is created containing the time in either UTC or Generalized time format
 * [SEC_TIME_S*|N/A]
 ...
 * N/A
 */
PSELINKDLL SEC_TIME_S *X509_createTime(DATETIME_S *pstSysTime);
 ...
 * @param[in] pNotBefore The notBefore time to be converted.The time format is taken as either Generalized or UTC based
 * on the year set in pSysTime input parameter [N/A]
 * @param[in] pNotAfter The notAfter time to be converted. The time format is taken as either Generalized or UTC based
 * on the year set in pSysTime input parameter [N/A]
 * @param[out] N/A N/A [N/A]
 ...
 * @retval SEC_VALIDITY_S* On failure conditions SEC_NULL is returned.The failures conditions can be one of the
 ...
 *
 */
PSELINKDLL SEC_VALIDITY_S *X509_createValidity(DATETIME_S *pNotBefore, DATETIME_S *pNotAfter);
 ...
 * @par Prototype
 * @code
 * DATETIME_S* X509_extractNotBefore (const X509_CERT_S* pstCertificate);
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL DATETIME_S *X509_extractNotBefore(const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * DATETIME_S* X509_extractNotAfter (const X509_CERT_S* pstCertificate);
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL DATETIME_S *X509_extractNotAfter(const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_PKEY_S* X509_extractPublicKey (const X509_CERT_S* pstCertificate);
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_PKEY_S *X509_extractPublicKey(const X509_CERT_S *pstCert);
 ...
 * @par Prototype
 * @code
 * SEC_BIGINT_S* X509_getSN (const X509_CERT_S* pstCertificate);
 * @endcode
 * @par Purpose
 * To fetch the serial number from the given certificate.
 * @par Description
 * This function returns the certificate serial number from the certificate
 ...
 * N/A
 */
PSELINKDLL SEC_BIGINT_S *X509_getSN(const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_ALG_IDENTIFIER_S* X509_getSignAlgId(const X509_CERT_S* pstCertificate)
 * @endcode
 * @par Purpose
 * To fetch the signature algorithm from the given cerfificate.
 * @par Description
 * The function returns the Signature algorithm structure that contains the OID and the parameter associated with the
 * algorithm if any.
 ...
 * N/A
 */
PSELINKDLL SEC_ALG_IDENTIFIER_S *X509_getSignAlgId(const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 X509_setSubjectUID (const SEC_UCHAR* pucUniqueID,
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_setSubjectUID(const SEC_UCHAR *pucUniqueID, SEC_UINT32 uiUIDLength,
    const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_AsnBits_S* X509_getSubjectUID (const X509_CERT_S* pstCertificate);
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_AsnBits_S *X509_getSubjectUID(const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 X509_setIssuerUID (const SEC_UCHAR* pucUniqueID,
 * SEC_UINT32 uiUIDLength,
 * const X509_CERT_S* pstCertificate
 * );
 * @endcode
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_setIssuerUID(const SEC_UCHAR *pucUniqueID, SEC_UINT32 uiUIDLength,
    const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_AsnBits_S* X509_getIssuerUID (const X509_CERT_S* pstCertificate);
 * @endcode
 * @par Purpose
 ...
 * @param[in] pstCertificate The certificate  from which the UID has to be retrieved [N/A]
 * @param[out] N/A N/A [N/A]
 * @retval SEC_AsnBits_S* The structure that contains the String and the length.The length has to be divided by 8 to
 * indicate * the exact byte length of the string since the length in the structure is bitlength [SEC_AsnBits_S*|N/A]
 ...
 * N/A
 */
PSELINKDLL SEC_AsnBits_S *X509_getIssuerUID(const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_NAME_S* X509_createDN(SEC_UINT32 uiDNAttrLen, const X509_DN_ATTR_VALUE_S* pstDNAttrValue)
 * @endcode
 * @par Purpose
 ...
 *
 * @param[in] uiDNAttrLen Number of array elements. Length is limited to 65535 [N/A]
 ...
 * string. Should be NULL terminated string [N/A]
 * @param[out] N/A N/A [N/A]
 *
 * @retval SEC_NAME_S* On successful execution pointer to SEC_NAME_S structure is returned [SEC_NAME_S|N/A]
 * @retval SEC_NAME_S* On failure conditions SEC_NULL is returned. The failures conditions can be one of the
 * following:\n 1) Input parameter is NULL\n 2) Memory Allocation fail\n 3) Attribute Type may not be represented in the
 ...
 *
 */
PSELINKDLL SEC_NAME_S *X509_createDN(SEC_UINT32 uiDNAttrLen, const X509_DN_ATTR_VALUE_S *pstDNAttrValue);
 ...
 * @param[in] uiAttrValueLen The length of the input data that to be put in the AttributeTypeAndValue structure member
 * [N/A]
 * @param[in] pucAttrValue The string that has to be set in the AnyDefinedBy field of the AttributeTypeAndValue
 * structure The string is not validated for String Type other than Printable string. [N/A]
 * @param[out] N/A N/A [N/A]
 *
 ...
 * @retval SEC_ATTR_TYPE_AND_VALUE_S On failure conditions SEC_NULL is returned. The failures conditions can be one of
 * the following:\n 1.Input parameter is NULL\n 2.Memory Allocation fails\n 3.Attribute Type may not be represented in
 * the requested String format\n 4.Input data is incorrect [SEC_NULL|N/A]
 *
 * @par Dependency
 * x509.h
 *
 ...
 * N/A
 */
PSELINKDLL SEC_ATTR_TYPE_AND_VALUE_S *X509_createAttrTypeAndValue(SEC_UINT32 uiAttrType, SEC_UINT32 enStringFormat,
    SEC_UINT32 uiAttrValueLen, SEC_UCHAR *pucAttrValue);
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_setSubjectName(SEC_NAME_S *pstSubjName, const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_NAME_S* X509_getSubjectName (const X509_CERT_S* pstCertificate);
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_NAME_S *X509_getSubjectName(const X509_CERT_S *pstCertificate);
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_setIssuerName(SEC_NAME_S *pstIssuerName, const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_NAME_S* X509_getIssuerName (const X509_CERT_S* pstCertificate);
 * @endcode
 * @par Purpose
 * To get the issuer name from the given certificate.
 * @par Description
 * The function returns the Issuer name from the certificate.
 ...
 * N/A
 */
PSELINKDLL SEC_NAME_S *X509_getIssuerName(const X509_CERT_S *pstCertificate);
 ...
 * @par Description
 * This function returns the effective key usage for the certificate. This function returns the combined key usage that
 * is permitted for the certificate. During the certification validation, this method can be used to get the effective
 * key usage. In this case the key usages consistent with both the extensions has to be used.
 * @param[in] uiKeyUsage The integer value of the keyUsage. Key Usage specifies for what purpose the certificate has to
 * be used [N/A]
 * @param[in] pExtnKeyUsageList The extended key usage used to get the effective key usage [N/A]
 * @param[out] N/A N/A [N/A]
 * @retval SEC_INT32 On successful execution the combined key usage(integer value) is returned.0 is also a valid key
 * usage. [SEC_INT32|N/A]
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509Extn_getEffectiveKeyUsages(SEC_UINT32 uiKeyUsage, SEC_List_S *pExtnKeyUsageList);
 ...
 * @par Prototype
 * @code
 * SEC_EXTN_S* X509_getExtnByCID (const X509_CERT_S* pstCertificate,
 * SEC_UINT32 uiExtnID
 * );
 * @endcode
 * @par Purpose
 ...
 * @retval SEC_EXTN_S* On all failure conditions SEC_NULL is returned. The failure condition can be one of the
 * following: Input parameter is NULL, Unknown CID, Extension matching the CID is not present in the certificate
 * [SEC_NULL|N/A]
 * @par Dependency
 * x509.h
 *
 ...
 * N/A
 */
PSELINKDLL SEC_EXTN_S *X509_getExtnByCID(const X509_CERT_S *pstCertificate, SEC_UINT32 uiCID);
 ...
 * @par Prototype
 * @code
 * SEC_List_S*  X509_getExtnByCriticalFlag (const X509_CERT_S* pstCertificate,
 * SEC_BOOL bCritical
 * );
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_List_S *X509_getExtnByCriticalFlag(const X509_CERT_S *pstCertificate, SEC_BOOL bCriticalFlag);
 ...
 * be set to a predefined criticality. If the criticality is other than the mentioned value the extension is not
 * created. For CA certificate, if Digital signature bit is set in Key usage extension and basic constraint extension
 * present then the criticality of the Basic Constriant extension has to be set to TRUE by the application. Some of the
 * extension criticality that has to be always SEC_FALSE are listed below.\n For X509 EXTENSIONS:\n Authority Key
 ...
 * To create KEY USAGE extension, the macros that are provided are to be used to specify the key usages of the
 * certificate.\n To create ANY POLICY extension, the number that has to be set has to be passed to function using a
 * pointer variable.
 *
 ...
 * @retval SEC_EXTN_S* On failure conditions SEC_NULL is returned. The failures conditions can be one of the
 * following:\n
 * 1. Input parameter is NULL\n
 * 2. Memory Allocation fails\n
 ...
 *
 */
PSELINKDLL SEC_EXTN_S *X509_createExtn(SEC_UINT32 uiCID, SEC_BOOL bCriticalFlag, SEC_VOID *pstExtnData);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509_getCIDFromExtn(const SEC_EXTN_S* pstExtn)
 * @endcode
 * @par Purpose
 * To get the CID from the given extension.
 * @par Description
 * The function returns the Common ID from the extension. The function calls the SEC_getCID function to get the integer
 * mapping of of the OID in Extension.
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509_getCIDFromExtn(const SEC_EXTN_S *pstExtn);
 ...
 * @par Prototype
 * @code
 * SEC_VOID* X509Extn_extractContent(const SEC_EXTN_S* pstExtn)
 * @endcode
 * @par Purpose
 * To get the decoded data from the given extension.
 * @par Description
 * This function is used to get the decoded data from the extension. The extension structure passed is decoded depending
 * on the OID in the extension.The function returns the pointer to the decoded data from the extension.
 ...
 * N/A
 */
PSELINKDLL SEC_VOID *X509Extn_extractContent(const SEC_EXTN_S *pstExtn);
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509Extn_setCritical(SEC_BOOL bCriticalFlag, SEC_EXTN_S *pstExtn);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509Extn_getCritical(const SEC_EXTN_S* pstExtn)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509Extn_getCritical(const SEC_EXTN_S *pstExtn);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509_getExtnCount(const X509_CERT_S* pstCertificate)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509_getExtnCount(const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_EXTN_S* x509CRL_getExtnFromList(SEC_List_S* pEList,SEC_UINT32  uiExtensionID)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_EXTN_S *x509CRL_getExtnFromList(SEC_List_S *pEList, SEC_UINT32 uiExtensionID);
 ...
 * @par Description
 * The function compares two names structures. The comparison is done by first comparing the length and then the
 * comparison of the value in the AttributeTypeAndValue structure that holds the data.
 ...
 * @par Description
 * The function compares two names structures. The comparison is done by first comparing the length and then the
 * comparison of the value in the AttributeTypeAndValue structure that holds the data.The comparison is continued in
 * the' second list until the match is found.
 * @param[in] pstSrcName The source name that has to be compared [N/A]
 * @param[in] pstTrgName The name that has to be compared against the source name [N/A]
 * @param[out] N/A N/A [N/A]
 * @retval SEC_UINT32 This is returned if the name mathces [SEC_SUCCESS|N/A]
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 X509_compareIssuerAndSerial(const X509_CERT_S* pstSourceCert,
 * const X509_CERT_S* pstTargetCert
 * )
 * @endcode
 * @par Purpose
 * To compare the Issuer name and Serial number of two certificates.
 * @par Description
 * The function extracts the issuer name and serial number from the certificates. This Compares the Issuer name and
 * serial number of two certificates. This is Used during certificate verification and validation.
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_compareIssuerAndSerial(const X509_CERT_S *pstSourceCert, const X509_CERT_S *pstDestCert);
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509_calcNameHash(SEC_NAME_S *pstName, SEC_UINT32 uiAlgoID, SEC_UINT32 *puiHashLen);
 ...
 * @par Description
 * The function generates the hash of the Issuer Name and serial number.The pstIssuerAndSerial structure is encoded and
 * then hash on in it calculated based on the hash algorithm given as input. The hash value can be used for verification
 * of the issuer name and serial number.The hash algorithms to be used are one of the below: CID_MD5 CID_SHA1 CID_SHA256
 * CID_SHA384
 * CID_SHA512
 * CID_SHA224
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509_calcIssuerAndSerialHash(SEC_ISSUER_AND_SERIAL_S *pstIssuerAndSerial, SEC_UINT32 uiAlgoID,
 ...
 * @par Description
 * The function creates the BasicConstraint extension. The basic constraints extension is used only in CA certificate.
 * The path length field in the structure indicates number of certificates that appear in the certificate path. This
 * number is used during certificate path validation. The use of Pathlength in BAsic Constraint extension has to be
 * decided by the application. If this field has to be optional then a negative value has to be provided as input.
 * @param[in] bCAFlag Variable to indicate CA certificate [N/A]
 * @param[in] iPathLength Number of certificate in certificate path used during verification. If the pathlength should
 * not be created then input to the function has to be negative value. Zero and any positive value creates pathLength
 * optional field [N/A]
 * @param[out] N/A N/A [N/A]
 ...
 * N/A
 */
PSELINKDLL X509_BASIC_CONSTRAINT_S *X509Extn_createBasicConstraint(SEC_BOOL bIsCA, SEC_INT32 uiPathLength);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509_isCACert(const X509_CERT_S* pstCertificate)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509_isCACert(const X509_CERT_S *pstCertificate);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509_getPathLen(const X509_BASIC_CONSTRAINT_S* pstBasicConstraint)
 * @endcode
 * @par Purpose
 * To fetch the path length from
 * @par Description
 * The function returns the path length from the certificate if the certificate is CA certificate.
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509_getPathLen(const X509_BASIC_CONSTRAINT_S *pstBasicConstraint);

/**
 * @defgroup X509Extn_createAKID
 * @ingroup x509Functions
 * @image html icon-stop.png <b> This API is deprecated. Use the X509Extn_createAKID_ex API instead. </b>
 * @par Prototype
 * @code
 ...
 * @par Description
 * This API is used for a CA certificate. This extension is used when the CA has more than one signing keys and this
 * extension helps in finding the key pair. The keyIdentifier field of the extension must be included in all
 * certificates generated by confirming CAs to facilitate certification path construction. The key identifier generation
 * is either 160 bit SHA1 hash value or 64 bit value. But the usage of this API is deprecated. Please refer the Note
 ...
 * @retval X509_AKID_S* On failure conditions SEC_NULL is returned. The failure conditions can be one of the
 * following:\n 1> Input parameters is NULL,\n 2> Memory allocation fails, 3> Encoding of Public key fails, 4> Hash
 * calculation fails [SEC_NULL|N/A]
 * @par Dependency
 * x509.h
 *
 * @par Memory Handling
 ...
 *
 */
PSELINKDLL X509_AKID_S *X509Extn_createAKID(SEC_BIGINT_S *pstCertSerialNum, SEC_List_S *pCertificateIssuer,
    SEC_PKEY_S *pstPublicKey, X509_KIDGENTYPE_E enKIDType);
 ...
 * @par Prototype
 * @code
 * SEC_BIGINT_S* X509Extn_getSNFromAKID(const X509_AKID_S* pAKID)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_BIGINT_S *X509Extn_getSNFromAKID(const X509_AKID_S *pstAKID);
 ...
 * @par Prototype
 * @code
 * SEC_List_S* X509Extn_getIssuerFromAKID(const X509_AKID_S* pAKID)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_List_S *X509Extn_getIssuerFromAKID(const X509_AKID_S *pstAKID);
 ...
 * @param[out] N/A N/A [N/A]
 * @retval SEC_AsnOcts_S* Pointer to the KeyIdentifier structure that consists of the hash of the public key and the
 * hash length [SEC_AsnOcts_S*|N/A]
 ...
 * N/A
 */
PSELINKDLL SEC_AsnOcts_S *X509Extn_getKIDFromAKID(X509_AKID_S *pstAKID);
 ...
 * @par Prototype
 * @code
 * SEC_UCHAR* X509Extn_generateKID(const SEC_UCHAR* pucPublicKey,
 ...
 * @par Description
 * The function is used to generate KeyIdentifier based on the given method. This function is used for calculating the
 * hash on the public key. The length of the hash value is determined by the KID generation type. The process is to
 * encode the Public Key and then hash the encoded public key using SHA1 algorithm
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509Extn_generateKID(const SEC_UCHAR *pucPublicKey, SEC_UINT32 uiPubKeyLen,
    X509_KIDGENTYPE_E enKeyHashType, SEC_UINT32 *uiHashLength);
 ...
 * Such an action will lead to undefined behaviour. The list to the function is a sequence of X509_COMMONDATA_S
 * structures, containing either a X509_COMMONDATA_S with the List in the structure set or a String containing the data
 * that has to be set. In case the type of X509_COMMONDATA_S is TYPE_URL, TYPE_DNSNAME or TYPE_RFC822NAME it should
 * contain an IA5String. Options in General Name structure created with NULL terminated String only as input\n URI, All
 * the validation for the URL has to be taken care in the application.\n DNSName, All the validation for the DNSName has
 * to be taken care in the application.\n RFC822Name, All the validation for the RFC822Name has to be taken care in the
 * application.\n IPAddress, The IPAddress is validated only for IPv4\n RegisteredID Input to this type will be a OID of
 * this format. Each element in OID should fall in the range of [0, 2147483646] Example - 4.23.567.89  \n Options in
 * GeneralName structure created with List provided as input. The list has to be a number of X509_COMMONDATA_S
 * structures only.\n To set EDIPartyName, ORAddress or OtherName options in GeneralName then pass the list as input
 * that contains TYPE_EDIPARTYNAME, TYPE_OTHERNAME or TYPE_ORADDRESS and the list of input to set the appropriate data
 * in those structures.\n In GeneralName\n EDIPArtyName - The list should contain either or both TYPE_NAMEASSINGER or
 * TYPE_PARTYNAME as the type to be created in the X509_COMMONDATA_S structures added to the list.\n TYPE_NAMEASSIGNER
 * only does not create the structure alone, It should always go along with TYPE_PARTYNAME.\n OtherName - The other Name
 * contains a AnyDefinedType value in which a structure PermanentIdentifier has to be created and set to the parameter
 * value. To create PermanentIDentifier, the list can contain either or both TYPE_IDENTIFIERVALUE and TYPE_ASSIGNER as
 * the types in the X509_COMMONDATA_S structures added to the list. Both Types are optional. Hence any one option will
 * create the OtherName structure.Each element in OID should fall in the range of [0, 2147483646]\n ORAddress - It is
 * another type of struture that can give the name information in a more clearer way. The structure needs to be created
 * using the the below API.The input to create the ORAddress structure is a list of common nodes. The user should never
 * acces the structure directly, any creation should go through the create X509_createGeneralName. All input validation
 * will be done in the internal API.
 *
 * @param[in] pDataList The list that contains data to create GeneralName with all options other than Directory Name.
 * To create GeneralName with DirectoryName option call X509Extn_setNameToGeneralName function. The list is a sequence
 * of X509_COMMONDATA_S structures that hold the data to create the general name [N/A]
 * @param[out] N/A N/A [N/A]
 *
 * @retval SEC_GEN_NAME_S* Upon successful execution, a pointer to SEC_GEN_NAME_S structure is returned
 * [SEC_GEN_NAME_S*|N/A]
 ...
 *
 */
PSELINKDLL SEC_GEN_NAME_S *X509_createGeneralName_ex(X509_TYPE_E enGenNameType, SEC_UCHAR *pucData, SEC_UINT32 uLen);
 ...
 * @par Description
 * This function creates the extended keyusage extension for the certificate. The list here is a list of CIDs that
 * specifies the Extended Key Usages that need to be supported. The options that are allowed to create the Extended key
 ...
 * @param[out] N/A N/A [N/A]
 * @retval SEC_List_S* Upon successful execution, extended key usage list is created with the OIDs corresponding to the
 * CIDs passed as input [SEC_List_S*|N/A]
 ...
 *
 */
PSELINKDLL SEC_List_S *X509Extn_createExtendedKeyUsage(SEC_List_S *pIdentifierList);
 ...
 * @endcode
 * @par Purpose
 * To create the Subject Alternate Directory list from the list of X509_COMMONDATA_S structures.
 * @par Description
 * This API creates a list of Attribute structures. This is an extension supported by the standard. This extension is
 * used to provide additional identification of the certificate user. The input list consists of a list of
 * X509_COMMONDATA_S structures.Some of the values in the Subject Directory Attributes extension are a) Place of Birth [
 ...
 * For all the above mentioned types the X509_COMMONDATA_S structures contain a list used to create the extension.
 ...
 * @par Prototype
 * @code
 * X509_PRIVKEY_VALIDITY_S* X509Extn_createPrivKeyValidity(const DATETIME_S* pstNotBefore,
 * const DATETIME_S* pstNotAfter
 * )
 * @endcode
 * @par Purpose
 * To create the X509_PRIVKEY_VALIDITY_S structure given the date & time of the validity period.
 * @par Description
 * The function creates the Private Key Usage period. This extension is used to restrict the usage of the certificate
 * after the validity period of the private key has elapsed. The extension identifies the time period from which the
 * private key is usable and the time after which the private key is no longer trusted/used. The time period is
 * converted to Generalized Time format. When both the inputs are SEC_NULL the structure is not created.
 ...
 * @param[out] N/A N/A [N/A]
 * @retval X509_PRIVKEY_VALIDITY_S* On successful execution, a pointer to the X509_PRIVKEY_VALIDITY_S structure is
 * returned [X509_PRIVKEY_VALIDITY_S*|N/A]
 ...
 *
 */
PSELINKDLL X509_PRIVKEY_VALIDITY_S *X509Extn_createPrivKeyValidity(const DATETIME_S *pstDopraNotBefore,
    const DATETIME_S *pstDopraNotAfter);
 ...
 * @par Prototype
 * @code
 * DATETIME_S* X509Extn_extractNotAfter(const X509_PRIVKEY_VALIDITY_S* pstPrivKUPeriod)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL DATETIME_S *X509Extn_extractNotAfter(const X509_PRIVKEY_VALIDITY_S *pstPrivKUPeriod);
 ...
 * @par Prototype
 * @code
 * DATETIME_S* X509Extn_extractNotBefore(const X509_PRIVKEY_VALIDITY_S* pstPrivKUPeriod)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL DATETIME_S *X509Extn_extractNotBefore(const X509_PRIVKEY_VALIDITY_S *pstPrivKUPeriod);
 ...
 * If both pucDataString and pDataList is present the structure will not be created instead SEC_NULL will be returned
 * back.
 * @param[in] enInfoType The type of information for which the the structure is being created [N/A]
 * @param[in] pucDataString NULL terminated Character string that has to be used depending on the type of previous
 * argument [N/A]
 ...
 * @param[out] N/A N/A [N/A]
 * @retval X509_COMMONDATA_S* Upon successful execution, a pointer to the X509_COMMONDATA_S structure is returned
 * [N/A|N/A]
 * @retval X509_COMMONDATA_S* On failure conditions SEC_NULL is returned. The failure conditions can be one of the
 * following: If either or both simple string and List are NULL or If Memory Allocation fails [SEC_NULL|N/A]
 * @par Dependency
 * x509.h
 *
 * @par Memory Handling
 ...
 * N/A
 */
PSELINKDLL X509_COMMONDATA_S *X509_createCommonData(X509_TYPE_E enInfoType, SEC_UCHAR *pucDataString, SEC_List_S *pList,
    X509_STRINGTYPE_E enStringType);
 ...
 * are acceptable is considered. Each policy has an OID associated with it. The policy is set using the TYPE_POLICYID
 * and the string containing the OID. Each element in OID should fall in the range of [0, 2147483646]. If the policy
 * alone is not sufficient then the qualifiers are used. Each policy consists of CPS URI or UserNotice qualifiers.	The
 * list is a set of X509_COMMONDATA_S structures which are used to set CPS URI, or UserNotice qualifier.\n CPS URI - The
 * X509_COMMONDATA_S has the simple string containing the URI where the Certificate Policy Statements can be
 * referred.Uses TYPE_CPS to create this qualifier.\n
 * UserNotice - The UserNotice contains the explict text, organization text and list of numbers. To create UserNotice
 * the structure contains  a list of X509_COMMONDATA_S structures.\n TYPE_USERNOTICE - To create UserNotice the types to
 * be used are TYPE_ORG_TXT, TYPE_EXPLICIT_TEXT and 	TYPE_NOTICE_NUMBERS. The API fails, if more than one data of the
 * same type is tried to be added.
 ...
 *
 */
PSELINKDLL X509_POLICY_INFO_S *X509Extn_createPolicyInformation(SEC_List_S *pDataList);
 ...
 * @par Prototype
 * @code
 * SEC_AsnOcts_S* X509Extn_getExplicitText(const X509_USER_NOTICE_S* pstUNotice,
 ...
 * N/A
 */
PSELINKDLL SEC_AsnOcts_S *X509Extn_getExplicitText(const X509_USER_NOTICE_S *pstUNotice, SEC_UINT32 *puiStringType);
 ...
 * @par Prototype
 * @code
 * SEC_AsnOcts_S* X509Extn_getOrganizationText(const X509_USER_NOTICE_S* pstUNotice,
 * SEC_UINT32* puiStringType
 * )
 * @endcode
 * @par Purpose
 ...
 * @par Description
 * Returns the organization text from the X509_USER_NOTICE_S certificate policy qualifier. The organization text states
 * the policy acceptable to an organization. This text is used during certificate path validation process
 * @param[in] pstUNotice The qualifier from which the text has to be retrieved [N/A]
 * @param[out] puiStringType The string format in which the text has been represented [N/A]
 ...
 * N/A
 */
PSELINKDLL SEC_AsnOcts_S *X509Extn_getOrganizationText(const X509_USER_NOTICE_S *pstUNotice, SEC_UINT32 *puiStringType);
 ...
 * @par Prototype
 * @code
 * SEC_List_S* X509Extn_getNoticeNumbers(const X509_USER_NOTICE_S* pstUNotice)
 * @endcode
 * @par Purpose
 * To fetch a list of Notice Numbers from the given X509_USER_NOTICE_S structure.
 * @par Description
 * Returns a list of NoticeNumbers from the NoticeReference structure in the X509_USER_NOTICE_S qualifer of the
 * certificate policy. The list of numbers point to the organization text that has to be referred during certificate
 * policy verification.
 ...
 * N/A
 */
PSELINKDLL SEC_List_S *X509Extn_getNoticeNumbers(const X509_USER_NOTICE_S *pstUNotice);
 ...
 * @param[in] pIssuerDomPolicyList The list of Issuer Domain policy. This List contains the list of policies supported
 * by the CA within its domain [N/A]
 * @param[in] pSubjectDomPolicyList The list of Subject Domain policy equivalent to the Issuing CA domain policies [N/A]
 * @param[out] N/A N/A [N/A]
 * @retval X509_POLICY_MAPPING_LIST* The structure that contains the mappings of the Issuing and Subject Domain policy *
 * [X509_POLICY_MAPPING_LIST*|N/A]
 * @retval X509_POLICY_MAPPING_LIST* On failure conditions SEC_NULL is returned. The failure conditions can be one of
 * the following: Input parameter is NULL, Memory Allocation fails or Input data is incorrect [SEC_NULL|N/A]
 * @par Dependency
 * x509.h
 *
 * @par Memory Handling
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509Extn_getIssuerDomainPolicy (const X509_POLICY_MAPPING_S* pPolMapping);
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509Extn_getIssuerDomainPolicy(const X509_POLICY_MAPPING_S *pPolicyMappings);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509Extn_getSubjectDomainPolicy(const X509_POLICY_MAPPING_S* pPolMapping)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509Extn_getSubjectDomainPolicy(const X509_POLICY_MAPPING_S *pPolicyMappings);
 ...
 * It can be used to prohibit policy mapping or require that each certificate in a path contain an acceptable policy
 * identifier. This extension is used in CA certificates. If any of the Policies has to be omitted then the value to the
 * function has to negative value. If both the values are negative then the structure is not created. Positive values
 * indicate the value that has to be reflected in the certificate.
 ...
 * Please note that negative value should be used if this field is intended to be omitted during creation of the
 * structure.[N/A]
 * @param[out] N/A N/A [N/A]
 ...
 * N/A
 */
PSELINKDLL X509_POLICY_CONSTRAINT_S *X509Extn_createPolicyConstraints(SEC_INT32 iExplicitPolicy,
    SEC_INT32 iInhibitPolicy);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509Extn_getExplicitPolicy (const X509_POLICY_CONSTRAINT_S* pPolConstraint);
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509Extn_getExplicitPolicy(const X509_POLICY_CONSTRAINT_S *pstPolyConstr);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509Extn_getInhibitPolicy(const X509_POLICY_CONSTRAINT_S* pPolConstraint)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509Extn_getInhibitPolicy(const X509_POLICY_CONSTRAINT_S *pstPolyConstr);
 ...
 * @par Description
 * This function creates the EDIPartyName structure that is part of the GeneralName Structure. The EDIPartyName consists
 * of PartyName and NameAssigner fields. The input to this function is a list of X509_COMMONDATA_S structures. The
 * structues should contain only simple strings and the list member must be NULL for the types mentioned below.  This
 * function should not be used for creating EDIPartyName in GeneralName structure. To set EDIPartyName in GeneralName
 * use X509_createGeneralName function and provide list of X509_COMMONDATA_S as input. The types that has to be used to
 * create EDIPartyName Structure are TYPE_NAMEASSIGNER and TYPE_PARTYNAME.
 *
 * @param[in] pDataList The list that contains the necessary data to create EDIPartyName. The List contains
 * X509_COMMONDATA_S structure, that contain the data to create the EDIPartyName [N/A]
 * @param[out] N/A N/A [N/A]
 *
 ...
 * @par Prototype
 * @code
 * SEC_AsnOcts_S* X509Extn_getPartyName(const X509_EDI_PARTYNAME_S* pstEDIPartyName,
 * SEC_UINT32* puiStringType
 * )
 * @endcode
 * @par Purpose
 ...
 * @par Description
 * Returns the PartyName from the EDIPartyName Structure. The EDIPartyName structure has to be obtained from the
 ...
 * @retval SEC_AsnOcts_S* Pointer to the Structure that contains the character string and its length
 * [SEC_AsnOcts_S*|N/A]
 ...
 * N/A
 */
PSELINKDLL SEC_AsnOcts_S *X509Extn_getPartyName(const X509_EDI_PARTYNAME_S *pstEDIPartyName, SEC_UINT32 *puiStringType);
 ...
 * @par Prototype
 * @code
 * SEC_AsnOcts_S* X509Extn_getNameAssigner(const X509_EDI_PARTYNAME_S* pstEDIPartyName,
 * SEC_UINT32* puiStringType
 * )
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_AsnOcts_S *X509Extn_getNameAssigner(const X509_EDI_PARTYNAME_S *pstEDIPartyName,
    SEC_UINT32 *puiStringType);
 ...
 * @par Description
 * This function is used to create AuthorityInfoAccess or SubjectInfoAccess. The list contains X509_ACCESS_DESC_S
 * structures that specifies the method from where the services from the CA can be accessed in case of CA certificate or
 ...
 * @param[in] enInfoAccessChoice The enum that indicates whether the SubjectInfoAccess or the AuthorityInfoAccess list
 * has to be created [N/A]
 ...
 *
 */
PSELINKDLL SEC_List_S *X509Extn_createInfoAccess(X509_INFO_ACCESS_CHOICE_E enInfoAccessChoice, SEC_List_S *pOIDList,
    SEC_List_S *pAccessLocList);
 ...
 *
 */
PSELINKDLL SEC_List_S *X509CRLExtn_createAuthorityInfoAccess(SEC_List_S *pOIDList, SEC_List_S *pAccessLocList);
 ...
 * @par Description
 * The function is used to create the SubTree that helps in certificate path validation. The structure contains the name
 * that is either added to the permitted or excluded list.
 ...
 * N/A
 */
PSELINKDLL X509_GEN_SUBTREE_S *X509Extn_createGeneralSubTree(SEC_GEN_NAME_S *pstGeneralName, SEC_UINT32 uiMinimum,
    SEC_INT32 iMaximum);
 ...
 * @par Description
 * This function creates the Name Constraints structure that contains the Permitted and Excluded subtrees that indicates
 * a name space with all subject names in subsequent certificates in a certification path. This extension is only used
 * in CA certificate. This extension consists of the list of permitted and excluded certificate names that has to be
 * verified during path validation. If the certificate name is in the excluded list then certificate is invalid. Each of
 * the permitted and excluded name in the NameConstraint consists of GeneralSubTree. The minimum value in the SubTree is
 * usually 0 and maximum is not used, hence the value for maximum has to be always -1
 ...
 * @param[out] N/A N/A [N/A]
 * @retval X509_NAME_CONSTRAINT_S* On successful execution pointer to X509_NAME_CONSTRAINT_S structure is returned *
 * [X509_NAME_CONSTRAINT_S*|N/A]
 ...
 * @par Prototype
 * @code
 * SEC_List_S* X509Extn_getPermittedSubTree(const X509_NAME_CONSTRAINT_S* pstNameConstr)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_List_S *X509Extn_getPermittedSubTree(const X509_NAME_CONSTRAINT_S *pstNameConstr);
 ...
 * @par Prototype
 * @code
 * SEC_List_S* X509Extn_getExcludedSubTree(const X509_NAME_CONSTRAINT_S* pstNameConstr)
 * @endcode
 * @par Purpose
 * To fetch the list of Excluded names from a Name constraint structure.
 * @par Description
 * Returns the excluded list of names from the structure. The list is used during certificate path validation process.
 ...
 * N/A
 */
PSELINKDLL SEC_List_S *X509Extn_getExcludedSubTree(const X509_NAME_CONSTRAINT_S *pstNameConstr);
 ...
 * @param[in] pstName The name that has to be set. The pstName can be either the RelativeDistinguishedName or the
 * FullName i.e, the AlternateName (list of General Name) [N/A]
 * @param[out] N/A N/A [N/A]
 * @retval X509_DPN_S* The structure that contains the name of of the CRL Issuer. The name can be either the Full Name
 * or the Relative distinguished Name of the CA that has issued the CRL [X509_DPN_S*|N/A]
 ...
 *
 * @par Description
 * X509_DP_S, the CRL Distribution point, indicates where and how the CRL can be accessed. The structure also contains
 * the reasons for the creation of CRL along with the CRL Issuer Name. The CRLIssuer field is filled when the CRL is
 * generated by a different CA. The reason flags has to be present with either The CRLIssuer or the
 * DistributionPointName. The Distribution Point must consist of either of the following combinations:\n X509_DPN_S and
 * reasons\n CRL Issuer and reasons\n X509_DPN_S, CRL Issuer and reasons
 *
 * @param[in] pstDistPointName The structure that contains the RelativeDistinguishedName or FullName of the CRL issuer
 * [N/A]
 * @param[in] uiReason The reason associated with the CRL that is generated by the CRL Issuer [N/A]
 * @param[in] pGenNamesList CRL Issuer Alternate name if the CRL is issued by a CA that has not created the certificate
 * [N/A]
 * @param[out] N/A N/A [N/A]
 *
 * @retval X509_DP_S* Pointer to the created Structure. This structure is added to the CRL Distribution Point List
 * [X509_DP_S*|N/A]
 ...
 *
 */
PSELINKDLL X509_DP_S *X509Extn_createDP(X509_DPN_S *pstDistPointName, SEC_UINT32 uiReason, SEC_List_S *pCRLIssuer);
 ...
 * @par Prototype
 * @code
 * SEC_List_S* X509Extn_getCRLIssuerFromDistPoint(const X509_DP_S* pstDistPoint)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_List_S *X509Extn_getCRLIssuerFromDistPoint(const X509_DP_S *pstDistPoint);
 ...
 * @par Prototype
 * @code
 * X509_DPN_S* X509Extn_getDPNFromDistPoint(const X509_DP_S* pstDistPoint)
 * @endcode
 * @par Purpose
 * To fetch the Distribution point name from the Distribution point structure.
 * @par Description
 * This function returns the distribution point name from the given X509_DP_S structure.
 ...
 * N/A
 */
PSELINKDLL X509_DPN_S *X509Extn_getDPNFromDistPoint(const X509_DP_S *pstDistPoint);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509Extn_getReasonFromDistPoint(const X509_DP_S* pstDistPoint)
 * @endcode
 * @par Purpose
 * To fetch the CRL reason from the X509_DP_S structure.
 * @par Description
 * This function returns the CRL reason from distribution point name structure.
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509Extn_getReasonFromDistPoint(const X509_DP_S *pstDistPoint);
 ...
 * @retval X509_CERT_S* On failure conditions SEC_NULL is returned. The failure conditions can be one of the
 * following:\n
 ...
 * N/A
 */
PSELINKDLL X509_CERT_S *X509_decodeCert(SEC_UCHAR *pucEncodedCertificate, SEC_UINT32 uiEncodedLength,
    SEC_UINT32 *puiDecodedLength);
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509_genCertFingerPrint(X509_CERT_S *pstCertificate, SEC_UINT32 uiHashAlgo,
    SEC_UINT32 *puiFingerPrintLength);
 ...
 * @par Prototype
 * @code
 * X509_CERT_S* X509_searchByName(const SEC_List_S* pCertList,
 * SEC_NAME_S* pstName)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL X509_CERT_S *X509_searchByName(const SEC_List_S *pCertList, SEC_NAME_S *pstSubjectName);
 ...
 * @par Description
 * The function is used to find the certificate in a list based on the issuer name and serial number. The name and
 * serial number passed as input are compared against the issuer name and serial number in the list of certificates. If
 * both name and serial number matches then the pointer to the certificate that matches the name and serial number is
 * returned. The list can contain a list of CA certificates that can be used by the application to check whether the
 * Certificate is issued by one of the trusted Certificate authority.
 * @param[in] pCertList The list of certificates whose issuer name and serial number that has to be compared against the
 * name given as part of input [N/A]
 ...
 * N/A
 */
PSELINKDLL X509_CERT_S *X509_searchByIssuerAndSerial(SEC_List_S *pCertList, SEC_NAME_S *pstIssuerName,
    SEC_BIGINT_S *pstCertSerialNum);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 X509_verifySignature(const X509_CERT_S* pstCertificate,SEC_PKEY_S* pstPublicKey)
 * @endcode
 * @par Purpose
 ...
 * certificate. To verify an End entity certificate with DSA or ECDSA key. The parameters have to be retrieved from the
 * CA certificate and have to be set to the publickey decoded from the certificate using CRYPT_PKEY_getAlgParams. For
 * RSA the certificate can be validation by decoding the public key using X509_extractPublicKey API and calling this
 * API.
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_verifySignature(const X509_CERT_S *pstCertificate, SEC_PKEY_S *pPublicKey);
 ...
 * @par Description
 * This function adds a certificate to the certificate List. The certificate passed as input is duplicated and added to
 * the list. The list has to be created by the user before adding a certificate to the list.
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509_delCertByName(SEC_NAME_S *pstSourceName, SEC_List_S *pCertList);
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509_delCertBySN(SEC_BIGINT_S *pstSourceSerialNum, SEC_List_S *pCertList);
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509_compareSN(SEC_BIGINT_S *pSrcSerialNum, SEC_BIGINT_S *pTrgSerialNum);
 ...
 * @par Description
 * This function is used to create the algorithm identifier structure with the parameter and CID of the algorithm. The
 * OID of the algorithm is obtained inside the function.
 * @param[in] uiAlgoID The integer value(CID) for the Algorithm. For All the signing algorithms the parameters
 * are directly taken from the public key and so parameter should be NULL. The signature algorithms to be set are one of
 * the following. ALGID_MD5WITHRSA ALGID_SHA1WITHRSA ALGID_SHA1WITHRSAOLD ALGID_DSA_WITH_SHA1 ALGID_DSAWITHSHA1_2
 * ALGID_ECDSA_WITH_SHA1
 * ALGID_ECDSA_WITH_SHA224
 * ALGID_ECDSA_WITH_SHA256
 * ALGID_ECDSA_WITH_SHA384
 ...
 * @param[in] pstParameter The parameter that has to be filled in the parameter field of the algorithm identifier. For
 * the signing algorithms the parameter is null and is taken from the Public Key [N/A]
 * @param[out] N/A N/A [N/A]
 ...
 * @par Prototype
 * @code
 * SEC_INT32 X509_getCIDFromAlgId(const SEC_ALG_IDENTIFIER_S*  pstAlgoId)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_INT32 X509_getCIDFromAlgId(const SEC_ALG_IDENTIFIER_S *pstAlgorithmIdent);
 ...
 * @par Prototype
 * @code
 * SEC_VOID* X509_getParamFromAlgId(const SEC_ALG_IDENTIFIER_S* pstAlgoId)
 * @endcode
 * @par Purpose
 * To get the parameter structure corresponding to a particular Algorithm Identifier structure.
 * @par Description
 * This function returns the parameter structure corresponding to the algorithm set in the algorithm structure.
 ...
 * N/A
 */
PSELINKDLL SEC_VOID *X509_getParamFromAlgId(const SEC_ALG_IDENTIFIER_S *pstAlgoId);
 ...
 * @par Prototype
 * @code
 * SEC_PUBKEY_INFO_S* X509_createPubKeyInfo(const SEC_PKEY_S* pstPubKey)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_PUBKEY_INFO_S *X509_createPubKeyInfo(const SEC_PKEY_S *pstPublicKey);
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509_encodePubKeyInfo(SEC_PUBKEY_INFO_S *pstPubKeyInfo, SEC_UINT32 *puiEncodedLen);
 ...
 * N/A
 */
PSELINKDLL SEC_PUBKEY_INFO_S *X509_decodePubKeyInfo(SEC_UCHAR *pucBuffer, SEC_UINT32 uiEncodedLength,
    SEC_UINT32 *pstDecodedLen);
 ...
 * @par Prototype
 * @code
 * SEC_ALG_IDENTIFIER_S* X509_getAlgIdFromPubKeyInfo(const SEC_PUBKEY_INFO_S* pstSubjPKInfo)
 * @endcode
 * @par Purpose
 * To extract the Algorithm ID structure from Public Key Info structure.
 * @par Description
 * This fucntion returns the Algorithm Identifier structure from the SubjectPuclicKeyInfo. The SubjectPuclicKeyInfo
 * contains the algorithm used to create the public key. The algorithm used may be RSA, DSA, ECDSA,DH etc.
 ...
 * N/A
 */
PSELINKDLL SEC_ALG_IDENTIFIER_S *X509_getAlgIdFromPubKeyInfo(const SEC_PUBKEY_INFO_S *pstSubPKInfo);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32 X509_setSN(const SEC_BIGINT_S* pstSerialNumber,
 * const X509_CERT_S* pstCertificate)
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_setSN(const SEC_BIGINT_S *pstSerialNumber, const X509_CERT_S *pstCertificate);
 ...
 * @par Description
 * The function performs the encoding of the name structure. The length of the encoded name is reflected in
 * uiReturnEncodedLen parameter.
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509_encodeName(SEC_NAME_S *pstName, SEC_UINT32 *uiReturnEncodedLen);
 ...
 * @par Description
 * The function performs the decoding of name structure. The length of the encoded name is passed in uiEncNameLen
 * parameter. The decoded length is returned in puiReturnDecodedLen.
 ...
 * N/A
 */
PSELINKDLL SEC_NAME_S *X509_decodeName(SEC_UCHAR *pstDERCodeOfName, SEC_UINT32 uiEncNameLen,
    SEC_UINT32 *puiReturnDecodedLen);
 ...
 * @ingroup x509Functions
 * @image html icon-stop.png <b> This API is deprecated. Use the X509Extn_createSKID_ex  API instead. </b>
 * @par Prototype
 * @code
 ...
 * @par Description
 * This function generates the SubjectKeyIdentifier given the Public key and the mode in which the key identifier has to
 * be generated.  The mode can be either SHA1 160 bits or 64 bits. Returns hash value of the public key along with the
 * length. But the usage of this API is deprecated. Please refer the Note section for further details.
 ...
 * Input parameter is NULL, Encoding of the PublicKey fails, Hash operation on encoded public key fails or Memory
 * allocation fails [NULL|N/A]
 * @par Dependency
 * x509.h
 *
 * @par Memory Handling
 ...
 *
 */
PSELINKDLL SEC_AsnOcts_S *X509Extn_createSKID(SEC_PKEY_S *pstPublicKey, X509_KIDGENTYPE_E enKIDGenType);
 ...
 * @retval SEC_AsnOcts_S* Pointer to the string in the Attribute type. The output parameter puiStringType specifies the
 * string format used to represent the string [SEC_AsnOcts_S*|N/A]
 ...
 * N/A
 */
PSELINKDLL SEC_AsnOcts_S *X509_getAttrFromName(SEC_UINT32 uiAttrType, const SEC_NAME_S *pstName,
    SEC_UINT32 *puiStringType);
 ...
 * @par Prototype
 * @code
 * SEC_AsnOcts_S* X509_getDataFromAttr(const SEC_ATTR_TYPE_AND_VALUE_S* pstAttrTypeAndValue,
 * SEC_UINT32* puiStringType
 * )
 * @endcode
 * @par Purpose
 ...
 * N/A
 */
PSELINKDLL SEC_AsnOcts_S *X509_getDataFromAttr(const SEC_ATTR_TYPE_AND_VALUE_S *pstAttrTypeAndValue,
    SEC_UINT32 *puiStringType);
 ...
 * @par Prototype
 * @code
 * SEC_ATTR_TYPE_AND_VALUE_S* X509_getAttrFromNameByIndex(const SEC_NAME_S* pstName, SEC_UINT32 uiIndex)
 * @endcode
 * @par Purpose
 * To fetch the Attribute type & value structure from the given name structure based on the given index as input.
 * @par Description
 * Retrieves the AttributeTypeAndValue Structure from Name structure based on the index given as input. The index starts
 * from 0. The function returns the pointer to the AttributeTypeAndValue structure. This function can be used to for
 * comparing of name attributes during certificate verification.
 * @param[in] pstName Pointer to the Name structure. The name can be either the Subject name or the Issuer Name [N/A]
 * @param[in] uiIndex The index identifies the position from which AttributeTypeAndValue structure is to be retrieved
 * [N/A]
 * @param[out] N/A N/A [N/A]
 ...
 * N/A
 */
PSELINKDLL SEC_ATTR_TYPE_AND_VALUE_S *X509_getAttrFromNameByIndex(const SEC_NAME_S *pstName, SEC_UINT32 uiIndex);
 ...
 * @par Prototype
 * @code
 * SEC_UINT32  X509_deleteExtn (SEC_UINT32 uiExtnCID ,const X509_CERT_S *pstCert);
 * @endcode
 * @par Purpose
 * To remove an extension from the given certificate based on the extension CID.
 * @par Description
 * This function is used to delete the extension from the certificate.Extension are deleted based on extension CID which
 * is taken as input.
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509_deleteExtn(SEC_UINT32 uiExtnCID, const X509_CERT_S *pstCertificate);
 ...
 * @retval SEC_List_S* Input parameters are invalid, memory allocation fails or decoding of extension fails
 * [SEC_NULL|N/A]
 * @par Dependency
 * x509.h
 *
 ...
 * N/A
 */
PSELINKDLL SEC_List_S *X509_decodeExtnList(SEC_UCHAR *pucEncodedExtnList, SEC_UINT32 uiEncLen, SEC_UINT32 *puiDecLen);
 ...
 * @retval SEC_UCHAR* If the input parameters are invalid or encoding is unsuccessful [SEC_NULL|N/A]
 * @par Dependency
 * x509.h
 *
 * @par Memory Handling
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509_encodeCertList(SEC_List_S *pstCertList, SEC_UINT32 *puiEncodedLen);
 ...
 * @param[out] puiDecodedLength Bytes that is decoded [N/A]
 * @retval SEC_List_S* Pointer to the list containing extensions [SEC_List_S*|N/A]
 * @retval SEC_List_S* Input parameters are invalid, Memory allocation fails or decoding of certificate fails
 * [SEC_NULL|N/A]
 * @par Dependency
 * x509.h
 *
 ...
 * reference counting is initialized to 1 for each certificate in the returned linked list.
 *
 * @par Related Topics
 * N/A
 */
PSELINKDLL SEC_List_S *X509_decodeCertList(SEC_UCHAR *pucEncodedCertList, SEC_UINT32 uiEncodedLength,
    SEC_UINT32 *puiDecodedLength);
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509_encodeGeneralName(SEC_GEN_NAME_S *pstGenName, SEC_UINT32 *uiReturnEncodedLen);
 ...
 * @par Prototype
 * @code
 * SEC_GEN_NAME_S* X509_decodeGeneralName(SEC_CHAR* pcEncBuf,
 * SEC_UINT32 uiEncLen,
 * SEC_UINT32* puiDecLen)
 * @endcode
 * @par Purpose
 ...
 * @param[out] puiDecodedLength Bytes that is decoded [N/A]
 *
 * @retval SEC_GEN_NAME_S* The General Name structure after decoding [SEC_GEN_NAME_S*|N/A]
 * @retval SEC_GEN_NAME_S* On failure conditions SEC_NULL is returned. The failure conditions can be one of the
 * following:\n
 ...
 * N/A
 */
PSELINKDLL SEC_GEN_NAME_S *X509_decodeGeneralName(SEC_CHAR *pcEncBuf, SEC_UINT32 uiEncLen, SEC_UINT32 *puiDecLen);
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509_encodeAKID(X509_AKID_S *pstAkid, SEC_UINT32 *uiReturnEncodedLen);
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509_encodeIDP(X509_IDP_S *pstIdp, SEC_UINT32 *uiReturnEncodedLen);
 ...
 * @par Prototype
 * @code
 * SEC_INT X509_compareGeneralName( SEC_GEN_NAME_S *pstSrcGenName,
 ...
 * N/A
 */
PSELINKDLL SEC_INT X509_compareGeneralName(SEC_GEN_NAME_S *pstSrcGenName, SEC_GEN_NAME_S *pstDestGenName);
 ...
 * @par Prototype
 * @code
 * SEC_INT X509_compareAkid( X509_AKID_S *pstSrcAkid,
 ...
 * N/A
 */
PSELINKDLL SEC_INT X509_compareAkid(X509_AKID_S *pstSrcAkid, X509_AKID_S *pstDestAkid);
 ...
 * @par Prototype
 * @code
 * SEC_INT X509_compareIdp( X509_IDP_S *pstSrcIdp,
 ...
 * N/A
 */
PSELINKDLL SEC_INT X509_compareIdp(X509_IDP_S *pstSrcIdp, X509_IDP_S *pstDesIdp);
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509_encodeSN(SEC_BIGINT_S *pstSN, SEC_UINT32 *uiRetEncodedLen);
 ...
 * @par Prototype
 * @code
 * SEC_BIGINT_S*  X509_decodeSN(SEC_UCHAR* pstDERCodeOfName,
 ...
 * @retval SEC_BIGINT_S* Pointer to decoded Serial Number [SEC_BIGINT_S*|N/A]
 * @retval SEC_BIGINT_S* Returns when memory allocation or encode of name fails [SEC_NULL|N/A]
 * @par Dependency
 * x509.h
 *
 * @par Memory Handling
 ...
 * N/A
 */
PSELINKDLL SEC_BIGINT_S *X509_decodeSN(SEC_UCHAR *pstDERCodeOfSN, SEC_UINT32 uiEncSNLen, SEC_UINT32 *puiRetDecodedLen);
 ...
 * extension helps in finding the key pair. The keyIdentifier field of the extension must be included in all
 * certificates generated by confirming CAs to facilitate certification path construction. The key identifier generation
 * is either 160 bit SHA1 hash value or 64 bit value.
 * @param[in] pstCertSerialNum The unique serialNumber of the CA [N/A]
 * @param[in] pCertificateIssuer The Alternate Name of the Certificate Issuer [N/A]
 * @param[in] pstPublicKey The public key that has to be encoded and hash calculated on it using
 * the option passed in the enKIDType argument in the function [N/A]
 ...
 *
 */
PSELINKDLL X509_AKID_S *X509Extn_createAKID_ex(SEC_BIGINT_S *pstCertSerialNum, SEC_List_S *pCertificateIssuer,
    SEC_PKEY_S *pstPublicKey, X509_KIDGENTYPE_E enKIDType);
 ...
 *
 */
PSELINKDLL SEC_AsnOcts_S *X509Extn_createSKID_ex(SEC_PKEY_S *pstPublicKey, X509_KIDGENTYPE_E enKIDGenType);
 ...
 * To sign a x509 certificate with the given private key.
 * @par Description
 * This function is extended feature of X509_signCertificate. The functionality is same as X509_signCertificate but it
 * has some extra features supported. It has control flags as the input parameter which can define different behaviour
 * for this API. Currently SEC_EC_NO_TRUNCATION is the only option available to this parameter. This function signs the
 * certificate given the private key for signing. Before signing the certificate the algorithm Object Identifiers that
 * are set in the certificate is compared. The certificate extensions is verified to find whether there are multiple
 * existence of same extensions.Based on the certificate version set comaparison is made to check whether the
 * certificate consists of only the permitted fields corresponding to the certificate version.
 * The algorithm used to sign the certificate has to match the algorithm object identifier set in the certificate.
 * For CA certificate, if Digital signature bit is set in Key usage extension and basic constraint extension present
 * then the criticality of the Basic Constriant extension has to be set to TRUE by the application.  Since the
 ...
 * features supported. It has control flags as the input parameter which has define certain behaviour for this API.
 * Currently SEC_EC_NO_TRUNCATION is the only option available to this parameter. This function signs the CRL by a given
 * private key.The signature is actually calculated on the hash computed on tbsCRL structure. The calculated signature
 * value is then assigned to the signature field. Intially the given CRL is assumed to have a null signature value,if
 * present will be overwritten with a new value The CRL issuer has to use the private key to sign the CRL.It is also
 * necessary to make sure that the all the valid extensions are present, if any invalid extension is present then
 * signing will not be a performed. If an invalid version is present then too the CRL will not be signed. The hash
 * algorithm required for signing will be fetched internally from the algorithm identifier.Therefore it is necessary for
 * the user to set the appropriate algorithm identifier.
 *
 * @param[in] pstPrivateKey The private key that is used to sign the CRL [N/A]
 * @param[in] uiCtrlFlag Control flag is used to define the different operation. Currently only option available is
 * SEC_EC_NO_TRUNCATION. The control flag should be passed as 0 or SEC_EC_NO_TRUNCATION.
 ...
 *
 */
PSELINKDLL SEC_UINT32 X509CRL_sign_ex(SEC_PKEY_S *pstPrivateKey, X509_CRL_S *pstCRL, SEC_UINT32 uiCtrlFlag,
 ...
 * other products.
 * - This API is not multi-thread safe and should not be called in multiple threads. This API should be called at the
 * time of system initialization only once. Calling IPSI_AR_enableCheckPointSupport & IPSI_AR_disableCheckPointSupport
 ...
 * other products.
 * - This API is not multi-thread safe and should not be called in multiple threads. This API should be called at the
 * time once all operations are finished & no check point support is required. Calling IPSI_AR_enableCheckPointSupport &
 * IPSI_AR_disableCheckPointSupport dynamically is not supported. N/A
 *
 * @par Related Topics
 * N/A
 *
 ...
 * N/A
 */
PSELINKDLL SEC_AsnOcts_S *X509_getAttrFromNamePvtOID(SEC_AsnOid_S *pstPvtOID, SEC_NAME_S *pstName,
    SEC_UINT32 *puiStringType);
 ...
 *
 * @param[in] iEnable 1 is passed to enable the check and 0 to disable the check. Default value is 1. [0-1]
 *
 * @retval SEC_VOID This function does not return any value. [N/A|N/A]
 *
 * @par Required Header File
 ...
 * @par Note
 * \n
 * This API is used in the main thread. It should be called at the start of application as part of init.
 *
 */
PSELINKDLL SEC_VOID IPSI_X509_setCertAlgIdCheck(SEC_INT iEnable);

/**
 * @defgroup X509Extn_createAccessDesc
 * @ingroup x509Functions
 * @par Prototype
 * @code
 * X509_ACCESS_DESC_S*  X509Extn_createAccessDesc(SEC_UINT32 uiCID, SEC_GEN_NAME_S *pstGenName)
 * @endcode
 *
 * @par Purpose
 * This is used to create the access descrption strcuture pointer.
 *
 * @par Description
 * X509Extn_createAccessDesc is used to create the access descrption strcuture pointer using the general name and the
 * cid of the access description
 *
 * @param[in] uiCID CID of the access description. [CID_AD_CAISSUERS | CID_AD_OCSP | CID_AD_CAREPOSITORY
 * | CID_AD_TIMESTAMPING]
 * @param[in] pstGenName general name [N/A|N/A]
 *
 * @retval X509_ACCESS_DESC_S returns pointer of X509_ACCESS_DESC_S strcuture. [N/A|N/A]
 *
 * @par Required Header File
 * x509.h
 *
 * @par Memory Handling
 * X509_freeAccessDescription is used to free the memory allocated by API X509Extn_createAccessDesc
 * @par Note
 * \n
 *
 */
PSELINKDLL X509_ACCESS_DESC_S *X509Extn_createAccessDesc(SEC_UINT32 uiCID, SEC_GEN_NAME_S *pstGenName);

/**
 * @defgroup X509_freeAccessDescription
 * @ingroup x509Functions
 * @par Prototype
 * @code
 * SEC_VOID X509_freeAccessDescription(X509_ACCESS_DESC_S *pstAccessDesc)
 * @endcode
 *
 * @par Purpose
 * This is used to free the access description pointer.
 *
 * @par Description
 * X509_freeAccessDescription is used to free the X509_ACCESS_DESC_S pointer
 *
 * @param[in] pstAccessDesc access description strcuture pointer [N/A|N/A]
 *
 * @retval SEC_VOID This function does not return any value. [N/A|N/A]
 *
 * @par Required Header File
 ...
 *
 */
PSELINKDLL SEC_VOID X509_freeAccessDescription(X509_ACCESS_DESC_S *pstAccessDesc);

/**
 * @defgroup X509_createSkaeExtension
 * @ingroup x509Functions
 * @par Prototype
 * @code
 * X509_SKAE_EXT_S* X509_createSkaeExtension (SEC_INT specMajor, SEC_INT specMinor, SEC_CHAR* certifyInfo,
 * SEC_UINT32 uiCertInfoLen, SEC_CHAR* certifyInfoSignature, SEC_UINT32 uiSignLen, SEC_List_S * pstAccessDescription,
 * X509_CERT_S *pstAikCert)
 * @endcode
 *
 * @par Purpose
 * This is used create the SKAE extension structure.
 *
 * @par Description
 * X509_createSkaeExtension is used create the SKAE extension structure.
 *
 * @param[in] specMajor Major version of SKAE [N/A|N/A]
 * @param[in] specMinor Minor version of SKAE [N/A|N/A]
 * @param[in] certifyInfo certificate info [N/A|N/A]
 * @param[in] uiCertInfoLen length of the certificate info (certifyInfo) [N/A|N/A]
 * @param[in] certifyInfoSignature signature of the certificate info [N/A|N/A]
 * @param[in] uiSignLen length of the signature (certifyInfoSignature) [N/A|N/A]
 * @param[in] pstAccessDescription list of AccessDescription[N/A|N/A]
 * @param[in] pstAikCert aik certificate [N/A|N/A]
 * @retval X509_SKAE_EXT_S return the pointer of SKAE structure [N/A|N/A]
 *
 * @par Required Header File
 * x509.h
 *
 * @par Memory Handling
 * X509_freeSkaeExtension is used to free memory allocated by X509_createSkaeExtension
 * @par Note
 * uiCertInfoLen and uiSignLen input value can be maximum of 1MB.
 * \n
 *
 */
PSELINKDLL X509_SKAE_EXT_S *X509_createSkaeExtension(SEC_INT specMajor, SEC_INT specMinor, SEC_CHAR *certifyInfo,
    SEC_UINT32 uiCertInfoLen, SEC_CHAR *certifyInfoSignature, SEC_UINT32 uiSignLen, SEC_List_S *pstAccessDescription,
    X509_CERT_S *pstAikCert);

/**
 * @defgroup X509_freeSkaeExtension
 * @ingroup x509Functions
 * @par Prototype
 * @code
 * SEC_VOID X509_freeSkaeExtension(X509_SKAE_EXT_S *pstSkae)
 * @endcode
 *
 * @par Purpose
 * This is used to free the skae pointer.
 *
 * @par Description
 * X509_freeSkaeExtension is used to free the X509_SKAE_EXT_S pointer
 *
 * @param[in] pstSkae skae structure pointer [N/A|N/A]
 *
 * @retval SEC_VOID This function does not return any value. [N/A|N/A]
 *
 * @par Required Header File
 ...
 *
 */
PSELINKDLL SEC_VOID X509_freeSkaeExtension(X509_SKAE_EXT_S *pstSkae);

/**
 * @defgroup X509_encodeSignDataInCert
 * @ingroup x509Functions
 * @par Prototype
 * @code
 * SEC_UCHAR* X509_encodeSignDataInCert(X509_CERT_S *pstCertificate,
 * SEC_UINT32* puiEncodedLen, IPSI_AUX_PAR_S *pstAuxPar)
 * @endcode
 * @par Purpose
 * To encode sign data in cert
 * @par Description
 * The following method allows encoding of the sign data in cert
 * @param[in] pstCertificate cert [N/A]
 * @param[out] puiEncodedLen The encoded length [N/A]
 * @param[in] pstAuxPar reserved data [N/A]
 *
 * @retval SEC_UCHAR* Encoded Sign Data [SEC_UCHAR*|N/A]
 * @retval SEC_UCHAR* If the input is NULL [SEC_NULL|N/A]
 * @retval SEC_UCHAR* If memory allocation fails [SEC_NULL|N/A]
 *
 * @par Dependency
 ...
 *
 * @par Memory Handling
 * ipsi will allocate the memory for the encoded cert data to be signed, to free this memory
 * application has to call the ipsi_free function.
 *
 * @par Note
 *
 * @li Memory operation: alloc、free、size
 *    申请了puiEncodedLen指向长度的内存，该内存由调用者调用 ipsi_free 释放内存
 * @li Thread safe:
 *     线程安全函数
 * @li OS difference:
 *     无OS差异
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509_encodeSignDataInCert(X509_CERT_S *pstCertificate, SEC_UINT32* puiEncodedLen,
    IPSI_AUX_PAR_S *pstAuxPar);

/**
 * @defgroup X509_setCertSignature
 * @ingroup x509Functions
 * @par Prototype
 * @code
 * SEC_UINT32 X509_setCertSignature(X509_CERT_S *pstCertificate, SEC_UCHAR *pucSignature,
 * SEC_UINT32 uiSignatureLen, SEC_PKEY_S* pstPublicKey, IPSI_AUX_PAR_S *pstAuxPar)
 * @endcode
 * @par Purpose
 * To set the sign to a given x509 certificate structure.
 * @par Description
 * This function is used to set the sign data to cert.
 * pstCertificate also must be efficient.
 * function will use pstPublicKey to check pucSignature is right
 * @param[out] pstCertificate The certificate to which the sign has to be set [N/A]
 * @param[in] pucSignature The certificate sign to be set in the certificate [N/A]
 * @param[in] uiSignatureLen The signLen [N/A]
 * @param[in] uiCtrlFlag The ctrlFlag [N/A]
 * @param[in] pstPublicKey The pubkey to check the sign [N/A]
 * @param[in] pstAuxPar reserved data [N/A]
 *
 * @retval SEC_UINT32 When cert is set successfully [SEC_SUCCESS|N/A]
 * @retval SEC_UINT32 Error code when cert is set fail [N/A]
 * @par Dependency
 * x509.h
 * @par Note
 *
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509_setCertSignature(X509_CERT_S *pstCertificate, SEC_UCHAR *pucSignature,
    SEC_UINT32 uiSignatureLen, SEC_UINT32 uiCtrlFlag, SEC_PKEY_S *pstPublicKey, IPSI_AUX_PAR_S *pstAuxPar);

/**
 * @defgroup X509CRL_encodedSignDataInCRL
 * @ingroup x509Functions
 * @par Prototype
 * @code
 * SEC_UCHAR *X509CRL_encodedSignDataInCRL(X509_CRL_S *pstCRL, SEC_UINT32 *puiEncodedLen)
 * @endcode
 * @par Purpose
 * To encode sign data in x509Crl
 * @par Description
 * The following method allows encoding of the sign data in x509Crl
 * @param[in] pstCRL x509Crl [N/A]
 * @param[out] puiEncodedLen The encoded length [N/A]
 * @param[in] pstAuxPar reserved data [N/A]
 *
 * @retval SEC_UCHAR* Encoded Sign Data [SEC_UCHAR*|N/A]
 ...
 *
 * @par Memory Handling
 * ipsi will allocate the memory for the encoded x509Crl data to be signed, to free this memory
 * application has to call the ipsi_free function.
 *
 * @par Note
 *
 ...
 * N/A
 */
PSELINKDLL SEC_UCHAR *X509CRL_encodedSignDataInCRL(X509_CRL_S *pstCRL, SEC_UINT32 *puiEncodedLen,
    IPSI_AUX_PAR_S *pstAuxPar);

/**
 * @defgroup X509CRL_setSignature
 * @ingroup x509Functions
 * @par Prototype
 * @code
 * SEC_UINT32 X509CRL_setSignature(X509_CRL_S *pstCRL, SEC_UCHAR *pucSignature, SEC_UINT32 uiSignatureLen,
 * SEC_PKEY_S *pstPublicKey, IPSI_AUX_PAR_S *pstAuxPar)
 * @endcode
 * @par Purpose
 * To set the sign to a given x509 X509_CRL_S structure.
 * @par Description
 * This function is used to set the sign data to x509Crl.
 * pstCRL also must be efficient.
 * function will use pstPublicKey to check pucSignature is right
 * @param[out] pstCRL The x509Crl to which the sign has to be set [N/A]
 * @param[in] pucSignature The x509Crl sign to be set in the x509Crl [N/A]
 * @param[in] uiSignatureLen The signLen [N/A]
 * @param[in] pstPublicKey The pubkey to check the sign [N/A]
 * @param[in] pstAuxPar reserved data [N/A]
 *
 * @retval SEC_UINT32 When x509Crl is set successfully [SEC_SUCCESS|N/A]
 * @retval SEC_UINT32 Error code when x509Crl is set fail [N/A]
 * @par Dependency
 * x509.h
 * @par Note
 *
 ...
 * N/A
 */
PSELINKDLL SEC_UINT32 X509CRL_setSignature(X509_CRL_S *pstCRL, SEC_UCHAR *pucSignature, SEC_UINT32 uiSignatureLen,
    SEC_PKEY_S *pstPublicKey, IPSI_AUX_PAR_S *pstAuxPar);
