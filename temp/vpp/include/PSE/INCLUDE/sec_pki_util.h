...
************************************************************************** */
#ifndef SEC_PKI_UTIL_H
#define SEC_PKI_UTIL_H
...
typedef SEC_INT (*SEC_PKI_CertVerifyResultCB)(X509_CERT_S *pstIssuerCert, X509_CERT_S *pstSubjectCert,
    SEC_INT errorCode);
/**
 * @defgroup PKI
...
*
*/
PSELINKDLL SEC_INT SEC_PKI_getCertChainByCert(SEC_List_S *pCertList, X509_CERT_S *pstCert, SEC_List_S **dpOutCertChain);
 ...
 *
 * @par Description
 * SEC_PKI_getAllCertChainByAltName function returns the certificate(s) chain from
 ...
 * @param[in] ulLen Length of the Alt Name. Altname length should not be more than 0x100000 [NA/NA]
 ...
 * \n
 * 	- Only IPV4 address is supported for input alt name of type IP addr.
 * \n
 * 	- IP addr should be given in decimal dotted format.
 * \n
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_getAllCertChainByAltName(SEC_List_S *pstCertChainList, SEC_CHAR *pcAltName, SEC_UINT ulLen,
    SEC_PKI_X509_ALT_NAME_TYPE_E enId, SEC_List_S **dpOutCertChainList);
 ...
 *
 * @par Description
 * SEC_PKI_getAllCertChainByAlgId function returns the certificate(s) chain from
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_getAllCertChainByAlgId(SEC_List_S *pstCertChainList, SEC_PKI_X509_ALG_TYPE_E enAlgId,
    SEC_List_S **dpOutCertChainList);
 ...
 *
 * @par Description
 * SEC_PKI_getAllCertChainBySamePubKey function returns the certificate(s) chain
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_getAllCertChainBySamePubKey(SEC_List_S *pstCertChainList, SEC_List_S **dpOutCertChainList);

/**
* @defgroup SEC_PKI_VerifyCertFileTime
* @ingroup Utility_Functions
* @image html Security.jpg <b> User must take care the file access controls and
  must provide canonical path when using this API. </b>
* @par Prototype
* @code
...
* @param[in] iDifference In Seconds, can be positive or negative seconds.
* If iDifference is positive, it means that the overall valid period of certificate
* verification is advanced by iDifference seconds. If iDifference is negative,
* it means that the overall valid period of certificate verification is
* delayed by iDifference seconds.[NA/NA]
...
* effective Check time and the not after field (includes input
* iDifference seconds). [NA/NA]
* @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
* @retval SEC_INT On all failure conditions. [SEC_PKI_ERROR|NA]
*
* @par Note
...
* in Dopra.
*    - The Certifcate field "not before" and "not after" of the certificate must be less than 68 years from the current
system time.
*       Ex: Current system time - certificate field "not before" < 68 years
*             Certificate field "not after"  - current system time < 68 years
...
*/
PSELINKDLL SEC_INT SEC_PKI_VerifyCertFileTime(const SEC_CHAR *pcCertFile, SEC_INT iCertFileType, SEC_CHAR *pcCertPasswd,
    SEC_UINT uCertPwdLen, DATETIME_S *pstChecktime, SEC_TIME_T iDifference, SEC_List_S **pstVerifyResult);
...
* (pass "ipsi_free" as free function to SEC_LIST_FREE).
*/
PSELINKDLL SEC_INT SEC_PKI_VerifyCertBufTime(const SEC_CHAR *pcCertBuffer, SEC_UINT uCertBufLen, SEC_INT iCertFileType,
    SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen, DATETIME_S *pstChecktime, SEC_TIME_T iDifference,
    SEC_List_S **pstVerifyResult);
 ...
 */
PSELINKDLL SEC_INT SEC_PKI_VerifyCRLFileTime(const SEC_CHAR *pcCrlFile, SEC_INT iType, DATETIME_S *pstChecktime,
    SEC_TIME_T iDifference, SEC_List_S **pstVerifyResult);
 ...
 * then need to call API IPSI_setPemMaxSize.
 */
PSELINKDLL SEC_INT SEC_PKI_VerifyCRLBufTime(const SEC_CHAR *pcCRLBuffer, SEC_UINT uCertBufLen, SEC_INT iType,
    DATETIME_S *pstChecktime, SEC_TIME_T iDifference, SEC_List_S **pstVerifyResult);
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_enCodeCert(X509_CERT_S *pstCert, SEC_PKI_ENCODE_TYPE_E encType, SEC_CHAR **pcEncodedCert,
    SEC_UINT *puLen);
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_deCodeCert(SEC_CHAR *pcEncodedCert, SEC_UINT uiLen, SEC_PKI_ENCODE_TYPE_E encType,
    X509_CERT_S **pstCert);
 ...
 *
 * @par Description
 * SEC_PKI_getAllEndEntityCertFromList function returns all the end entity
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_getAllEndEntityCertFromList(SEC_List_S *pstInCertList, SEC_List_S **pstCertList);
 ...
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_isSelfSignedCertificate (const X509_CERT_S *pstCert);
 * @endcode
 *
 * @par Purpose
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_isSelfSignedCertificate(const X509_CERT_S *pstCert);
...
*/
PSELINKDLL SEC_INT SEC_PKI_extractCDPFromCert(X509_CERT_S *pstCert, SEC_List_S **pListOutDataBuf);
...
*/
PSELINKDLL SEC_INT SEC_PKI_CheckPrivateKeyFile(const SEC_CHAR *pcCertFile, SEC_INT iCertFileType,
    const SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen, const SEC_CHAR *pcKeyFile, SEC_INT iKeyFileType,
    const SEC_CHAR *pcKeyPasswd, SEC_UINT uKeyPwdLen, PEM_WARNING_CODE_E *peWarningCode);
...
*   - DRBG is enabled by default.
*/
PSELINKDLL SEC_INT SEC_PKI_CheckPrivateKeyBuffer(const SEC_CHAR *pcCertBuffer, SEC_UINT uCertBufLen, SEC_INT iCertType,
    const SEC_CHAR *pcCertPasswd, SEC_UINT uCertPwdLen, const SEC_CHAR *pcKeyBuffer, SEC_UINT uKeyBufLen,
    SEC_INT iKeyType, const SEC_CHAR *pcKeyPasswd, SEC_UINT uKeyPwdLen, PEM_WARNING_CODE_E *peWarningCode);
 ...
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_getAttrFromSubjectName(const X509_CERT_S *pstCert,
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_getAttrFromSubjectName(const X509_CERT_S *pstCert, X509_NAME_ATTR_TYPE_E uiAttrType,
    SEC_CHAR **pcName);
 ...
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_getAttrFromIssuerName(const X509_CERT_S *pstCert,
 * X509_NAME_ATTR_TYPE_E uiAttrType,
 * SEC_CHAR ** pcName)
 * @endcode
 * @par Purpose
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_getAttrFromIssuerName(const X509_CERT_S *pstCert, X509_NAME_ATTR_TYPE_E uiAttrType,
    SEC_CHAR **pcName);
...
* @par Prototype
* @code
* SEC_INT SEC_PKI_readCertFromFile(const SEC_CHAR *pcCertfile,
* SEC_INT iType,
...
*
*/
PSELINKDLL SEC_INT SEC_PKI_readCertFromFile(const SEC_CHAR *pcCertFile, SEC_INT iType, SEC_CHAR *pucPassword,
    SEC_UINT uCertPwdLen, SEC_List_S **pstCertList);
 ...
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_isCACert (const X509_CERT_S *pstCert);
 *
 * @endcode
 *
 * @par Purpose
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_isCACert(const X509_CERT_S *pstCert);
 ...
 *
 */
PSELINKDLL SEC_INT SEC_PKI_sortCertChain(SEC_List_S *pstCertChain, SEC_List_S **pstOutChain);

/**
* @defgroup SEC_PKI_getCRLInfo
* @ingroup Utility_Functions
* @par Prototype
* @code
* SEC_INT SEC_PKI_getCRLInfo (SEC_CHAR *pcCrlFile,SEC_BOOL bTimeValid, SEC_UINT *puiType,
*    X509_CRL_S** dpstCRL,SEC_PKI_VERIFY_TIME_RESULT_S *pstVerifyResult);
*
* @endcode
*
* @par Purpose
* This function is used to decode the CRL from file and validate CRL with current time if bTimeValid is SEC_TRUE.
*
* @par Description
* SEC_PKI_getCRLInfo function is used to decode the CRL from file and validate CRL with current time if bTimeValid is
* SEC_TRUE. This API returns success if decoding of CRL is passed. To check if Time validation of CRL is passed or not
* the application must check the output parameter pstVerifyResult->iErrorCode.
*
* @param[in] pcCrlFile Indicates the CRL file name. [NA/NA]
* @param[in] bTimeValid Flag to decide whether CRL need to validate with time or not. SEC_TRUE for validating CRL with
    time, SEC_FALSE for just decoding the CRL file.  [NA/NA]
* @param[out] puiType Type of CRL file SEC_PKI_FILETYPE_PEM or SEC_PKI_FILETYPE_ASN1 [NA/NA]
* @param[out] dpstCRL Indicates a double pointer to decoded CRL [NA/NA]
* @param[out] pstVerifyResult Indicates a pointer to SEC_PKI_VERIFY_TIME_RESULT_S [NA/NA]
*
* @retval SEC_INT on success  [SEC_PKI_SUCCESS|NA]
* @retval SEC_INT on failure conditions [SEC_PKI_ERROR|NA]
*
* @par Note
* \n
* 	- If decoding of CRL is passed, to free the decode CRL memory (dpstCRL), the application must call X509CRL_free.
* 	- It is recommended that CRL dates are verified if bTimeValid is SEC_TRUE.\n
* 	- PEM list of CRL is not suppoted.
* 	- If not found CRL file name, then error will be returned.
...
*
*/
PSELINKDLL SEC_INT SEC_PKI_getCRLInfo(SEC_CHAR *pcCrlFile, SEC_BOOL bTimeValid, SEC_UINT *puiType, X509_CRL_S **dpstCRL,
    SEC_PKI_VERIFY_TIME_RESULT_S *pstVerifyResult);

/**
* @defgroup SEC_PKI_getAllCertChainBySubName
* @ingroup Utility_Functions
* @par Prototype
* @code
* SEC_INT SEC_PKI_getAllCertChainBySubName (
*                                   SEC_List_S *pstCertChainList,
*                                   SEC_NAME_S *pstName,
*                                   SEC_List_S **dpOutCertChainList);
* @endcode
* @par Purpose:
* This function is used to get all matching certificate(s) chain from the input certificate(s) chain list by matching
subject name.

* @par Description:
* SEC_PKI_getAllCertChainBySubName function returns the certificate(s) chain from the pstCertChainList by matching
subject name.
* If there are multiple certs which are having same subject  name, then it will return the cert chain(s) corresponding
to the all the
* starting certs which are matching the subject name

* @param[in] pstCertChainList Input cert chain list. pstCertChainList is a list which holds elements of type SEC_List_S
which in turn
*   holds elements of type X509_CERT_S.[NA/NA]
* @param[in] pstName Subject name.[NA/NA]
* @param[out] dpOutCertChainList which contains the chain(s) for the matching alt name. dpOutCertChainList is a list
which holds elements of type SEC_List_S which in turn holds elements of type X509_CERT_S.[NA/NA]

* @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
* @retval SEC_INT On all failure conditions. [SEC_PKI_ERROR|NA]
*/
PSELINKDLL SEC_INT SEC_PKI_getAllCertChainBySubName(SEC_List_S *pstCertChainList, SEC_NAME_S *pstName,
    SEC_List_S **dpOutCertChainList);

/**
* @defgroup SEC_PKI_getAllCertChainByEncodedSubName
* @ingroup Utility_Functions
* @par Prototype
* @code
* SEC_INT SEC_PKI_getAllCertChainByEncodedSubName (
*                                    const SEC_List_S *pstCertChainList,
*                                    SEC_UCHAR  *pucEncodedBuff,
*                                    size_t uiEncodedLen,
*                                    SEC_List_S *const *dpOutCertChainList);
* @endcode
* @par Purpose:
* This function is used to get all matching certificate(s) chain from the input certificate(s) chain list by matching
...
* @par Description:
* SEC_PKI_getAllCertChainByEncodedSubName function returns the certificate(s) chain from the pstCertChainList by
matching encoded subject name.
* If there are multiple certs which are having same encoded subject  name, then it will return the cert chain(s)
corresponding to the all the starting certs
* which are matching the encoded subject name.

* @param[in] pstCertChainList Input cert chain list. pstCertChainList is a list which holds elements of type SEC_List_S
which in turn holds elements of type X509_CERT_S.[NA/NA]
* @param[in] pucEncodedId  encoded Subject name.[NA/NA]
* @param[in] uiEncodedLen Length of encoded Subject name.[NA/NA]
* @param[out] dpOutCertChainList which contains the chain(s) for the matching alt name. dpOutCertChainList is a list
which holds elements of type SEC_List_S which in turn holds elements of type X509_CERT_S.[NA/NA]
...
* @retval SEC_INT On all failure conditions. [SEC_PKI_ERROR|NA]
*/
PSELINKDLL SEC_INT SEC_PKI_getAllCertChainByEncodedSubName(SEC_List_S *pstCertChainList, SEC_UCHAR *pucEncodedBuff,
    SEC_UINT uiEncodedLen, SEC_List_S **dpOutCertChainList);

/**
* @defgroup SEC_PKI_getCertType
* @ingroup Utility_Functions
* @par Prototype
* @code
* SEC_INT SEC_PKI_getCertType(const X509_CERT_S *pstCert, SEC_INT32 *piCertType,
*                                                   SEC_UINT32 *puiCurveType)
*
* @endcode
*
* @par Purpose
* This function is used to get the certificate type from the certificate. .
*
* @par Description
* This function is used to get the certificate type from the certificate. If certificate type is ECDSA it will update
* the group id of curve.
*
...
*
* @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
* @retval SEC_INT On all failure conditions. If the input is invalid (NULL pointer). [SEC_PKI_ERROR|NA]
*
* @par Note
* \n
* NA.
*
*/
PSELINKDLL SEC_INT SEC_PKI_getCertType(const X509_CERT_S *pstCert, SEC_INT32 *piCertType, SEC_UINT32 *puiCurveType);

/**
 * @defgroup SEC_PKI_objVerifyCertExtern
 * @ingroup Utility_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_objVerifyCertExtern(const SEC_PKI_OBJ_S *pstObject,
 * SEC_PKI_CERT_TYPE_E enCertType,
 * SEC_UINT32 uiExternCID,
 * SEC_PKI_VERIFY_SCENARIO_E enVerifyScenario,
 * SEC_PKI_EXTERN_VERIFY_CALLBACK extnVerifyCb)
 *
 * @endcode
 *
 * @par Purpose
 * This function is used to verify the certificate field that specified by the caller. .
 *
 * @par Description
 * This function is used to verify the certificate field that specified by the caller. If the extnVerifyCb
 * is not SEC_NULL, it will verify the certificate by callback function first.
 *
 * @param[in] pstObject Pointer to PKI Object.[NA/NA]
 * @param[in] enCertType Type of local certificate which need to be checked.[NA/NA]
 * @param[in] uiExternCID The extension ID that need to be verified.[NA/NA]
 * @param[in] enVerifyScenario Check standards or scenarios, such as SEC_PKI_KEYUSAGE_WITH_ANY_DIGITAL_SIGN.[NA/NA]
 * @param[in] extnVerifyCb Caller-defined verification callback function.[NA/NA]
 * @param[out] NA NA [NA/NA]
 *
 * @retval SEC_INT On successful execution. [SEC_PKI_SUCCESS|NA]
 * @retval SEC_INT On all failure conditions. [SEC_PKI_ERROR|NA]
 ...
 * @par Note
 * \n
 * The error code reason is obtained through the SEC_PKI_ERR_REASON_CODE(SEC_PKI_getlast_error());
 *
 */
PSELINKDLL SEC_INT SEC_PKI_objVerifyCertExtern(const SEC_PKI_OBJ_S *pstObject, SEC_PKI_CERT_TYPE_E enCertType,
    SEC_UINT32 uiExternCID, SEC_PKI_VERIFY_SCENARIO_E enVerifyScenario, SEC_PKI_EXTERN_VERIFY_CALLBACK extnVerifyCb);

/**
 * Func Name: SEC_PKI_VerifyCertIntegrate
 * Description: Verify the certificate list Integrate
 * Input: SEC_List_S's element is  X509_CERT_S
 * pstCtxList - the certificates from context
 * pstAddList - the certificates from add, we only check the integrate of pstAddList.
 * verifyResultCB callback function for verification result, used to show whitch cert verify failed.
 * Return: SEC_PKI_SUCCESS on success
 * SEC_PKI_ERROR on failure
 */
PSELINKDLL SEC_INT SEC_PKI_VerifyCertIntegrate(SEC_List_S *pstCtxList, SEC_List_S *pstAddList,
    SEC_PKI_CertVerifyResultCB verifyResultCB);

/**
 * Func Name: SEC_PKI_CheckPathLen
 * Description: Check the pathlen of basicconstraints extension is valid or not
 * this api will check a certificate list, but when it find a certificate is not valid
 * it will return directly, other certificates won't check at all.
 * Input: SEC_List_S's element is  X509_CERT_S, usually is  certificate.
 * pstList - the certificate list
 * Return: SEC_PKI_SUCCESS on success
 * SEC_PKI_ERROR on failure, As long as there is one certificate in pstList check failed, then return  SEC_PKI_ERROR
 */
PSELINKDLL SEC_INT SEC_PKI_CheckPathLen(SEC_List_S *pstList);

/**
 * Func Name: SEC_PKI_ExtnisCritical
 * Description: Check the critical of extension is true or false
 * Input: pstCertificate - a certificate
 * uiExtnID - the id of extension
 * Return: SEC_TRUE on extension is critical.
 * SEC_FALSE on failure, the extension is not exist or the extenson is not critical.
 */
PSELINKDLL SEC_BOOL SEC_PKI_ExtnisCritical(const X509_CERT_S *pstCertificate, SEC_UINT32 uiExtnID);

/**
 * Func Name: SEC_PKI_getExtnKeyUsage
 * Description: get the extend key usage of certificate
 * Input: pstCertificate - a certificate
 * Return: extend key usage of certificate
 * if extend key usage is not exist or get extend key usage failed, then return 0.
 */
PSELINKDLL SEC_UINT SEC_PKI_getExtnKeyUsage(const X509_CERT_S *pstCertificate);

/**
 * @defgroup SEC_PKI_checkIssued
 * @ingroup Utility_Functions
 * @par Prototype
 * @code
 * SEC_INT SEC_PKI_checkIssued(const X509_CERT_S *subject, const X509_CERT_S *issuer)
 *
 * @endcode
 *
 * @par Purpose
 * check issuer of two cert
 *
 * @par Description
 * check issuer of two cert
 *
 * @param[in] subject a subject certificate.[NA/NA]
 * @param[in] issuer a issuer certificate.[NA/NA]
 *
 * @retval SEC_PKI_SUCCESS issuer issued subject.
 */
PSELINKDLL SEC_INT SEC_PKI_checkIssued(const X509_CERT_S *subject, const X509_CERT_S *issuer);
#ifdef __cplusplus
}
#endif
