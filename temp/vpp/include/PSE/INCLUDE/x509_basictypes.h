 ...
 * Description: x509 structures declaration
 * Author: VPP
 * Create: 2012/10/20
 */

#ifndef X509_BASICTYPES_H
#define X509_BASICTYPES_H
...
/**
 * @defgroup SEC_SHA_IDENTIFIER_S
 * @ingroup RSA_PSS_PARA_S and MGF1_PARA_S
 * @par Prototype
 * @datastruct algorithm SHA Algorithm OID.
 */
typedef struct stShaAlgorithmIdentifier {
    SEC_AsnOid_S algorithm; /* SHA Algorithm OID */
    SEC_AsnNull *nullParameter; /* NULL parameter */
} SEC_SHA_IDENTIFIER_S;

/**
 * @defgroup MGF1_PARA_S
 * @ingroup RSA_PSS_PARA_S
 * @par Prototype
 * @datastruct algorithm: MGF1 Algorithm OID.
 * @datastruct shaId: SHA Algorithm OID.
 */
typedef struct stMGF1_PARA {
    SEC_AsnOid_S mgf1;
    SEC_SHA_IDENTIFIER_S *shaId;
} MGF1_PARA_S;

/**
 * @defgroup RSA_PSS_PARA_S
 * @ingroup x509_basictypesStructures
 * @par Prototype
 * @datastruct hashAlgorithm: SHA Algorithm OID.
 * @datastruct maskGenAlgorithm: MGF1_PARA_S datastruct.
 * @datastruct saltLength: INTEGER DEFAULT 20.
 * @datastruct trailerField: INTEGER DEFAULT 1.
 */
typedef struct stPSS_PARA {
    SEC_SHA_IDENTIFIER_S *hashAlgorithm;
    MGF1_PARA_S *maskGenAlgorithm;
    SEC_AsnInt *saltLength;
    SEC_AsnInt *trailerField;
} RSA_PSS_PARA_S;
...
/**
 * @defgroup SEC_CRL_TIME_S
 * @ingroup x509_basictypesStructures
 * @par Prototype
 * @code
 * typedef struct stCRLTime
 * {
 * enum CrlTimeChoiceId
 * {
 * TIME_CRL_UTCTIME,
 * TIME_CRL_GENERALIZEDTIME
 * } choiceId;
 * union CrlTimeChoiceUnion
 * {
 * SEC_CRL_UTCTime_S utcTime;
 * SEC_CRL_GeneralizedTime_S generalizedTime;
 * } a;
 * } SEC_CRL_TIME_S;
 * @endcode
 *
 * @datastruct choiceId Formats in which the time will be represented.
 * @datastruct a Union that can hold either the UTC Time or Generalized Time.
 * @datastruct TIME_CRL_UTCTIME UTC time format.
 * @datastruct TIME_CRL_GENERALIZEDTIME Genaralized time format.
 * @datastruct utcTime UTC time.
 * @datastruct generalizedTime Generalized time.
 */
enum CrlTimeChoiceId {
    TIME_CRL_UTCTIME,        /* UTC time format */
    TIME_CRL_GENERALIZEDTIME /* Genaralized time format */
};

typedef struct stCRLTime { /* CHOICE */
    /* Formats in which the time will be represented */
    enum CrlTimeChoiceId choiceId;

    /* Union that can hold either the UTC Time or Generalized Time */
    union CrlTimeChoiceUnion {
        SEC_CRL_UTCTime_S utcTime;                 /* UTC time */
        SEC_CRL_GeneralizedTime_S generalizedTime; /* Genaralized time */
    } a;
} SEC_CRL_TIME_S;
...
/**
* @defgroup SEC_BIGINT_CRL_S
* @ingroup x509_basictypesStructures
* @par Prototype
* @code
* typedef struct stBIGINTCRL
* {
*  SEC_UINT32 uiLength
*  SEC_INT8   *aVa
* }SEC_BIGINT_CRL_S
...
* @datastruct aVal Holds the value of big integer with MSB @ aVal[0].
*/
typedef struct stBIGINTCRL {
...
* {
* #ifdef REDUCE_CRL_MEMORY
*    SEC_BIGINT_CRL_S userCertificate;
* #else
...
* @par Note
* \n
* 1. Users should not define macro "REDUCE_CRL_MEMORY" directly in their makefile as this macro will be used
*     in libraries only for specific feature. This macro is only to be defined by HERT. If used by any other user, can
...
typedef struct stX509Revoked {
    ...
    SEC_BIGINT_CRL_S userCertificate;
#else
    SEC_BIGINT_S userCertificate; /* The userCertificate, revocationDate are mandatory fields that needs to be present
                                     always */
#endif

#ifdef REDUCE_CRL_MEMORY
    SEC_CRL_TIME_S revocationDate;
#else
...
/* The structure contains the certificate encoded buffer and length of the buffer. */
/**
 * @defgroup X509_CertInfo_ENCODING_S
 * @ingroup x509_basictypesStructures
 * @par Prototype
 * @code
 * typedef struct stCertInfo_ENCODING
 * {
 * SEC_UCHAR *pucenc
 * SEC_UINT32 uiLen
 * } X509_CertInfo_ENCODING_S
 * @endcode
 *
 * @datastruct pucenc is used to store the certificate info encoded buffer.
 * @datastruct uiLen Length of encoded buffer.
 */
typedef struct stCertInfo_ENCODING {
    SEC_UCHAR *pucenc; /* DER encoding */
    SEC_UINT32 uiLen;  /* Length of encoding */
} X509_CertInfo_ENCODING_S;
 ...
 * 	SEC_INT iReferences
 * #ifdef __IPSI_STORE_CERT_INFO_ENCODEBUF__
 * X509_CertInfo_ENCODING_S stEncBuf
 * #endif
 * } X509_CERT_S
 * @endcode
 *
 ...
 * @datastruct stEncBuf The structure contains encoded certificate info buffer
 */
typedef struct stCertificate {
...
#ifdef REDUCE_CRL_MEMORY
/* The structure contains the certificate buffer and length of the buffer. */
/**
 * @defgroup X509_BUFF_S
 * @ingroup stCrlBuff
 * @par Prototype
 * @code
 * typedef struct stCrlBuff
 * {
 * SEC_UCHAR *pData
 * SEC_UINT32 uiLen
 * } SEC_BUFF_S
 * @endcode
 *
 * @datastruct pData is used to store the certificate buffer.
 * @datastruct uiLen Length of certificate buffer.
 */
typedef struct stCrlBuff {
    SEC_UCHAR *pBuf;
    SEC_UINT32 uiLen;
...
#ifdef REDUCE_CRL_MEMORY
    SEC_BUFF_S rawCrl; /* crl certificate buffer */

    SEC_BUFF_S crlTbs; /* tbs buffer, this buffer no need malloc,just used the raw buffer */
#endif
} X509_CRL_S;
