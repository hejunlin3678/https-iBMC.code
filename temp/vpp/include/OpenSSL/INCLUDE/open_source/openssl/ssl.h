...
# define SSL_OP_COMPATIBLE_GMSSL_CLIENT_MODE             SSL_OP_BIT(32)
# define SSL_OP_NO_GMTLS                                 SSL_OP_BIT(33)
# define SSL_OP_NO_SSL_MASK (SSL_OP_NO_GMTLS | SSL_OP_NO_SSLv3 \
...
# define SSL_CTRL_SET_EC_POINT_FORMATS           112
...
# define SSL_CTRL_GET_ENCRYPT_THEN_MAC_TYPE      139
...
# define SSL_set1_ec_point_formats(s, plst, plstlen) \
        SSL_ctrl(s,SSL_CTRL_SET_EC_POINT_FORMATS, plstlen, plst)
# define SSL_CTX_set1_ec_point_formats(ctx, plst, plstlen) \
        SSL_CTX_ctrl(ctx, SSL_CTRL_SET_EC_POINT_FORMATS, plstlen, plst)
...
# define SSL_get_encrypt_then_mac_type(s) \
        SSL_ctrl(s, SSL_CTRL_GET_ENCRYPT_THEN_MAC_TYPE, 0, NULL)
...
__owur X509_STORE *SSL_CTX_get_cert_chain_store(const SSL_CTX *);
__owur X509_STORE *SSL_CTX_get1_chain_cert_store(const SSL_CTX *);
__owur X509_STORE *SSL_CTX_get_verify_cert_store(const SSL_CTX *);
...
__owur int SSL_update_cipher_sk_list(SSL *s, STACK_OF(SSL_CIPHER) *sk);
...
/* if open this flag, when encode a session, the output buf won't contain the peer cert */
#define SSL_SESSION_ASN1_NO_PEER_CERT   0x1
 
__owur int i2d_SSL_SESSION_ex(const SSL_SESSION *in, unsigned char **pp, int op);
...
#ifndef OPENSSL_NO_GMTLS
__owur int GMTLS_CTX_check_enc_private_key(const SSL_CTX *ctx);
__owur int GMTLS_check_enc_private_key(const SSL *ssl);
#endif
...
__owur int SSL_CTX_remove_cert_and_key(SSL_CTX *ctx);
...
/* SSL client will reset the client hello record version by default,
 * Some low version SSL server cannot accept after reset the client record layer version.
 * Allows SSL client application to decide whether to reset the client record layer version.
 */
void SSL_enable_record_version_compat(SSL *s, int enable);
#endif
