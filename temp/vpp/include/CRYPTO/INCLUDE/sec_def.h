...
* @image html Security.jpg <b> User must take care the file access controls and must provide canonical path when using
this API. </b>
* @par Prototype
* @code
...
#define SEC_VFS_FRead(buffer, charsize, size, stream) ipsi_fread(buffer, (size_t)((charsize) * (size)), stream)
#define SEC_VFS_FWrite(buffer, charsize, size, stream) ipsi_fwrite(buffer, (size_t)((charsize) * (size)), stream)
...
*
* @par Description
* SEC_GetSysTime function returns system time and stores the same in pstSysTime.

* @param[in] pstSysTime Pointer to DATETIME_S structure [N/A]
*
* @retval SEC_UINT32 If pstSysTime is NULL [SEC_ERR_INVALID_ARG|N/A]
* @retval SEC_UINT32 If returns system time [SEC_SUCCESS|N/A]
*
* @par Required Header File
* sec_def.h
*
