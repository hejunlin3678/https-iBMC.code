...
#ifndef SEC_LIST_EX_H
#define SEC_LIST_EX_H
 ...
 * @par Prototype
 * @code
 * SEC_ListNode_S *SEC_LIST_firstNode ( const SEC_List_S *list);
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_ListNode_S *SEC_LIST_firstNode(const SEC_List_S *list);
 ...
 * @par Prototype
 * @code
 * SEC_VOID *SEC_LIST_getData ( const SEC_ListNode_S *pstNode);
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_VOID *SEC_LIST_getData(const SEC_ListNode_S *pstNode);
 ...
 * @par Prototype
 * @code
 * SEC_VOID *SEC_LIST_getNextNode ( const SEC_List_S *pstList,
 * const SEC_ListNode_S *pstListNode);
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_ListNode_S *SEC_LIST_getNextNode(const SEC_List_S *pstList, const SEC_ListNode_S *pstListNode);
 ...
 * @par Prototype
 * @code
 * SEC_VOID *SEC_LIST_getPrevNode ( const SEC_ListNode_S *pstListNode);
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_ListNode_S *SEC_LIST_getPrevNode(const SEC_ListNode_S *pstListNode);
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_VOID *SEC_LIST_getIndexNode_ex(SEC_UINT32 ulIndex, const SEC_ListNode_S *pstListNode,
    const SEC_List_S *pstList);
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_VOID SEC_LIST_deleteNode(SEC_List_S *pstList, const SEC_ListNode_S *pstListNode,
    PFUNC_FREE pfFreeFunc);
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_VOID SEC_LIST_detachNode(SEC_List_S *pstList, SEC_ListNode_S **pstListNode);
 ...
 * @par Prototype
 * @code
 * SEC_List_S* SEC_listCopy_Ex(const SEC_List_S* pSrcList,
 * PFUNC_DUP pFuncCpy,
 * PFUNC_FREE pfFreeFunc)
 * @endcode
 *
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_List_S *SEC_listCopy_Ex(const SEC_List_S *pSrcList, PFUNC_DUP pFuncCpy, PFUNC_FREE pfFreeFunc);

/* Declares and initializes the current reference node in case of single list */
#define SEC_LIST_ITERATOR SEC_ListNode_S *pstRefNode = SEC_NULL;

/* returns the first element in case of single list */
#define SEC_LIST_GET_FIRST_EX(pList)                                                                            \
    ((pList) ? (((pstRefNode = SEC_LIST_firstNode(pList)) != NULL) ? SEC_LIST_getData(pstRefNode) : SEC_NULL) : \
               (SEC_NULL))

/* returns the next element in case of single list */
#define SEC_LIST_GET_NEXT_EX(pList)                                                                                    \
    ((pList) ?                                                                                                         \
        (((pstRefNode = SEC_LIST_getNextNode(pList, pstRefNode)) != NULL) ? SEC_LIST_getData(pstRefNode) : SEC_NULL) : \
        (SEC_NULL))

/* Deletes the current element in case of single list */
#define SEC_LIST_DELETE_CURRENT_EX(pList, pFreeFunc) SEC_LIST_deleteNode(pList, pstRefNode, pFreeFunc)

/* Detaches the current element in case of single list */
#define SEC_LIST_DETACH_CURRENT_EX(pList) SEC_LIST_detachNode(pList, &(pstRefNode))

/* Gives the current element in case of single list */
#define SEC_LIST_GET_CURRENT_EX(pList) ((pList) ? (SEC_LIST_getData(pstRefNode)) : (SEC_NULL))

/* Declares and initializes the current reference node in case of multiple
 list */
#define SEC_LIST_VAR_ITERATOR(pList) SEC_ListNode_S *pstRefNode##_##pList = SEC_NULL

/* returns the first element in case of multiple list */
#define SEC_LIST_VAR_GET_FIRST_EX(pList)                                                                         \
    ((pList) ?                                                                                                   \
        (((pstRefNode##_##pList = SEC_LIST_firstNode(pList)) != NULL) ? SEC_LIST_getData(pstRefNode##_##pList) : \
                                                                        SEC_NULL) :                              \
        (SEC_NULL))

/* returns the next element in case of multiple list */
#define SEC_LIST_VAR_GET_NEXT_EX(pList)                                                                \
    ((pList) ? (((pstRefNode##_##pList = SEC_LIST_getNextNode(pList, pstRefNode##_##pList)) != NULL) ? \
        SEC_LIST_getData(pstRefNode##_##pList) :                                                       \
        SEC_NULL) :                                                                                    \
               (SEC_NULL))

/* Deletes the current element in case of multiple list */
#define SEC_LIST_VAR_DELETE_CURRENT_EX(pList, pFreeFunc) SEC_LIST_deleteNode(pList, pstRefNode##_##pList, pFreeFunc)

/* Detaches the current element in case of multiple list */
#define SEC_LIST_VAR_DETACH_CURRENT_EX(pList) SEC_LIST_detachNode(pList, &(pstRefNode##_##pList))

/* Gives the current element in case of multiple list */
#define SEC_LIST_VAR_GET_CURRENT_EX(pList) ((pList) ? (SEC_LIST_getData(pstRefNode##_##pList)) : (SEC_NULL))

/* checks if current node is the last one in case of multiple list */
#define SEC_LIST_VAR_IS_END_EX(pList) ((pList) ? (pstRefNode##_##pList == SEC_NULL) : 0)

/* Get the index of the element in the list in case of single list */
#define SEC_LIST_GET_ELEMENT_EX(ulIndex, pList) SEC_LIST_getIndexNode_ex(ulIndex, pstRefNode, pList)

/* Get the index of the element in the list in case of single list */
#define SEC_LIST_VAR_GET_ELEMENT_EX(ulIndex, pList) SEC_LIST_getIndexNode_ex(ulIndex, pstRefNode##_##pList, pList)

/* checks if current node is the last one in case of single list */
#define SEC_LIST_IS_END_EX(pList) ((pList) ? (pstRefNode == SEC_NULL) : 0)
