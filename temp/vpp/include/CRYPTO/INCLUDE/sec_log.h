...
/* ipsi_LogCb - type of the callback to be provided by application
  to handle log event. */
typedef SEC_INT (*SEC_LOG_CB)(int level, const char *file, int line, const char *msg);
 ...
 * argument to the log macro. The macros are one of INFO, DBG, ERR, FATAL. It is upto these macros to set
 ...
 * Application should take care that API is success in the callback function. If the API is failed, log file would be
 * NULL and new logs would not be logged.
 *
 * @par Related Topics
 * SEC_getLogLevel,\n
 * SEC_setLogLevel
 */
CRYPTOLINKDLL SEC_INT SEC_log(SEC_LOG_LEVEL level, const char *file, int line,
    const SEC_CHAR *fmt, ...) IPSIFORMAT(printf, 4, 5); /* 4 fmt位置 5 可变参位置 */
 ...
 * @par Note
 * \n
 * SEC_logBuf function will be called when application will log huge string logging message such as certificate.
 * \n
 * If application set maximum size for logging & current log file exceeds maximum limit, it is expected that
 * application should backup the existing file & create new file using SEC_setLogFile.
 * Application should take care that API is success in the callback function. If the API is failed, log file would be
 ...
 * SEC_logBuf
 */
CRYPTOLINKDLL SEC_INT SEC_logBuf(SEC_LOG_LEVEL level, const char *file, int line, const SEC_CHAR *buf,
    unsigned int len);
 ...
 * N/A
 *
 * @retval SEC_LOG_LEVEL The current log level
 * [The log levels can be one of these:SEC_LOG_INFO, SEC_LOG_DEBUG, SEC_LOG_ERR, SEC_LOG_FATAL,
 * SEC_LOG_WARNING, SEC_LOG_TRACE, SEC_LOG_DISABLED, SEC_LOG_ALL|N/A]
 *
 * @par Required Header File
 * sec_log.h
 *
 ...
 * <b> User must take care the file access controls and must provide canonical path when using this API. </b>
 * @par Prototype
 * @code
 ...
 * SEC_setLogSizeHandler
 */
CRYPTOLINKDLL SEC_INT SEC_setLogFile_MTSafe(const char *file, SEC_LOG_LEVEL eLevel);
 ...
 *
 * @par Description
 * SEC_getLogFile function returns the current log file set.
 *
 * @par Parameters
 * N/A
 *
 ...
 *
 * @par Description
 * SEC_setLogMaxSize function sets the maximum bytes to be logged to log device.Application can set
 * max 1 GB(1073741824 bytes) as max log file size. If application try to set more than 1 GB ,
 * library will set 1GB as max log file size.
 *
 ...
 * \n
 * - This API should not be called in any of the log callbacks.
 * - Default max log file size is 50 MB (52428800 bytes)
 *
 * @par Related Topics
 * SEC_setLogFile \n
 * SEC_getLogMaxSize
 ...
 *
 * @par Description
 * SEC_getLogMaxSize function returns the size limit for log device.
 *
 * @par Parameters
 * N/A
 *
 ...
 *
 * @par Note
 * - It is recommned not to set small value as max log file size
 * NA
 *
 * @par Related Topics
 * SEC_setLogFile,\n
 ...
 * @par Note
 * \n
 * In the pFunc calback function, user need to set the new file name which will set the file pointer in the log module
 * using API SEC_setLogFile.
 * If the same file name is provided which is currently logging the log, then the existing log file will append. It will
 * be the same as not doing anything in the callback.
 * Return value of the callback is neglected.
 *
 * @par Related Topics
 * SEC_setLogFile \n
 * SEC_getLogMaxSize.
 ...
 *
 * @par Description
 * SEC_setLogCallback function sets the handler for logging. It is used to override the default logging to file
 * mechanism.
 *
 ...
 * threadsafe. when user uses the system in mutil-tread environment, they should call this API to initialize the
 * loglevel lock.
 *
 * @par Parameters
 * N/A
