/**
 * ****************************************************************************
 * Copyright (c) Huawei Technologies Co., Ltd. 2005-2021. All rights reserved.
 * ALL RIGHTS RESERVED
 * ------------------------------------------------------------------------------
 * sec_list.h
 * Project Code: SeCert
 ...
 */
extern SEC_INT32 g_i32MaxListCount;
...
typedef struct stSEC_List {
    ...
    int dataSize;          /* Memory needed for each node data */
} SEC_List_S;
 ...
 * @endcode
 *
 * @datastruct LIST_POS_BEFORE Indication to to add the element before the current element.
 * @datastruct LIST_POS_AFTER Indication to to add the element after the current element.
 * @datastruct LIST_POS_BEGIN Indication to to add the element at the beginning of the list.
 * @datastruct LIST_POS_END Indication to to add the element at the end of the list.
 */
typedef enum enSEC_Position {
    ...
    LIST_POS_END     /* Indication to to add the element at the end of the list */
} SEC_Position_E;
...
#define SEC_LIST_CURR_ELMT(pList) ((pList) ? ((pList)->curr ? ((pList)->curr->data) : SEC_NULL) : SEC_NULL)
...
#define SEC_LIST_LAST_ELMT(pList) ((pList) ? ((pList)->last ? ((pList)->last->data) : SEC_NULL) : SEC_NULL)

/* returns the first element */
#define SEC_LIST_FIRST_ELMT(pList) ((pList) ? ((pList)->first ? ((pList)->first->data) : SEC_NULL) : SEC_NULL)

/* checks if the list is empty */
#define SEC_LIST_EMPTY(pList) (((pList) != SEC_NULL) ? ((pList)->count == 0) : 0)

/* returns the number of nodes in the list */
#define SEC_LIST_COUNT(pList) ((pList) ? ((pList)->count) : 0)

/* checks if current node is the last one */
#define SEC_LIST_IS_END(pList) ((pList) ? (SEC_NULL == (pList)->curr) : 0)

/* checks if current node is the first one */
#define SEC_LIST_IS_START(pList) ((pList) ? ((pList)->first == (pList)->curr) : 0)
...
/* create a new list and initialize it */
#define SEC_LIST_NEW(ulSizeMemData) SEC_LIST_new(ulSizeMemData)

/* Get the current element */
#define SEC_LIST_GET_CURRENT(pList) ((pList) ? (SEC_LIST_curr(pList) ? SEC_LIST_CURR_ELMT(pList) : SEC_NULL) : SEC_NULL)

/* Get the next element */
#define SEC_LIST_GET_NEXT(pList) ((pList) ? (SEC_LIST_next(pList) ? SEC_LIST_CURR_ELMT(pList) : SEC_NULL) : SEC_NULL)

/* Get the previous element */
#define SEC_LIST_GET_PREV(pList) ((pList) ? (SEC_LIST_prev(pList) ? SEC_LIST_CURR_ELMT(pList) : SEC_NULL) : SEC_NULL)

/* Get the first element */
#define SEC_LIST_GET_FIRST(pList) ((pList) ? (SEC_LIST_first(pList) ? SEC_LIST_CURR_ELMT(pList) : SEC_NULL) : SEC_NULL)

/* Get the last element */
#define SEC_LIST_GET_LAST(pList) ((pList) ? (SEC_LIST_last(pList) ? SEC_LIST_CURR_ELMT(pList) : SEC_NULL) : SEC_NULL)

/* Get the index of the element in the list  */
#define SEC_LIST_GET_ELEMENT(ulIndex, pList) SEC_LIST_getIndexNode((ulIndex), (pList))
...
#define SEC_LIST_SORT(pList, pfCmp) SEC_LIST_sort(pList, pfCmp)

/* Add an element before/after  the current node */
#define SEC_LIST_ADD_ELEMENT(pList, pData, enPosition) SEC_LIST_addElement(pList, pData, enPosition)

/* Add an element before/after  the current node without checking the data pointer */
#define SEC_LIST_ADD_ELEMENT_EX(pList, pData, enPosition) SEC_LIST_addElement_ex(pList, pData, enPosition)
...
#define SEC_LIST_DELETE_ALL(pList, pFreeFunc) SEC_LIST_deleteAll((pList), pFreeFunc)

/* Delete the current element of list.
 After deletion curr becomes next of the deleted node
 */
#define SEC_LIST_DELETE_CURRENT(pList, pFreeFunc) SEC_LIST_deleteCurrent((pList), pFreeFunc)
...
#define SEC_LIST_SEARCH(pList, pSearchFor, pSearcher) SEC_LIST_search((pList), pSearchFor, pSearcher)
...
#define SEC_LIST_COPY(pSrcList, pFuncDup, pFuncFree) SEC_listCopy(pSrcList, pFuncDup, pFuncFree)

/* to create a copy of a list.  It returns the pointer of the new list(copy of the original pSrcList) */
#define SEC_LIST_COPY_EX(pSrcList, pFuncDup, pFuncFree) SEC_listCopy_Ex(pSrcList, pFuncDup, pFuncFree)
 ...
 */
/**
 * @defgroup SEC_LIST_setMaxElements
 * @ingroup sec_listFunctions
 * @par Prototype
 * @code
 * SEC_INT SEC_LIST_setMaxElements(SEC_INT32 iMaxElements)
 * @endcode
 *
 * @par Purpose
 * Set Max Element in SEC_LIST.
 *
 * @par Description
 * This function sets the max element in SEC_LIST.Default value is 10000000 (10 Million).
 *
 * @param[in] iMaxElements Max allowed element in SEC_LIST. It should be in range[0xffff, 0xfffffff] [N/A]
 * @param[out] N/A N/A [N/A]
 *
 * @retval SEC_INT If input falls outside the range [SEC_ERROR|N/A]
 * @retval SEC_INT If successful [SEC_SUCCESS|N/A]
 *
 * @par Dependency
 * sec_list.h
 *
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_INT SEC_LIST_setMaxElements(SEC_INT32 iMaxElements);

/**
 * @defgroup SEC_LIST_getMaxElements
 * @ingroup sec_listFunctions
 * @par Prototype
 * @code
 * SEC_INT32 SEC_LIST_getMaxElements(SEC_VOID)
 * @endcode
 *
 * @par Purpose
 * Returns the Max allowed elements in SEC_LIST.
 *
 * @par Description
 * This function returns the max allowed elements in SEC_LIST.
 *
 * @param[in] N/A N/A [N/A]
 * @param[out] N/A N/A [N/A]
 *
 * @retval SEC_INT32 Max configured elements in SEC_LIST
 *
 * @par Dependency
 * sec_list.h
 *
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_INT32 SEC_LIST_getMaxElements(SEC_VOID);

/**
 * @defgroup SEC_LIST_addElement
 * @ingroup sec_listFunctions
 * @par Prototype
 * @code
 * SEC_UINT32 SEC_LIST_addElement (SEC_List_S *pList, void *pData,
 * SEC_Position_E enPosition );
 * @endcode
 *
 * @par Purpose
 ...
 * empty, the node will be added as the only node.The current pointer is changed to point to the newly added node in the
 * list. If the current pointer is SEC_NULL then this operation fails.
 *
 ...
 * @param[out] N/A N/A [N/A]
 *
 * @retval SEC_UINT32 The error code [SEC_UINT32|N/A]
 * @retval SEC_UINT32 If current pointer is SEC_NULL and list is not empty [SEC_ERR_LIST_CURRENT_NULL|N/A]
 * @retval SEC_UINT32 If data pointer is SEC_NULL [SEC_ERR_DATA_NOT_AVAILABLE|N/A]
 * @retval SEC_UINT32 If failure to allocate memory for new node [SEC_ERR_MALLOC_FAIL|N/A]
 * @retval SEC_UINT32 If successful [SEC_SUCCESS|N/A]
 *
 * @par Dependency
 * sec_list.h
 *
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_UINT32 SEC_LIST_addElement(SEC_List_S *pList, void *pData, SEC_Position_E enPosition);

/**
 * @defgroup SEC_LIST_addElement_ex
 * @ingroup sec_listFunctions
 * @par Prototype
 * @code
 * SEC_UINT32 SEC_LIST_addElement_ex (SEC_List_S *pList, void *pData,
 * SEC_Position_E enPosition );
 * @endcode
 *
 * @par Purpose
 ...
 * This function creates a new node before, after or at the begining or end of the current node. If the list was already
 * empty, the node will be added as the only node.The current pointer is changed to point to the newly added node in the
 * list. If the current pointer is SEC_NULL then this operation fails. The data pointer is not checked, so if it is null
 * then also it will get added to list
 *
 * @param[in] pList The list [N/A]
 * @param[in] pData The element to be added [N/A]
 * @param[in] enPosition Whether the elemnt is to be added before or after the list [N/A]
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_UINT32 SEC_LIST_addElement_ex(SEC_List_S *pList, void *pData, SEC_Position_E enPosition);
 ...
 * @par Prototype
 * @code
 * void SEC_LIST_deleteAll (SEC_List_S *pList,
 ...
 * N/A
 */
CRYPTOLINKDLL void SEC_LIST_deleteAll(SEC_List_S *pList, PFUNC_FREE pfFreeFunc);
 ...
 * @par Prototype
 * @code
 * void SEC_LIST_deleteCurrent (SEC_List_S *pList, PFUNC_FREE pfFreeFunc );
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL void SEC_LIST_deleteCurrent(SEC_List_S *pList, PFUNC_FREE pfFreeFunc);
 ...
 * @par Prototype
 * @code
 * void SEC_LIST_detachCurrent (SEC_List_S *pList);
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL void SEC_LIST_detachCurrent(SEC_List_S *pList);
 ...
 * @par Prototype
 * @code
 * void* SEC_LIST_search (SEC_List_S *pList, void *pSearchFor, PFUNC_CMP pSearcher );
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL void *SEC_LIST_search(SEC_List_S *pList, const void *pSearchFor, PFUNC_CMP pSearcher);
 ...
 * @par Prototype
 * @code
 * void* SEC_LIST_search_Ex (SEC_List_S *pList,void *pSearchFor,PFUNC_CMP pSearcher,SEC_INT * pstErr)
 * @endcode
 *
 * @par Purpose
 * To search a list based on the comparator function supplied (3rd param).
 ...
 * N/A
 */
CRYPTOLINKDLL void *SEC_LIST_search_Ex(SEC_List_S *pList, const void *pSearchFor, PFUNC_CMP pSearcher, SEC_INT *pstErr);
 ...
 * @par Prototype
 * @code
 * void* SEC_LIST_getIndexNode(SEC_UINT32 ulIndex, SEC_List_S* pList)
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL void *SEC_LIST_getIndexNode(SEC_UINT32 ulIndex, SEC_List_S *pList);
 ...
 *
 * @par Note
 * - If the datasize of the pSrcList is 0 and the pFuncCpy is NULL than SEC_listCopy will return fail
 * N/A
 *
 * @par Related Topics
 * N/A
 ...
 * @par Prototype
 * @code
 * SEC_List_S* SEC_LIST_sort(SEC_List_S *pList, PFUNC_CMP pfCmp)
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_List_S *SEC_LIST_sort(SEC_List_S *pList, PFUNC_CMP pfCmp);
 ...
 * @param[out] N/A N/A [N/A]
 *
 * @retval SEC_List_S* An empty list [SEC_List_S*|N/A]
 *
 * @par Dependency
 * sec_list.h
 *
 ...
 * @par Prototype
 * @code
 * SEC_VOID *SEC_LIST_curr  (const SEC_List_S *pstList);
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_VOID *SEC_LIST_curr(const SEC_List_S *pstList);
 ...
 * @par Prototype
 * @code
 * SEC_INT32 SEC_LIST_getElmtIndex  (const SEC_VOID *elmt, SEC_List_S *pstList);
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_INT32 SEC_LIST_getElmtIndex(const SEC_VOID *elmt, SEC_List_S *pstList);
 ...
 * @par Prototype
 * @code
 * SEC_List_S *SEC_LIST_concat  (SEC_List_S *pDestList, const SEC_List_S *pSrcList);
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @par Note
 * - If the total number of nodes of pDestList + pSrcList is greater than the value set by the SEC_LIST_setMaxElements
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_List_S *SEC_LIST_concat(SEC_List_S *pDestList, const SEC_List_S *pSrcList);
...
/**
 * @defgroup SEC_setMaxQsortCount
 * @ingroup sec_listFunctions
 * @par Prototype
 * @code
 * SEC_INT32 SEC_setMaxQsortCount(SEC_UINT32 uiQsortSize)
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_INT32 SEC_setMaxQsortCount(SEC_UINT32 uiQsortSize);

/**
 * @defgroup SEC_getMaxQsortCount
 * @ingroup sec_listFunctions
 * @par Prototype
 * @code
 * SEC_UINT32 SEC_getMaxQsortCount()
 * @endcode
 *
 * @par Purpose
 ...
 * @param[out] N/A N/A [N/A]
 *
 * @retval SEC_UINT32 Returns PSE qsort Size[N/A|N/A]
 *
 * @par Dependency
 * sec_common.h
 *
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_UINT32 SEC_getMaxQsortCount(SEC_VOID);
