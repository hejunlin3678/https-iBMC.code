 ...
 * Description: Contains wrapper interface functions for OpenSSL Big Num functions
 * ALL RIGHTS RESERVED
 * Project Code: iPSI
 ...
 * --------------------------------------------------------------------
 */
#ifndef SEC_OSSL_WRAP_BN_H
#define SEC_OSSL_WRAP_BN_H
 ...
 * N/A
 */
CRYPTOLINKDLL IPSI_OSSL_WRAP_BIGNUM *ipsi_ossl_wrap_BN_new(void);
 ...
 * @par Purpose
 * Frees the components of the IPSI_OSSL_WRAP_BIGNUM, and if it was created by ipsi_ossl_wrap_BN_new(),
 * also the structure itself. Additionally it is used to destroy important data in memory such as keys when they
 ...
 * N/A
 */
CRYPTOLINKDLL IPSI_OSSL_WRAP_BN_CTX *ipsi_ossl_wrap_BN_CTX_new(void);
 ...
 * ipsi_ossl_wrap_BN_MONT_CTX_new(), also the structure itself.
 *
 * @param[in] mont  Pointer to IPSI_OSSL_WRAP_BN_MONT_CTX structure which needs to be freed [N/A]
 *
 * @retval SEC_VOID This function does not return any value.[NA|NA]
 *
 * @par Required Header File
...
* N/A
*/
CRYPTOLINKDLL SEC_INT ipsi_ossl_wrap_BN_MONT_CTX_set(IPSI_OSSL_WRAP_BN_MONT_CTX *mont, const IPSI_OSSL_WRAP_BIGNUM *mod,
    IPSI_OSSL_WRAP_BN_CTX *ctx);
 ...
 * 5. If the flag IPSI_XBN_FLG_CONSTTIME is set for any of the parm p,m then API will return SEC_ERROR.\n
 * 6. If m_ctx is not NULL then the modulus value should be set by the application in the m_ctx using
 * ipsi_ossl_wrap_BN_MONT_CTX_set.\n
 *
 * @par Related Topics
 * N/A
 */
CRYPTOLINKDLL SEC_INT ipsi_ossl_wrap_BN_mod_exp_mont_word(IPSI_OSSL_WRAP_BIGNUM *r, IPSI_OSSL_WRAP_BN_ULONG a,
    ...
    IPSI_OSSL_WRAP_BN_MONT_CTX *m_ctx);
 ...
 * as NULL.\n
 * 3. If the flag IPSI_XBN_FLG_CONSTTIME is set for any of the parm a,p,m then consttime operation will be performed.\n
 *
 * @par Related Topics
 * N/A
 */
 ...
 *
 * @par Note
 * The top variable value of the IPSI_OSSL_WRAP_BIGNUM passed structure should not be more than 1048576.
 *
 * @par Related Topics
 * N/A
 */
 ...
 * @retval IPSI_OSSL_WRAP_BN_ULONG On error or if NULL pointer is passed as big number or if value itself is zero.[0
 * |NA]
 *
 * @par Required Header File
 * sec_ossl_wrap_bn.h
 ...
 * @par Prototype
 * @code
 * SEC_INT ipsi_ossl_wrap_BN_is_negative(const IPSI_OSSL_WRAP_BIGNUM *a)
 * @endcode
 *
 * @par Purpose
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_INT ipsi_ossl_wrap_BN_is_negative(const IPSI_OSSL_WRAP_BIGNUM *a);
 ...
 * @param[in] iFlag The Flags to set in the big number . [N/A]
 *
 * @retval SEC_INT On success.[1|NA]
 ...
 * @par Prototype
 * @code
 * SEC_INT ipsi_ossl_wrap_BN_get_flags(const IPSI_OSSL_WRAP_BIGNUM *a, SEC_INT iFlags)
 * @endcode
 *
 * @par Purpose
 ...
 *
 * @param[in] a The BIGNUM in which the flag has to be set. [N/A]
 * @param[in] iFlags The Flags to set in the big number . [N/A]
 *
 ...
 * N/A
 */
CRYPTOLINKDLL SEC_INT ipsi_ossl_wrap_BN_get_flags(const IPSI_OSSL_WRAP_BIGNUM *a, SEC_INT iFlags);
 ...
 * @par Prototype
 * @code
 * IPSI_OSSL_WRAP_BIGNUM *ipsi_ossl_wrap_BN_copy(IPSI_OSSL_WRAP_BIGNUM *dst, const IPSI_OSSL_WRAP_BIGNUM *src)
 * @endcode
 *
 * @par Purpose
...
CRYPTOLINKDLL IPSI_OSSL_WRAP_BIGNUM *ipsi_ossl_wrap_BN_copy(IPSI_OSSL_WRAP_BIGNUM *dst,
    const IPSI_OSSL_WRAP_BIGNUM *src);
...
*
* @par Note
* 1. This API fails if any one of the paramter is passed as NULL.\n
* 2. If the flag IPSI_XBN_FLG_CONSTTIME is set for any of the parm a,p,m then consttime operation will be performed.\n
* 3. If the flag IPSI_XBN_FLG_CONSTTIME is set for any of the parm a,p,m & m is odd then API will return SEC_ERROR.\n
*
* @par Related Topics
* N/A
*/
