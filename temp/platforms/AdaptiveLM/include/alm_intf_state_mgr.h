...
typedef enum {
    ...
    /* License或者Feature快到截止日期,触发PreDeadlineAlarm */
...
/* 设置是否开启在Feature宽限期内每日上报Feature宽限告警功能
 * 该功能默认不开启，若要开启该功能，可通过该API传ALM_TRUE
 * 该API需要在ALM_Start之前调用
 */
LIC_EXPORT ALM_INT32 ALM_AlarmFeatureGracePeriodAlarmSwitch(IN ALM_BOOL bTurnOn);

/* 设置开启或者关闭固定期限License、永久+固定期限License，在Feature的Deadline到期前N天开始上报周期性告警
 * iPrdId产品Id;
 * bSwitch: ALM_TRUE, 开启PreDeadline周期性告警功能；
 * bSwitch: ALM_FALSE, 关闭PreDeadline周期性告警功能；
 */
...
/* 设置固定期限License、永久+固定期限License，在Feature的Deadline到期前N天开始上报周期性告警
 * iPrdId产品Id;
 * iPreDay: 到期前N天开始上报。默认值是0，设置后，该值会一直有效，直到下一次设置。
 * 如果调用ALM_AlarmSetPreDeadlineAlarmSwitch(iPrdId, ALM_TRUE)来开启该功能，
 * 但是没有调用ALM_AlarmSetPreDeadlineAlarmDays来设置提前N天, 则N就是默认值0，此时亦不会上报到期前告警
 */
